/**
 * Drop leftover merch from wave18 and backfill missing ingredients from product pages.
 */
...
async function main() {
  ...
  const cleaned = all.filter((p) => !(isWave17(p) && MERCH.test(p.name)));
  const dropped = before - cleaned.length;

  const missing = cleaned.filter((p) => isWave17(p) && (!p.ingredients || p.ingredients.length === 0));
  let filled = 0;
  ...
  const wave = cleaned.filter(isWave17);
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave18Products: wave.length,
    wave18WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave18-ingredients-backfill.json"),
