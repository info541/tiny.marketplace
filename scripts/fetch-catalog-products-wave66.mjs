/**
 * Fetch Shopify catalogs for wave66 brands (c1266+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave66.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave66.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave66Products: waveCount,
        wave66BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave66-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave66=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
