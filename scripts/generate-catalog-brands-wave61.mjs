/**
 * Write src/lib/catalog-brands-wave61.ts from the wave61 seed.
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { brands } from "./catalog-brands-seed-wave61.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const out = path.join(__dirname, "../src/lib/catalog-brands-wave61.ts");
...
const rows = brands.map((b, i) => {
  const id = `c${String(i + 1235).padStart(4, "0")}`;
  return `  {
    id: ${tsString(id)},
