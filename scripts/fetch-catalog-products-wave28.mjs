/**
 * Fetch Shopify catalogs for wave28 brands (c907+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave28.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave28.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave28Products: waveCount,
        wave28BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave28-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave28=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
