/**
 * Drop leftover merch from wave12 and backfill missing ingredients from product pages.
 */
...
async function main() {
  ...
  const cleaned = all.filter((p) => !(isWave12(p) && MERCH.test(p.name)));
  const dropped = before - cleaned.length;

  const missing = cleaned.filter((p) => isWave12(p) && (!p.ingredients || p.ingredients.length === 0));
  let filled = 0;
  ...
  const wave = cleaned.filter(isWave12);
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave12Products: wave.length,
    wave12WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave12-ingredients-backfill.json"),
