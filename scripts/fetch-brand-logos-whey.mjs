/**
 * Download logos for organic whey brands into public/brands/{slug}.png|.svg
 * and patch logoUrl paths in src/lib/catalog-brands-whey.ts
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { brands } from "./catalog-brands-seed-whey.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const outDir = path.join(__dirname, "../public/brands");
const catalogPath = path.join(__dirname, "../src/lib/catalog-brands-whey.ts");
...
for (const [slug, file] of Object.entries(bySlug)) {
  catalog = catalog.replace(
    new RegExp(`(slug: ${JSON.stringify(slug)}[\\s\\S]*?logoUrl: )"\\/brands\\/${slug}\\.(?:png|svg)"`),
    `$1"${file}"`,
  );
}
...
fs.writeFileSync(
  path.join(__dirname, "../data/brand-logo-manifest-whey.json"),
