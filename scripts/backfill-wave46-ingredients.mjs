/**
 * Drop leftover merch from wave46 and backfill missing ingredients from product pages.
 */
...
function reclassify(p) {
  ...
  const n = Number(String(p.brandId).replace(/^c/, ""));
  if (n === 1136) p.category = "oral";
  if (n === 1137 || n === 1138) p.category = "sunscreen";
  if (n === 1139) {
  ...
  if (n === 1140) p.category = "skincare";
  if (n === 1141 || n === 1142) p.category = "electrolytes";
  if (n === 1143) p.category = "protein";
  if (n === 1144) p.category = "hair";
  if (n === 1145) {
...
async function main() {
  ...
  const cleaned = all
    .filter((p) => {
      ...
      return !(MERCH.test(p.name) || EXTRA_DROP.test(p.name) || EXTRA_DROP_WAVE38.test(p.name) || EXTRA_DROP_WAVE41.test(p.name) || EXTRA_DROP_WAVE42.test(p.name) || EXTRA_DROP_WAVE43.test(p.name) || EXTRA_DROP_WAVE46.test(p.name));
    })
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave46Products: wave.length,
    wave46WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave46-ingredients-backfill.json"),
