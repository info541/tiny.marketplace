/**
 * Fetch Shopify catalogs for wave55 brands (c1199+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave55.mjs";
...
function flavorOptionIndex(raw) {
  ...
  const scored = options.map((o, i) => {
    ...
    let score = 1;
    if (/flavor|flavour|scent|fragrance|smell|aroma|colour|color|shade|tint|material/i.test(name)) score = 3;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave55.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave55Products: waveCount,
        wave55BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave55-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave55=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
