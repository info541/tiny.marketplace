/**
 * Generate logos for wave3 brands (uplead → favicon → SVG mark).
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { brands } from "./catalog-brands-seed-wave3.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const outDir = path.join(__dirname, "../public/brands");
const catalogPath = path.join(__dirname, "../src/lib/catalog-brands-wave3.ts");
...
async function downloadOne(brand, tryRemote) {
...
const results = [];
// Try remote logos for first 200 only (rate/time); SVG for the rest
for (let i = 0; i < brands.length; i++) {
  const brand = brands[i];
  try {
    const r = await downloadOne(brand, i < 200);
    results.push(r);
    if ((i + 1) % 100 === 0) console.log(`logos ${i + 1}/${brands.length}`);
  } catch (e) {
...
fs.writeFileSync(
  path.join(__dirname, "../data/brand-logo-manifest-wave3.json"),
