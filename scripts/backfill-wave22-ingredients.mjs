/**
 * Drop leftover merch from wave22 and backfill missing ingredients from product pages.
 */
...
async function main() {
  ...
  const cleaned = all.filter((p) => !(isWave22(p) && MERCH.test(p.name)));
  const dropped = before - cleaned.length;

  const missing = cleaned.filter((p) => isWave22(p) && (!p.ingredients || p.ingredients.length === 0));
  let filled = 0;
  ...
  const wave = cleaned.filter(isWave22);
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave22Products: wave.length,
    wave22WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave22-ingredients-backfill.json"),
