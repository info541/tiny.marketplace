/**
 * Fetch Shopify catalogs for wave9 brands (c561+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave9.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave9.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave9Products: waveCount,
        wave9BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave9-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave9=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
