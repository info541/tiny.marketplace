/**
 * Drop leftover merch from wave13 and backfill missing ingredients from product pages.
 */
...
async function main() {
  ...
  const cleaned = all.filter((p) => !(isWave13(p) && MERCH.test(p.name)));
  const dropped = before - cleaned.length;

  const missing = cleaned.filter((p) => isWave13(p) && (!p.ingredients || p.ingredients.length === 0));
  let filled = 0;
  ...
  const wave = cleaned.filter(isWave13);
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave13Products: wave.length,
    wave13WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave13-ingredients-backfill.json"),
