/**
 * Drop leftover merch from wave23 and backfill missing ingredients from product pages.
 */
...
async function main() {
  ...
  const cleaned = all.filter((p) => !(isWave23(p) && MERCH.test(p.name)));
  const dropped = before - cleaned.length;

  const missing = cleaned.filter((p) => isWave23(p) && (!p.ingredients || p.ingredients.length === 0));
  let filled = 0;
  ...
  const wave = cleaned.filter(isWave23);
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave23Products: wave.length,
    wave23WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave23-ingredients-backfill.json"),
