/**
 * Fetch Shopify catalogs for wave34 brands (c1008+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave34.mjs";
...
function refineCategory(category, title, brand) {
  ...
  if (/\b(hydration drops?|electrolytes?|electrolyte powder|yuzu citrus|asian pear|white peach|lychee)\b/.test(t) && !/\bprotein\b/.test(t)) {
    return "electrolytes";
  }
...
function isOnNiche(raw, brand) {
  ...
  if (skipped) {
    ...
    if (!keepDespiteEssentialOil && !keepSample && !keepTallowAftershave) return false;
  }
  if (NICHE.test(hay) || /\bscrub\b/i.test(hay)) return true;
  if (brand.slug === "zebra" && /\b(the tablets|the floss|the toothpaste|the deodorant|the toothbrush|the trio|the eye cream)\b/i.test(title)) {
    return true;
  }
  if (brand.slug === "tuttofare" && /\b(tallow|deodorant|balm|mist|cleansing|body oil|face bundle|tallow trio)\b/i.test(title)) {
    return true;
  }
  if (brand.slug === "own-skin" && /\b(tallow|deodorant|dry shampoo|after sun|lip balm|shave|baby|bakuchiol|whip)\b/i.test(title)) {
    return true;
  }
  if (brand.slug === "earthling-co" && /\b(shampoo|conditioner|body wash|dry shampoo|serum|lotion|lip balm|gummies|hair oil)\b/i.test(title)) {
    return true;
  }
  if (brand.slug === "first-curve" && /\b(extract|tincture|bitters|tonic|salve|infused oil|potion)\b/i.test(hay)) {
    return true;
  }
  if (brand.slug === "shelter-skin" && /\b(tallow|body (oil|butter)|lip elixir|bath melt)\b/i.test(title)) {
    return true;
  }
  if (brand.slug === "denttabs" && /\b(zahnputz|toothpaste tablet|denttabs|kids set|yearly set)\b/i.test(title)) {
    return true;
  }
  if (brand.slug === "boba-tea-protein" && /\b(protein|whey|sample)\b/i.test(title)) {
    return true;
  }
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave34.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave34Products: waveCount,
        wave34BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave34-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave34=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
