/**
 * Fetch Shopify catalogs for wave85 brands (c1379+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave85.mjs";
...
const WAVE85_EXTRA =
  /\b(gift card|e-gift|gift pack|gift set|gift basket|mystery (box|gift)|shipping protection|package protection|savedby|navidium|build your (own )?bundle|bundles?\b|duo\b|duet\b|trio\b|variety pack|holiday gift|shaker bottle|shakers?\b|whisk\b|sampler pack|cashback|enamel pin|empty bottle|frother|mixer|water bottle|sachet|sample kit|sweatshirt|hoodie|\bhat\b|scrunchie|pillowcase|keychain|tote|socks|tumbler|straws?|hand blender|canvas tote|protein tin|crew socks|magnetic shopping|pre-workout|preworkout|gummies|pancake|banana bread|quick oats|hot choccy|protein bar|snack|ashy bines (whey|vegan) protein bundle|neem comb|sisal|soap dish|sponsorship|ebook|tank "?your sunscreen)\b/i;
...
function refineCategory(category, title, brand) {
  ...
  if (brand?.slug === "zebra") {
    ...
    if (/lip gloss|eye cream|puffy/.test(t)) return "skincare";
    return "oral";
  }
  if (brand?.slug === "nion-health") {
    if (/capsule|bioelectric/.test(t)) return "supplements";
    return "electrolytes";
  }
  if (brand?.slug === "happy-way") {
    if (/hydrat|electrolyt/.test(t) && !/protein|whey|vegan/.test(t)) return "electrolytes";
    if (/collagen|creatine|magnesium|greens|reds/.test(t) && !/protein|whey/.test(t)) {
      return "supplements";
    }
    return "protein";
  }
  if (brand?.slug === "icarus") {
    if (/spf|sun|zinc|tin|tube/.test(t) && !/aftersun|soap|mousse/.test(t)) return "sunscreen";
    return "skincare";
  }
  if (brand?.slug === "tallowbourn") {
    if (/deodorants?/.test(t)) return "deodorant";
    if (/sun balm|zinc/.test(t)) return "sunscreen";
    return "skincare";
  }
  if (brand?.slug === "hikakai") return "hair";
  return category;
}
...
function isOnNiche(raw, brand) {
  ...
  if (SKIP_TITLE.test(title) || WAVE85_EXTRA.test(title)) return false;
  if (Number((raw.variants || [])[0]?.price || 0) <= 0) return false;
  if (brand.slug === "zebra") {
    if (/\b(mini|set|jet set|campus|hydration set|ultimate clean|trio|sweatshirt|hat|scrunchie|pillowcase|keychain|puff -|gift|savedby|package protection)\b/i.test(title)) {
      return false;
    }
    return /\b(toothpaste|tablets?|floss|toothbrush|deodorants?)\b/i.test(hay);
  }
  if (brand.slug === "nion-health") {
    if (/\b(2 jars|4 jars|120 capsules|240 capsules)\b/i.test(title)) return false;
    return /\b(electrolyte|capsule|bioelectric)\b/i.test(hay);
  }
  if (brand.slug === "happy-way") {
    if (
      /\b(bundle|sample|sachets?|30g|60g|900g|bar|oat|pancake|banana bread|hot choc|gummies|pre-workout|preworkout|shaker|tote|socks|tumbler|straw|blender|scoop|tin w\/|gift|merch|men'?s |ashy bines (whey|vegan) protein bundle)\b/i.test(
        title,
      )
    ) {
      return false;
    }
    if (/protein water/i.test(title)) return true;
    return /\b(whey|vegan protein|protein powder|hydration|electrolyt|collagen|creatine|magnesium|hemp vegan)\b/i.test(
      hay,
    );
  }
  if (brand.slug === "icarus") {
    if (/\b(tank|bundle|starter pack|complete skin)\b/i.test(title)) return false;
    return /\b(spf|sun|zinc|aftersun|soap)\b/i.test(hay);
  }
  if (brand.slug === "tallowbourn") {
    if (/\b(gift|kit|bundle|sample|3 pack|3-pack)\b/i.test(title)) return false;
    return /\b(deodorants?|sun balm|tallow balm|soap)\b/i.test(hay);
  }
  if (brand.slug === "hikakai") {
    if (/\b(comb|neem wood)\b/i.test(title)) return false;
    if (handle === "shikakai-hairwash") return false;
    return /\b(hairwash|shampoo|champi|keshasuddhi|rinse|oil)\b/i.test(hay);
  }
...
function flavorOptionIndex(raw, brand) {
  ...
  if (brand?.slug === "zebra" || brand?.slug === "nion-health" || brand?.slug === "icarus" || brand?.slug === "hikakai") {
    return -1;
  }
  if (brand?.slug === "happy-way") {
    return options.findIndex((o) => /flavou?r|wanna switch/i.test(o.name || ""));
  }
  if (brand?.slug === "tallowbourn") {
    return options.findIndex((o) => /style|scents?|flavor/i.test(o.name || ""));
  }
...
async function fetchCurrency(base, brand) {
  try {
    ...
    if (brand?.slug === "happy-way" && currency === "USD") currency = "AUD";
    return currency;
  } catch {
    return "USD";
  }
...
function pickVariant(variants, brand) {
  ...
  const scored = variants.map((v, i) => {
    ...
    if (/\b(two-pack|2-pack|three-pack|3-pack|four-pack|4-pack|2 boxes|4 boxes|6 boxes|0\.5oz|0\.5 oz|20 servings|3x90|250 g|250g|500 g|500g|1 kg|1kg|5g|sample size|refill only)\b/i.test(hay))
      score -= 8;
    if (brand?.slug === "happy-way") {
      if (/\b(500 ?g|500g)\b/i.test(hay)) score += 8;
      if (/\b(1 ?kg|2 ?kg|30g|60g|6x60|sample)\b/i.test(hay)) score -= 12;
    }
    if (brand?.slug === "tallowbourn") {
      if (/\b2 oz\b/i.test(hay)) score += 8;
      if (/\b(1 oz|4 oz|3 pack|3-pack)\b/i.test(hay)) score -= 10;
    }
    if (brand?.slug === "nion-health") {
      if (/60 capsules/i.test(hay)) score += 10;
      if (/120|240|2 jars|4 jars/i.test(hay)) score -= 14;
    }
    if (brand?.slug === "zebra") {
      if (/classic tortoise(?! mini)/i.test(hay)) score += 6;
    }
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel, currency) {
  ...
  if (brand.slug === "nion-health") {
    baseTitle = baseTitle
      .replace(/\s*-\s*60 Capsules$/i, " Capsules")
      .replace(/\s*-\s*30 Day Pack$/i, "")
      .replace(/\s+/g, " ")
      .trim();
  }
  if (brand.slug === "happy-way") {
    baseTitle = baseTitle
      ...
      .replace(/\s+/g, " ")
      .trim();
    if (!/^happy way\b/i.test(baseTitle)) baseTitle = `Happy Way ${baseTitle}`;
  }
  if (brand.slug === "icarus") {
    if (/^body spf 30/i.test(baseTitle)) baseTitle = "Icarus Body SPF 30 Squeeze Tube";
    if (/^spf 30 - white/i.test(baseTitle)) baseTitle = "Icarus SPF 30 White Tin";
    if (/^spf 30, 2 oz\. tin/i.test(baseTitle)) baseTitle = "Icarus SPF 30 Cacao Tin";
    if (/^daily aftersun/i.test(baseTitle)) baseTitle = "Icarus Daily Aftersun Mousse";
    if (/^ultra-rich soap/i.test(baseTitle)) baseTitle = "Icarus Goat Milk Olive Oil Soap";
  }
  if (brand.slug === "tallowbourn") {
    if (/^extra strength/i.test(baseTitle)) baseTitle = "Tallowbourn Extra Strength Deodorant";
    if (/^baking soda free/i.test(baseTitle)) baseTitle = "Tallowbourn Baking Soda Free Deodorant";
    if (/^tallow sun balm/i.test(baseTitle)) baseTitle = "Tallowbourn Tallow Sun Balm";
    if (/^grass-fed tallow balm/i.test(baseTitle)) baseTitle = "Tallowbourn Tallow Balm";
    if (/^grass-fed tallow soap/i.test(baseTitle)) baseTitle = "Tallowbourn Tallow Soap";
    if (/^athlete's shield/i.test(baseTitle)) baseTitle = "Tallowbourn Athlete's Shield Soap";
    if (/^natural tallow lip/i.test(baseTitle)) baseTitle = "Tallowbourn Tallow Lip Balm";
  }
  if (brand.slug === "hikakai") {
    if (/^shikakai hairwash/i.test(baseTitle)) baseTitle = "Hikakai Shikakai Hairwash";
    if (/^champi/i.test(baseTitle)) baseTitle = "Hikakai Champi Pre-Wash Oil";
    if (/^herbal post-wash/i.test(baseTitle)) baseTitle = "Hikakai Keshasuddhi Rinse";
  }
  ...
  title = title
    ...
    .replace(/&#39;/g, "'")
    .replace(/&quot;/g, '"');
  if (brand.slug === "happy-way" || brand.slug === "zebra") {
  ...
  if (brand.slug === "nion-health" || brand.slug === "happy-way") {
    ingredients = [];
  }
  if (brand.slug === "zebra") {
    const text = stripHtml(body);
    if (/toothpaste|tablets/i.test(title) && /xylitol|calcium carbonate|hydrated silica/i.test(text)) {
      ingredients = [
        "Xylitol",
        "Water",
        ...
        "Glycerin",
        "Calcium Carbonate",
        /coconut/i.test(title) ? "Natural Coconut Flavor" : /birthday|cake|vanilla/i.test(title) ? "Natural Vanilla Flavor" : "Mentha Piperita (Peppermint) Oil",
  ...
  if (brand.slug === "icarus") {
    const text = stripHtml(body);
    if (/spf|zinc|sun/i.test(title) && /zinc/i.test(text)) {
      const list = [
        "Non-nano Zinc Oxide",
        "Shea Butter",
        "Coconut Oil",
      ...
      if (!/white|no-tint|no cacao/i.test(title)) list.push("Cacao Powder");
      if (/body|squeeze tube/i.test(title)) {
        list.push("Squalane", "MCT Oil");
      }
      ingredients = list;
    } else if (/aftersun|mousse/i.test(title) && /shea|aloe/i.test(text)) {
      ingredients = [
        "Shea Butter",
        "Coconut Oil",
        "Aloe Vera",
    ...
    } else if (/soap/i.test(title) && /goat/i.test(text)) {
      ingredients = ["Goat Milk", "Saponified Extra Virgin Olive Oil"];
    }
  }
  if (brand.slug === "tallowbourn") {
    const text = stripHtml(body);
    if (/baking soda free/i.test(title) && /tallow|magnesium hydroxide/i.test(text)) {
      ingredients = [
  ...
  if (brand.slug === "hikakai" && /hairwash/i.test(title)) {
    const text = stripHtml(body);
    if (/coconut|shikakai|argan|castor/i.test(text) || true) {
      ingredients = [
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave85.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave85Products: waveCount,
        wave85BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave85-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave85=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
