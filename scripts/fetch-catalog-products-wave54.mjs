/**
 * Fetch Shopify catalogs for wave54 brands (c1193+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave54.mjs";
...
const FLAVOR_OPTION = /flavor|flavour|scent|fragrance|smell|aroma|colour|color|shade|scented|finish|constitutive|herb|fluord|fluoride|the taste|dry shampoo colour|style|tint|skin tone|formula|whip|blend|select|infusi[oó]n|\bmaku\b|botanical bars|choose your scent|select one|^product$|^variants$/i;
...
function isOnNiche(raw, brand) {
  ...
  const skipped =
    ...
    SKIP_TYPE.test(type) ||
    SKIP_TAGS.test(tags);
  if (WAVE54_SKIP.test(title) || WAVE54_EXTRA.test(title)) return false;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave54.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave54Products: waveCount,
        wave54BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave54-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave54=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
