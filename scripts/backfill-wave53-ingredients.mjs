/**
 * Drop leftover merch from wave53 and backfill missing ingredients from product pages.
 */
...
function reclassify(p) {
  ...
  const n = Number(String(p.brandId).replace(/^c/, ""));
  if (n === 1184) p.category = "oral";
  if (n === 1185) {
    if (/\b(castor oil)\b/.test(t)) p.category = "skincare";
    else if (/\b(toxin binder|pearl powder)\b/.test(t)) p.category = "supplements";
    else p.category = "oral";
  }
  if (n === 1186) p.category = /\bdeodorant\b/.test(t) ? "deodorant" : "skincare";
  if (n === 1187) {
    if (/\b(whey|protein)\b/.test(t)) p.category = "protein";
    else if (/\b(electrolyt|hydrate)\b/.test(t)) p.category = "electrolytes";
    else p.category = "supplements";
  }
  if (n === 1188) p.category = "electrolytes";
  if (n === 1189) p.category = "sunscreen";
  if (n === 1190) p.category = "skincare";
  if (n === 1191) p.category = /\b(hair|beard)\b/.test(t) ? "hair" : "skincare";
  if (n === 1192) p.category = "skincare";
  if (n === 1171) {
...
async function main() {
  ...
  const cleaned = all
    .filter((p) => {
      ...
      return !(MERCH.test(p.name) || EXTRA_DROP.test(p.name) || EXTRA_DROP_WAVE38.test(p.name) || EXTRA_DROP_WAVE41.test(p.name) || EXTRA_DROP_WAVE42.test(p.name) || EXTRA_DROP_WAVE43.test(p.name) || EXTRA_DROP_WAVE46.test(p.name) || EXTRA_DROP_WAVE47.test(p.name) || EXTRA_DROP_WAVE51.test(p.name));
    })
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave53Products: wave.length,
    wave53WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave53-ingredients-backfill.json"),
