/**
 * Fetch Shopify catalogs for wave47 brands (c1146+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave47.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave47.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave47Products: waveCount,
        wave47BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave47-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave47=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
