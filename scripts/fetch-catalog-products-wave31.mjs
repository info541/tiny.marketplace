/**
 * Fetch Shopify catalogs for wave31 brands (c970+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave31.mjs";
...
const FX_TO_USD = {
  "earth-herd-harvest": 0.65, // AUD
  "black-chicken": 0.65, // AUD
  "pure-tallow-eu": 1.08, // EUR
  "nz-tallow": 0.59, // NZD
  "mushrooms-for-life": 1.27, // GBP
  "pure-press-oils": 0.73, // CAD
};
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave31.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave31Products: waveCount,
        wave31BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave31-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave31=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
