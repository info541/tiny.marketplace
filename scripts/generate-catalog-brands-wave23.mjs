/**
 * Write src/lib/catalog-brands-wave23.ts from the wave23 seed.
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { brands } from "./catalog-brands-seed-wave23.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const out = path.join(__dirname, "../src/lib/catalog-brands-wave23.ts");
...
const rows = brands.map((b, i) => {
  const id = `c${String(i + 828).padStart(3, "0")}`;
  return `  {
    id: ${tsString(id)},
...
/** Wave 23: small organic / toxic-free brands across marketplace categories (c828+). */
export const catalogBrandsWave23: Brand[] = [
${rows.join(",\n")},
];
`;
