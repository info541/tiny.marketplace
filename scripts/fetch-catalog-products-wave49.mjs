/**
 * Fetch Shopify catalogs for wave49 brands (c1159+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave49.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave49.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave49Products: waveCount,
        wave49BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave49-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave49=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
