/**
 * Drop leftover merch from wave71 and backfill missing ingredients from product pages.
 */
...
const WAVE_START = 1295;
const WAVE_END = 1300;

const EXTRA_DROP =
  /\b(gift card|e-gift|bundle|duo\b|duet\b|trio\b|2 pack|3 pack|4 pack|ritual\b|makeover|bathroom cup|travel size|travel pack|shaker|skillet|bath soak|sticker|hoodie|beeswax melt|greeting|soap saver|subscription|novelty|pet shampoo|tinted\s*lip|tinted strawberry)\b/i;
...
function looksLikeInci(parts, raw) {
  ...
  if (
    parts.length >= 3 &&
    /water|aqua|glycerin|sodium|acid|extract|oil|oxide|protein|vitamin|tallow|wax|butter|zinc|salt|magnesium|potassium|gum|xylitol|hydroxyapatite|clay|shea|alcohol|honey|jojoba|goat milk|creatine/i.test(
      raw,
    )
  ) {
    return true;
  }
  if (parts.length >= 2 && /tallow|oil|wax|butter|zinc|beeswax|xylitol|hydroxyapatite|alcohol|goat milk/i.test(raw) && /,/.test(raw)) {
    return true;
  }
...
function sanitizeIngredients(parts) {
  ...
  for (const part of parts) {
    ...
    if (
      ...
      !/oil|butter|tallow|clay|wax|extract|oxide|acid|salt|magnesium|glycerin|hydroxyapatite|xylitol|beeswax|shea|goat milk/i.test(
        s,
      )
    ) {
      continue;
...
const PUBLISHED = [
  {
    test: (p) => p.brandId === "c1295" && /nano-hydroxyapatite|n-ha/i.test(p.name),
    ingredients: [
      "Water",
      "Xylitol",
      "Sorbitol",
  ...
  {
    test: (p) => p.brandId === "c1298" && /face cream|body butter/i.test(p.name),
    ingredients: [
      "3X clarified grass-fed beef suet (tallow)",
      "Organic coconut oil",
      "Organic beeswax",
      "Vitamin E",
      "Jojoba oil",
...
for (const p of products) {
  ...
  if (/toothbrush/i.test(p.name)) {
    if (seenToothbrush.has(p.brandId)) {
      dropped += 1;
      continue;
    }
    p.name = "Before Toothbrush";
...
fs.writeFileSync(
  path.join(root, "data/wave71-ingredients-backfill.json"),
  JSON.stringify(
    {
