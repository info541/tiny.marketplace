/**
 * Fetch Shopify catalogs for wave56 brands (c1207+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave56.mjs";
...
function isOnNiche(raw, brand) {
  ...
  const skipped =
    ...
    SKIP_TYPE.test(type) ||
    SKIP_TAGS.test(tags);
  if (WAVE56_EXTRA.test(title) || WAVE54_SKIP.test(title) || WAVE54_EXTRA.test(title)) return false;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave56.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave56Products: waveCount,
        wave56BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave56-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave56=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
