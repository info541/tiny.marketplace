/**
 * Fetch Shopify catalogs for wave102 brands (c1481+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave102.mjs";
...
function refineCategory(category, title, brand) {
  ...
  if (brand?.slug === "great-oral-health") return "oral";
  if (brand?.slug === "fire-salt") {
    if (/pre-workout|torched/i.test(t)) return "supplements";
    return "electrolytes";
  }
  if (brand?.slug === "chizfit") {
    if (/protein/i.test(t)) return "protein";
    return "skincare";
  }
  if (brand?.slug === "kokua-sun-care") {
    if (/sunscreen|spf/i.test(t)) return "sunscreen";
    return "skincare";
  }
  if (brand?.slug === "creations-by-the-creek") {
    if (/deodorant/i.test(t)) return "deodorant";
    if (/made in the shade|after sun/i.test(t)) return "sunscreen";
    if (/shampoo/i.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "rooted-mushrooms") return "supplements";
  return category;
}

const GOH_KEEP = new Set([
  "kids-yummy-orange-flavored-nano-hydroxyapatite-fluoride-free-toothpaste-copy-1",
  "fluoride-free-toothpaste-with-nano-hydroxyapatite-peppermint-wow-flavor",
  "fluoride-free-toothpaste-with-nano-hydroxyapatite-citrus-flavor",
  "orarestore-nano-hydroxyapatite-toothpaste",
  "dental-oral-probiotics-with-blis-k12-m18-mint-flavor",
  "oral-probiotics-kids-blis-k12-m18",
  "patent-7-strain-formulation-oral-probiotics",
  "our-crystlcare-biorestorative-dental-strips",
  "mt-fuji-bristlepure-bamboo-toothbrushes-3-pack",
  "ergonomically-designed-tongue-cleaner",
  "orarestore-blend-for-healthy-gums",
  "oraprotecx-hydroxyapatite-sugar-blocker-gum",
]);

const CHIZFIT_KEEP = new Set([
  "vanilla-plant-based-protein",
  "coffee-plant-based-protein",
  "eucalyptus-peppermint-liquid-castile-body-wash",
  "fire-grit-pre-workout-energizing-soap",
  "clean-calm-everyday-fresh-soap",
  "charcoal-eucalyptus-hemp-sports-soap-bar",
  "body-and-face-serum",
]);

const KOKUA_KEEP = new Set([
  "kokua-sun-care-hawaiian-natural-zinc-sunscreen-spf-50-80-min-water-resistance-3-oz",
  "spf-30-tinted-natural-hawaiian-sunscreen-3oz",
  "natural-hawaiian-sun-healing-serum-with-hyaluronic-acid-vitamin-c-hawaiian-sandalwood-and-aloe-vera",
]);

const CREEK_KEEP = new Set([
  "after-sun-spray",
  "timeless-whipped-moisturizer",
  "timeless-serum",
  "timeless-oil-cleanser",
  "timeless-beauty-balm",
  "thinning-hair-tallow-shampoo-bar",
  "tallow-made-in-the-shade",
  "tallow-deodorant",
  "spearmint-tallow-lip-balm",
  "rose-tallow-cream",
  "rosehip-tallow-soap",
  "rose-tallow-body-butter",
  "rose-mint-tallow-lip-balm",
  "peppermint-tallow-lip-balm",
  "rose-clay-mask-2oz",
  "original-tallow-soap",
  "original-whipped-moisturizer",
  "original-tallow-balm",
  "oily-hair-tallow-shampoo-bar",
  "normal-hair-tallow-shampoo-bar",
  "new-moon-pomegranate-hydrating-serum-1oz",
  "new-moon-nourishing-3-in-1-beauty-balm-2oz",
  "new-moon-hibiscus-brightening-toner-2oz",
  "new-moon-frankincense-moisturizer-2oz",
  "new-moon-hibiscus-cleansing-grains-50ml",
  "new-moon-frankincense-cleansing-oil-2oz",
  "new-moon-blue-spirulina-mask-2oz",
  "magnesium-spray",
  "lavender-tallow-soap",
  "lavender-tallow-lip-balm",
  "lavender-tallow-cream",
  "lavender-tallow-body-butter",
  "healthier-hair-tallow-shampoo-bar",
  "dry-hair-tallow-shampoo-bar",
  "eye-repair-balm",
  "crystal-hydrating-toner-2oz",
  "full-moon-rosehip-calming-serum-1oz",
  "full-moon-helichrysum-moisturizer-2oz",
  "full-moon-lavender-cleansing-grains-50ml",
  "full-moon-green-tea-mask-2oz",
  "full-moon-helichrysum-cleansing-oil-2oz",
  "full-moon-calming-3-in-1-beauty-balm-2oz",
  "full-moon-calendula-soothing-toner-2oz",
]);

const ROOTED_KEEP = new Set(["tinctures", "mushroom-powder"]);
...
function isOnNiche(raw, brand) {
  ...
  if (brand.slug === "great-oral-health") return GOH_KEEP.has(handle);
  if (brand.slug === "fire-salt") {
    return /^(fire-salt-electrolytes-32-pack|torched-pre-workout)$/.test(handle);
  }
  if (brand.slug === "chizfit") return CHIZFIT_KEEP.has(handle);
  if (brand.slug === "kokua-sun-care") return KOKUA_KEEP.has(handle);
  if (brand.slug === "creations-by-the-creek") return CREEK_KEEP.has(handle);
  if (brand.slug === "rooted-mushrooms") return ROOTED_KEEP.has(handle);
  if (SKIP_TITLE.test(title) || WAVE102_EXTRA.test(title) || WAVE102_EXTRA.test(handle)) return false;
...
function flavorOptionIndex(raw, brand) {
  ...
  const handle = raw.handle || "";
  if (brand?.slug === "great-oral-health") return -1;
  if (brand?.slug === "chizfit") return -1;
  if (brand?.slug === "kokua-sun-care") return -1;
  if (brand?.slug === "creations-by-the-creek") return -1;
  if (brand?.slug === "fire-salt") {
    return options.findIndex((o) => /flavou?r|pre-workout/i.test(o.name || ""));
  }
  if (brand?.slug === "rooted-mushrooms") {
    if (handle === "tinctures") return options.findIndex((o) => /variety/i.test(o.name || ""));
    if (handle === "mushroom-powder") return options.findIndex((o) => /title/i.test(o.name || ""));
    return -1;
  }
...
function pickVariant(variants, brand, raw) {
  ...
  const scored = variants.map((v, i) => {
    ...
    if (/\b(two-pack|2-pack|three-pack|3-pack|four-pack|4-pack|2 boxes|4 boxes|6 boxes|0\.5oz|0\.5 oz|20 servings|3x90|250 g|250g|500 g|500g|1 kg|1kg|5g|sample size|refill only|2 jars|3 jars|5 jars|2 bars|3 bars|8 oz extra|16 oz|5lb|5 lb|family size|81 servings|15 servings|30 ct|3\.3 oz|refill cartridge|30g sample)\b/i.test(hay))
      score -= 8;
    if (brand?.slug === "great-oral-health") {
      if (/\b1 (pack|bottle|toothbrush|kids toothpaste|peppermint|citrus|wintermint|orarestore|crystlcare pack - 14)\b/i.test(hay))
        score += 14;
      if (/\b(2|3|6) (pack|bottles|toothbrushes|toothpastes|crystlcare)\b/i.test(hay)) score -= 14;
    }
    if (brand?.slug === "fire-salt") {
      if (/variety pack/i.test(hay)) score -= 20;
    }
    if (brand?.slug === "creations-by-the-creek") {
      if (/\b4\s*oz\b/i.test(hay)) score += 8;
      if (/\b2\s*oz\b/i.test(hay)) score += 4;
      if (/1\/2\s*oz|3ml/i.test(hay)) score -= 8;
    }
    if (brand?.slug === "rooted-mushrooms") {
      if (/\b1\s*oz\b/i.test(hay)) score += 12;
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel, currency) {
  ...
  if (brand.slug === "great-oral-health") {
    const h = raw.handle || "";
    if (/kids-yummy-orange/.test(h))
      baseTitle = "Great Oral Health Kids ProMineralizer Toothpaste — Yummy Orange";
    else if (/peppermint-wow/.test(h))
      baseTitle = "Great Oral Health ProMineralizer Toothpaste — Peppermint Wow";
    else if (/citrus-flavor/.test(h))
      baseTitle = "Great Oral Health ProMineralizer Toothpaste — Citrus Blast";
    else if (/orarestore-nano-hydroxyapatite-toothpaste/.test(h))
      baseTitle = "Great Oral Health ProMineralizer Toothpaste — Wintermint";
    else if (/dental-oral-probiotics-with-blis-k12-m18-mint/.test(h))
      baseTitle = "Great Oral Health Dental Oral Probiotics — Mint";
    else if (/oral-probiotics-kids/.test(h))
      baseTitle = "Great Oral Health Kids Dental Oral Probiotics — Strawberry Vanilla";
    else if (/patent-7-strain/.test(h))
      baseTitle = "Great Oral Health Dental Oral Probiotics — Strawberry Vanilla";
    else if (/crystlcare/.test(h)) baseTitle = "Great Oral Health CrystLCare Biorestorative Dental Strips";
    else if (/mt-fuji/.test(h)) baseTitle = "Great Oral Health BristlePure Bamboo Toothbrush";
    else if (/tongue-cleaner/.test(h)) baseTitle = "Great Oral Health Stainless Steel Tongue Scraper";
    else if (/orarestore-blend/.test(h)) baseTitle = "Great Oral Health OraRestore Tooth & Gum Oil";
    else if (/oraprotecx/.test(h)) baseTitle = "Great Oral Health OraProtecX Hydroxyapatite Gum";
  }
  if (brand.slug === "fire-salt") {
    const h = raw.handle || "";
    if (/fire-salt-electrolytes-32-pack/.test(h)) baseTitle = "Fire Salt Essential Hydration";
    else if (/torched-pre-workout/.test(h)) baseTitle = "Fire Salt Torched Pre-Workout";
  }
  if (brand.slug === "chizfit") {
    const h = raw.handle || "";
    if (h === "vanilla-plant-based-protein") baseTitle = "Chizfit Plant-Based Protein — Vanilla";
    else if (h === "coffee-plant-based-protein") baseTitle = "Chizfit Plant-Based Protein — Coffee";
    else if (h === "eucalyptus-peppermint-liquid-castile-body-wash")
      baseTitle = "Chizfit Eucalyptus Peppermint Castile Body Wash";
    else if (h === "fire-grit-pre-workout-energizing-soap")
      baseTitle = "Chizfit Fire & Grit Citrus Spice Soap";
    else if (h === "clean-calm-everyday-fresh-soap") baseTitle = "Chizfit Clean & Calm Everyday Soap";
    else if (h === "charcoal-eucalyptus-hemp-sports-soap-bar")
      baseTitle = "Chizfit Charcoal Eucalyptus Hemp Sports Soap";
    else if (h === "body-and-face-serum") baseTitle = "Chizfit Hemp & Vitamin E Face and Body Serum";
  }
  if (brand.slug === "kokua-sun-care") {
    const h = raw.handle || "";
    if (/hawaiian-natural-zinc-sunscreen-spf-50/.test(h) && /3-oz/.test(h))
      baseTitle = "Kōkua Hawaiian Natural Zinc Sunscreen SPF 50";
    else if (/spf-30-tinted/.test(h)) baseTitle = "Kōkua Tinted Hawaiian Natural Zinc Sunscreen SPF 30";
    else if (/aftersun|sun-healing-serum/.test(h))
      baseTitle = "Kōkua Hawaiian Aftersun Serum";
  }
  if (brand.slug === "creations-by-the-creek") {
    if (!/^creations by the creek\b/i.test(baseTitle))
      baseTitle = `Creations by the Creek ${baseTitle}`;
  }
  if (brand.slug === "rooted-mushrooms") {
    const h = raw.handle || "";
    if (h === "tinctures") baseTitle = "Rooted Mushrooms Fruiting-Body Tincture";
    else if (h === "mushroom-powder") baseTitle = "Rooted Mushrooms Mushroom Powder";
  }
  ...
  if (
    brand.slug === "great-oral-health" ||
    brand.slug === "fire-salt" ||
    brand.slug === "chizfit" ||
    brand.slug === "creations-by-the-creek"
  ) {
    ingredients = [];
  }
  if (brand.slug === "kokua-sun-care") {
    if (/spf 50/i.test(title)) {
      ingredients = [
        ...
        "Hawaiian Macadamia ternifolia seed oil",
        "Hawaiian Aleurites moluccana (kukui) seed oil",
  ...
  if (brand.slug === "rooted-mushrooms") {
    if (/tincture/i.test(title)) {
      const variety = (flavorClean || title).replace(/^.*?—\s*/, "");
      ingredients = [
        `Idaho-grown ${variety} mushroom (fruiting body)`,
        "Purified water",
        "Ethanol",
      ];
    } else if (/powder/i.test(title)) {
      const variety = (flavorClean || title).replace(/^.*?—\s*/, "").replace(/\s*powder$/i, "").trim();
      const label = /daily blend/i.test(variety) ? "Daily Blend mushrooms" : `${variety} mushroom`;
      ingredients = [`Idaho-grown ${label} (fruiting body)`];
    } else {
      ingredients = [];
    }
  }
...
function expandProducts(raw, brand, brandId, index, currency) {
  ...
  if (flavorIdx >= 0 && variants.length > 1) {
    ...
    for (const v of variants) {
      ...
      if (brand?.slug === "fire-salt" && /variety pack/i.test(label)) continue;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave102.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave102Products: waveCount,
        wave102BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave102-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave102=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
