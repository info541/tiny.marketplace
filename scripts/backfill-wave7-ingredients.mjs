/**
 * Drop leftover merch from wave7 and backfill missing ingredients from product pages.
 */
...
async function main() {
  ...
  const cleaned = all.filter((p) => !(isWave7(p) && MERCH.test(p.name)));
  const dropped = before - cleaned.length;

  const missing = cleaned.filter((p) => isWave7(p) && (!p.ingredients || p.ingredients.length === 0));
  let filled = 0;
  ...
  const wave = cleaned.filter(isWave7);
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave7Products: wave.length,
    wave7WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave7-ingredients-backfill.json"),
