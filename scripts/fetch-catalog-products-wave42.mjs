/**
 * Fetch Shopify catalogs for wave42 brands (c1098+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave42.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave42.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave42Products: waveCount,
        wave42BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave42-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave42=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
