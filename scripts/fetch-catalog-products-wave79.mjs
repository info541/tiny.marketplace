/**
 * Fetch Shopify catalogs for wave79 brands (c1343+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave79.mjs";
...
function refineCategory(category, title, brand) {
  ...
  if (brand?.slug === "oh-care") return "oral";
  if (brand?.slug === "evolve-essentials") return "electrolytes";
  if (brand?.slug === "florida-mushroom-company" || brand?.slug === "florida-shroom-king") {
    return "supplements";
  }
  if (brand?.slug === "bodi-blends") {
    if (/creatine/.test(t)) return "supplements";
    return "protein";
  }
  if (brand?.slug === "skin-candy") {
    if (/\b(shampoo|conditioner|hair|beard|scalp)\b/.test(t)) return "hair";
    return "skincare";
  }
  return category;
}
...
function isOnNiche(raw, brand) {
  ...
  if (SKIP_TITLE.test(title) || WAVE79_EXTRA.test(title)) return false;
  if (brand.slug === "oh-care") {
    if (/\b(triple pack|duo|trio|shot glass|travel size|clearance|50ml)\b/i.test(title)) return false;
    return /toothpaste|mouthwash|gum serum|ha-x|hydroxyapatite|nanoxim/i.test(hay);
  }
  if (brand.slug === "evolve-essentials") {
    return /electrolyt|salted (lemonade|watermelon|tropical)|passionfruit|watermelon|lemonade/i.test(hay);
  }
  if (brand.slug === "bodi-blends") {
    if (/\b(cap|hat|tee|hoodie)\b/i.test(title)) return false;
    return /wpi|whey|protein|creatine/i.test(hay);
  }
  if (brand.slug === "skin-candy") {
    if (
      /\b(laundry|dish soap|candle|wax melt|shower steamer|bath bomb|room spray|linen spray|toilet|grab bag|soap ends|mystery|bundle|kit|home fragrance)\b/i.test(
        title,
      )
    ) {
      return false;
    }
    return /\b(shampoo|conditioner|soap|lotion|scrub|lip|beard|whipped soap|tallow|goat milk)\b/i.test(hay);
  }
  if (brand.slug === "florida-mushroom-company") {
    if (/\b(tee|tank|cap|hat|seasoning|ooh-mami|bundle|merch)\b/i.test(title)) return false;
    return /extract|lion'?s mane|reishi|cordyceps|turkey tail/i.test(hay);
  }
  if (brand.slug === "florida-shroom-king") {
    if (
      /\b(market friends|fresh|stack|bundle|chef|pioppino|oyster mushroom|culinary)\b/i.test(title)
    ) {
      return false;
    }
    return /triple extract|lion'?s mane|reishi|turkey tail|chaga|trifecta/i.test(hay);
  }
...
function flavorOptionIndex(raw, brand) {
  ...
  if (
    brand?.slug === "oh-care" ||
    brand?.slug === "evolve-essentials" ||
    brand?.slug === "bodi-blends" ||
    brand?.slug === "florida-mushroom-company" ||
    brand?.slug === "florida-shroom-king"
  ) {
    return -1;
  }
  if (brand?.slug === "skin-candy") {
    return options.findIndex((o) => /scent|fragrance|flavou?r/i.test(o.name || ""));
  }
...
async function fetchCurrency(base, brand) {
  if (brand?.slug === "oh-care" || brand?.slug === "bodi-blends") return "AUD";
  if (brand?.slug === "evolve-essentials") return "CAD";
  try {
...
function pickVariant(variants, brand) {
  ...
  const scored = variants.map((v, i) => {
    ...
    let score = 0;
    if (/\b(single|single pack|1 pack|default title|1 kg|2 oz|2 fl oz|60 ml)\b/i.test(hay)) score += 8;
    if (/\b(two-pack|2-pack|three-pack|3-pack|four-pack|4-pack|1 oz|30 ml)\b/i.test(hay)) score -= 8;
    if (brand?.slug === "florida-shroom-king") {
      if (/\b2\s*(fl\s*)?oz\b/i.test(hay) || /\b60\s*ml\b/i.test(hay)) score += 10;
      if (/\b1\s*(fl\s*)?oz\b/i.test(hay) || /\b30\s*ml\b/i.test(hay)) score -= 6;
    }
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel, currency) {
  ...
  title = title
    ...
    .replace(/&#39;/g, "'")
    .replace(/&quot;/g, '"');
  if (brand.slug === "evolve-essentials" && /^salted\b/i.test(title) && !/^evolve\b/i.test(title)) {
    title = `Evolve ${title}`;
  }
  if (brand.slug === "oh-care" && /^(h2 mouthwash|50ml)/i.test(title) && !/^oh/i.test(title)) {
    title = `OH! Care ${title}`;
  }
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave79.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave79Products: waveCount,
        wave79BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave79-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave79=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
