/**
 * Fetch Shopify catalogs for wave14 brands (c661+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave14.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave14.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave14Products: waveCount,
        wave14BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave14-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave14=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
