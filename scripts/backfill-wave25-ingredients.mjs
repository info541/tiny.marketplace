/**
 * Drop leftover merch from wave25 and backfill missing ingredients from product pages.
 */
...
async function main() {
  ...
  const cleaned = all.filter((p) => !(isWave25(p) && MERCH.test(p.name)));
  const dropped = before - cleaned.length;

  const missing = cleaned.filter((p) => isWave25(p) && (!p.ingredients || p.ingredients.length === 0));
  let filled = 0;
  ...
  const wave = cleaned.filter(isWave25);
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave25Products: wave.length,
    wave25WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave25-ingredients-backfill.json"),
