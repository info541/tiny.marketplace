/**
 * Fetch Shopify catalogs for wave57 brands (c1212+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave57.mjs";
...
function isOnNiche(raw, brand) {
  ...
  const skipped =
    ...
    SKIP_TYPE.test(type) ||
    SKIP_TAGS.test(tags);
  if (WAVE57_EXTRA.test(title) || WAVE56_EXTRA.test(title) || WAVE54_SKIP.test(title) || WAVE54_EXTRA.test(title)) return false;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave57.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave57Products: waveCount,
        wave57BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave57-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave57=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
