/**
 * Drop leftover merch from wave26 and backfill missing ingredients from product pages.
 */
...
async function main() {
  ...
  const cleaned = all.filter((p) => !(isWave26(p) && MERCH.test(p.name)));
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave26Products: wave.length,
    wave26WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave26-ingredients-backfill.json"),
