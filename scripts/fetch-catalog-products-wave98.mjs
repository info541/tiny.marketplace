/**
 * Fetch Shopify catalogs for wave98 brands (c1457+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave98.mjs";
...
function refineCategory(category, title, brand) {
  ...
  if (brand?.slug === "toothpow") return "oral";
  if (brand?.slug === "saltivate") return "electrolytes";
  if (brand?.slug === "purist-nutrition") {
    if (/hydrat|electrolyte/i.test(t)) return "electrolytes";
    if (/pre-workout|creatine/i.test(t)) return "supplements";
    return "protein";
  }
  if (brand?.slug === "aussiezinc") return "sunscreen";
  if (brand?.slug === "lathr") {
    if (/deodorant/i.test(t)) return "deodorant";
    if (/shampoo|conditioner|hair|beard|pomade|scalp/i.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "vesper-mushrooms") {
    if (/cream|dermush/i.test(t)) return "skincare";
    return "supplements";
  }
  return category;
}
...
function isOnNiche(raw, brand) {
  ...
  if (SKIP_TITLE.test(title) || WAVE98_EXTRA.test(title) || WAVE98_EXTRA.test(handle)) return false;
  if (Number((raw.variants || [])[0]?.price || 0) <= 0) return false;
  if (brand.slug === "toothpow") {
    return handle === "toothpow-spearmint";
  }
  if (brand.slug === "saltivate") {
    if (/\b(wholesale|tumbler|bottle|shaker|towel|organizer|variety|case of|merch)\b/i.test(hay)) {
      return false;
    }
    return /^(watermelon-electrolytes-jar|mango-pineapple-electrolytes-jar|orange-electrolytes-jar|jar-raw-90)$/.test(
      handle,
    );
  }
  if (brand.slug === "purist-nutrition") {
    if (
      /\b(bundle|tee\b|hoodie|hat\b|cap\b|sock|jogger|flannel|shaker|duffel|polo|zip|bar|gift)\b/i.test(
        title,
      ) ||
      /tee|hoodie|hat|cap|sock|jogger|flannel|shaker|duffel|polo|bundle|bar/.test(handle)
    ) {
      return false;
    }
    return /^(purist-whey-protein|purist-creatine-plus|purist-hydration|purist-pre-workout)$/.test(
      handle,
    );
  }
  if (brand.slug === "aussiezinc") {
    if (/mozzie|wholesale/i.test(hay)) return false;
    return /organic-mineral-zinc-sunscreen-spf50|aussiezinc-kids-organic-mineral-sunscreen-spf50|baby-organic-sunscreen-spf50/.test(
      handle,
    );
  }
  if (brand.slug === "lathr") {
    if (
      /\b(cologne|extrait|edp|dupe|bundle|kit|gift|merch|towel|bag|beanie|hoodie|tee\b|buzz off|room spray|air freshener|soap saver|2 pak|2-pak|starter|safari|sample|collection|essentials)\b/i.test(
        title,
      ) ||
      /cologne|extrait|edp|dupe|bundle|kit|gift|merch|buzz-off|room-spray|air-freshener|soap-saver|2-pak|starter|sample/.test(
        handle,
      )
    ) {
      return false;
    }
    if (/^natural-deodorant-/.test(handle) && !/starter/.test(handle)) return true;
    if (handle === "clear-deodorant") return true;
    if ((/soap-bar/.test(handle) || /\bsoap bar\b/i.test(title)) && !/2-pak|2 pak|buzz/i.test(hay))
      return true;
    if (/^(shampoo-|conditioner-)/.test(handle) && !/essentials/.test(handle)) return true;
    if (/body-butter/.test(handle)) return true;
    if (handle === "beef-tallow-skin-balm") return true;
    if (/lavender-lullaby-tallow-balm|citrus-sunrise-tallow-balm/.test(handle)) return true;
    if (/mens-natural-face-wash|mens-natural-face-cream/.test(handle)) return true;
    return false;
  }
  if (brand.slug === "vesper-mushrooms") {
    if (
      /\b(bundle|gift|tee\b|hoodie|hat\b|poster|bottle|wholesale|spour|coffee|matcha|chai|shirt)\b/i.test(
        title,
      ) ||
      /bundle|gift|tee|hoodie|poster|bottle|wholesale|spour|coffee|shirt/.test(handle)
    ) {
      return false;
    }
    if (/dermush-body-cream-4-oz/.test(handle)) return false;
    return /\b(extract|dermush)\b/i.test(hay);
  }
...
function flavorOptionIndex(raw, brand) {
  ...
  const handle = raw.handle || "";
  if (brand?.slug === "toothpow") return -1;
  if (brand?.slug === "saltivate") return -1;
  if (brand?.slug === "aussiezinc") return -1;
  if (brand?.slug === "vesper-mushrooms") return -1;
  if (brand?.slug === "purist-nutrition") {
    return options.findIndex((o) => /flavor/i.test(o.name || ""));
  }
  if (brand?.slug === "lathr") {
    if (/clear-deodorant/.test(handle)) {
...
function pickVariant(variants, brand, raw) {
  ...
  const scored = variants.map((v, i) => {
    ...
    if (/\b(two-pack|2-pack|three-pack|3-pack|four-pack|4-pack|2 boxes|4 boxes|6 boxes|0\.5oz|0\.5 oz|20 servings|3x90|250 g|250g|500 g|500g|1 kg|1kg|5g|sample size|refill only|2 jars|3 jars|5 jars|2 bars|3 bars|8 oz extra|16 oz|5lb|5 lb|family size|81 servings|15 servings|30 ct|3\.3 oz|refill cartridge)\b/i.test(hay))
      score -= 8;
    if (brand?.slug === "purist-nutrition") {
      if (/30 serving bag/i.test(hay)) score += 10;
      if (/15 single packets|stick pack/i.test(hay)) score -= 12;
    }
    if (brand?.slug === "lathr") {
      if (/414 ml|14 fl/i.test(hay)) score += 8;
      if (/jumbo|946|32 fl/i.test(hay)) score -= 8;
    }
    if (brand?.slug === "vesper-mushrooms") {
      if (/\b2\s?oz\b|\b8\s?oz\b/i.test(hay)) score += 6;
      if (/\b4\s?oz\b/i.test(hay) && /body cream/i.test(hay)) score -= 8;
    }
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel, currency) {
  let baseTitle = (raw.title || "")
    ...
    .trim();
  if (brand.slug === "toothpow") {
    baseTitle = "ToothPow Spearmint Tooth Powder";
  }
  if (brand.slug === "saltivate") {
    if (/watermelon/i.test(baseTitle)) baseTitle = "Saltivate Electrolytes — Watermelon";
    else if (/mango/i.test(baseTitle)) baseTitle = "Saltivate Electrolytes — Mango Pineapple";
    else if (/orange/i.test(baseTitle)) baseTitle = "Saltivate Electrolytes — Orange";
    else baseTitle = "Saltivate Electrolytes — Raw Unflavored";
  }
  if (brand.slug === "purist-nutrition") {
    if (/whey/i.test(baseTitle)) baseTitle = "Purist Grass-Fed Whey Protein Isolate";
    else if (/creatine/i.test(baseTitle)) baseTitle = "Purist Creatine Plus";
    else if (/hydration/i.test(baseTitle)) baseTitle = "Purist Hydration Powder";
    else if (/pre-workout/i.test(baseTitle)) baseTitle = "Purist Pre-Workout Powder";
    else if (!/^purist\b/i.test(baseTitle)) baseTitle = `Purist ${baseTitle}`;
  }
  if (brand.slug === "aussiezinc") {
    if (/vitamin c|anti-aging/i.test(baseTitle)) baseTitle = "AussieZinc Vitamin C Daily Glow Mineral SPF 50";
    else if (/tint|bb cream/i.test(baseTitle)) baseTitle = "AussieZinc Sheer Medium Tint Mineral SPF 50";
    else if (/extra sensitive/i.test(baseTitle)) baseTitle = "AussieZinc Extra Sensitive Mineral SPF 50";
    else if (/kids/i.test(baseTitle)) baseTitle = "AussieZinc Kids Mineral SPF 50";
    else if (/baby/i.test(baseTitle)) baseTitle = "AussieZinc Baby Mineral SPF 50";
    else baseTitle = "AussieZinc Everyday Mineral SPF 50";
  }
  if (brand.slug === "lathr") {
    if (!/^lathr\b/i.test(baseTitle)) baseTitle = `LATHR ${baseTitle}`;
  }
  if (brand.slug === "vesper-mushrooms") {
    baseTitle = baseTitle.replace(/^\s*NEW!\s*/i, "").replace(/\s*\(4oz\.?\/120ML\).*$/i, "").trim();
    if (!/^vesper/i.test(baseTitle)) baseTitle = `Vesper ${baseTitle}`;
  }
  ...
  if (
    brand.slug === "toothpow" ||
    brand.slug === "purist-nutrition" ||
    brand.slug === "aussiezinc" ||
    brand.slug === "lathr"
  ) {
    ingredients = [];
  }
  if (brand.slug === "saltivate") {
    if (/watermelon/i.test(title)) {
      ingredients = [
        "Sodium chloride",
        "Malic acid",
        "Potassium citrate",
      ...
      ingredients = ["Sodium chloride", "Potassium citrate", "Dimagnesium malate"];
    }
  }
  if (brand.slug === "vesper-mushrooms") {
    if (/dermush face/i.test(title)) {
      ingredients = [
        "Shea butter",
    ...
    } else if (/dermush body/i.test(title)) {
      ingredients = [
        "Aloe vera",
        "Snow Mushroom extract",
        "Reishi extract",
    ...
    } else if (/alcohol-free/i.test(title) && /cordyceps/i.test(title)) {
      ingredients = [
        "Fruiting body Cordyceps (Cordyceps militaris)",
        "Wild huckleberry",
        "Distilled water",
        "Organic vegetable glycerin from coconut oil",
      ];
    } else if (/alcohol-free/i.test(title) && /reishi/i.test(title)) {
      ingredients = [
        "Fruit body Reishi (Ganoderma lucidum)",
        "Wild huckleberry",
        "Distilled water",
        "Organic vegetable glycerin from coconut oil",
      ];
    } else if (/alcohol-free/i.test(title) && /turkey tail/i.test(title)) {
      ingredients = [
        "Fruiting body Turkey Tail (Trametes versicolor)",
        "Wild huckleberry",
        "Distilled water",
        "Organic vegetable glycerin from coconut oil",
      ];
    } else if (/alcohol-free/i.test(title) && /chaga/i.test(title)) {
      ingredients = [
        "Fruiting body Chaga (Inonotus obliquus)",
        "Wild huckleberry",
        "Distilled water",
        "Organic vegetable glycerin from coconut oil",
      ];
    } else if (/alcohol-free/i.test(title) && /lion/i.test(title)) {
      ingredients = [
        "Fruit body Lion's Mane (Hericium erinaceus)",
        "Wild huckleberry",
        "Distilled water",
        "Organic wheat ethanol",
      ];
    } else if (/gray matter/i.test(title)) {
      ingredients = [
        "Fruiting bodies of Lion's Mane, Cordyceps, Reishi, Chaga, and Turkey Tail",
        "Wild huckleberry",
        "Distilled water",
        "Organic wheat ethanol",
        "Spagyric minerals",
      ];
    } else if (/gold metal/i.test(title)) {
      ingredients = [
        "Fruiting body Cordyceps (Cordyceps militaris)",
        "Fruiting body Lion's Mane (Hericium erinaceus)",
        "Wild huckleberry",
        "Distilled water",
        "Organic wheat ethanol",
      ];
    } else if (/pink freud/i.test(title)) {
      ingredients = [
        "Fruiting body Lion's Mane (Hericium erinaceus)",
        "Fruiting body Reishi (Ganoderma lucidum)",
        "Wild huckleberry",
        "Distilled water",
        "Organic wheat ethanol",
      ];
    } else if (/chaga/i.test(title)) {
      ingredients = [
        "Fruiting body Chaga (Inonotus obliquus)",
        "Wild huckleberry",
        "Distilled water",
    ...
    } else if (/turkey tail/i.test(title)) {
      ingredients = [
        "Fruiting body Turkey Tail (Trametes versicolor)",
        "Wild huckleberry",
        "Distilled water",
    ...
    } else if (/maitake/i.test(title)) {
      ingredients = [
        "Fruiting body Maitake (Grifola frondosa)",
        "Wild huckleberry",
        "Distilled water",
        "Organic wheat ethanol",
      ];
...
function expandProducts(raw, brand, brandId, index, currency) {
  ...
  if (flavorIdx >= 0 && variants.length > 1) {
    ...
    for (const v of variants) {
      ...
      if (brand?.slug === "purist-nutrition" && /\+|bundle|packets/i.test(label)) continue;
      if (brand?.slug === "lathr" && /^\s*\d+(\.\d+)?\s*(ml|oz|fl)/i.test(label)) continue;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave98.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave98Products: waveCount,
        wave98BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave98-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave98=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
