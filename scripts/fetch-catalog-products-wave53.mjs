/**
 * Fetch Shopify catalogs for wave53 brands (c1184+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave53.mjs";
...
const FLAVOR_OPTION = /flavor|flavour|scent|fragrance|smell|aroma|colour|color|shade|scented|finish|constitutive|herb|fluord|fluoride|the taste|dry shampoo colour|style|tint|skin tone|formula|whip|blend|infusi[oó]n|\bmaku\b|botanical bars|choose your scent|select one|^product$|^variants$/i;
...
const WAVE53_EXTRA =
  /\b(smile ritual|oral detox kit|jar & refill|jar and refill|2-pack refill|ceramic (tooth|dish)|tooth powder dish|brush rinse smile|mouth tape|remineralisation kit|tooth powder trio|mouthwash cup|toothpaste spatula|mother of pearl|oral irrigation|gum syringe|toothbrush holder|skincare set|sleep set|tone set|get better sleep|virtual skincare|jade facial|jade roller|arnica bath|gua sha|essentials bundle|drawstring bag|founders.? (hoodie|club)|pravi snapback|stainless steel bottle|glass water bottle|life in motion|hidrate or diedrate|lemon mascot|swim cap|squeeze bottle|try me|corduroy hat|lip trio|pure beef tallow collection|nourishment duo|variety bundle|b[áa]lsamo de sebo|soap saver|variety pack \(5)\b/i;
...
function isOnNiche(raw, brand) {
  ...
  const skipped =
    ...
    WAVE53_SKIP.test(title) ||
    WAVE53_EXTRA.test(title) ||
    WAVE51_SKIP.test(title) ||
    WAVE50_SKIP.test(title) ||
    SKIP_TYPE.test(type) ||
    SKIP_TAGS.test(tags);
  if (WAVE53_SKIP.test(title) || WAVE53_EXTRA.test(title)) return false;
  if (skipped) {
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel) {
  ...
  let title = flavorLabel ? `${baseTitle} — ${flavorLabel}` : baseTitle;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave53.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave53Products: waveCount,
        wave53BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave53-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave53=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
