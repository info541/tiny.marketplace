/**
 * Fetch Shopify catalogs for wave11 brands (c601+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave11.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave11.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave11Products: waveCount,
        wave11BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave11-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave11=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
