/**
 * Drop leftover merch from wave9 and backfill missing ingredients from product pages.
 */
...
const MERCH =
  /\b(stuffed toy|plush|crew socks|socks\b|head wrap|tote bag|statement tote|\btote\b|silk scarf|t-?shirt|\btee\b|hoodie|sweatshirt|beanie|sticker pack|sticker\b|mug\b|apparel|poster|patch\b|keychain|storybook|\bbook\b|journal\b|candle holder|gift card|e[- ]?gift|shaker bottle|water bottle|empty bottle|nalgene bottle|team bottle|terrain bottle|fuel bottle|fanny pack|luggage tag|enamel pin|marketing card|deodorant scoop|cream applicator|aluminum bottle|plastic bottle|blue bottle|brown bottle|clear bottle|clear jar|roll-on bottle|for pets?|\bpet\b|dog shampoo|daily dawg|there's a mushroom|shipping protection|package protection|product protection|vip protection|priority handling|labl guarantee|labl protect|returns & exchanges|\bbroker\b|refill station|bottle tray|\+ shirt|shot glass|softflask|club cap|trucker hat|sun hat|wide brim|digital download|gravity feed|mystery gift|100%\s*off|spatula|spf brush|travel brush|sample tin|imperfect|prepaid return envelope|gua sha|soap saver|facial scrubbie|storage tin|room & pillow spray|cologne spray|makeup brush|kabuki brush|electric mixer|onesie)\b/i;
...
async function main() {
  ...
  const cleaned = all.filter((p) => !(isWave8(p) && MERCH.test(p.name)));
  const dropped = before - cleaned.length;

  const missing = cleaned.filter((p) => isWave8(p) && (!p.ingredients || p.ingredients.length === 0));
  let filled = 0;
  ...
  const wave = cleaned.filter(isWave8);
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave9Products: wave.length,
    wave9WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave9-ingredients-backfill.json"),
