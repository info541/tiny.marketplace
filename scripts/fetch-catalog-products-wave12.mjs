/**
 * Fetch Shopify catalogs for wave12 brands (c621+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave12.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave12.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave12Products: waveCount,
        wave12BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave12-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave12=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
