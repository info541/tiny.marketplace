/**
 * Backfill missing wave73 ingredients from live product pages.
 */
...
function looksLikeInci(parts, raw) {
  ...
  if (
    parts.length >= 3 &&
    /water|aqua|glycerin|sodium|acid|extract|oil|oxide|protein|vitamin|tallow|wax|butter|zinc|salt|magnesium|potassium|gum|xylitol|hydroxyapatite|clay|shea|alcohol|honey|jojoba|goat milk|creatine|whey|theobromine/i.test(
      raw,
    )
  ) {
    return true;
...
function sanitizeIngredients(parts) {
  ...
  for (const part of parts) {
    ...
    if (!s) continue;
    if (
      /\b(product (benefits?|highlights?)|perfect for|elevate your|from farm to formula|how to|directions?|q:|note:|best for:|disclaimer|safety information|keep out of reach|for external use|this product has not been evaluated|available blends|why you.?ll love)\b/i.test(
        s,
      )
    ) {
      break;
...
function extractIngredients(html) {
  ...
  const markers = [
    ...
    /ingredients?\s+(?=glycerin|water|aqua|cocos|sodium|goat milk)([^\n]{12,2000})/i,
    /key ingredients?:\s*([^\n]{12,2000})/i,
  ];
  let best = [];
  for (const re of markers) {
    ...
    let raw = m[1];
    raw = raw.split(
      /(?:nutritional information|directions?|how to (?:use|apply)|reviews?|add to cart|best for|made in usa|rise\.\s*grind|customers also|you may also|food supplements should|this product is not intended)/i,
    )[0];
  ...
  const inci = text.match(
    /\b(?:Aqua|Water|Eau|Glycerin|Cocos Nucifera)\b(?:\s*\/\s*(?:Aqua|Water|Eau))?[^.]{0,40}(?:,\s*[A-Za-z0-9*][^,]{1,100}){4,60}/i,
  );
  if (inci) {
...
const PUBLISHED = [
  {
    test: (p) => p.brandId === "c1308",
    ingredients: [
      "Organic Irish Sea Salt (38%)",
      "Citric Acid",
      "Potassium Citrate",
      "Natural Flavouring",
  ...
  {
    test: (p) => p.brandId === "c1310" && /mineral (stick|balm)/i.test(p.name) && /tinted/i.test(p.name),
    ingredients: [
  ...
  {
    test: (p) => p.brandId === "c1310" && /mineral (stick|balm)/i.test(p.name),
    ingredients: [
      "Cocos Nucifera (Coconut) Oil",
      "Butyrospermum Parkii (Shea Butter)",
      "Cera Alba (Beeswax)",
...
fs.writeFileSync(
  path.join(root, "data/wave73-ingredients-backfill.json"),
  JSON.stringify({ generatedAt: new Date().toISOString(), filled, scraped, report }, null, 2),
);
