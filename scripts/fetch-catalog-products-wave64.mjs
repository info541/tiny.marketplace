/**
 * Fetch Shopify catalogs for wave64 brands (c1254+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave64.mjs";
...
function isOnNiche(raw, brand) {
  ...
  const wave64Drop = WAVE64_EXTRA.test(title);
  if (brand.slug === "keeko") {
    if (wave64Drop) return false;
  ...
  if (brand.slug === "oovii") {
    return /\b(face first|spf|sunscreen)\b/i.test(`${title} ${type}`) && !wave64Drop;
  }
  if (brand.slug === "bison-protein") {
    return /\bprotein\b/i.test(title) && !wave64Drop && Number(raw.variants?.[0]?.price || 0) > 0;
  }
  if (brand.slug === "massie-provisions") {
    if (wave64Drop) return false;
  ...
  if (brand.slug === "longevity-power") {
    if (wave64Drop || /\b(stack|bundle|kit|gift|pack)\b/i.test(title)) return false;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave64.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave64Products: waveCount,
        wave64BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave64-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave64=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
