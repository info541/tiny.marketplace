/**
 * Drop leftover merch from wave56 and backfill missing ingredients from product pages.
 */
...
const WAVE_START = 1207;
const WAVE_END = 1211;
// 5 brands: c1207–c1211
...
function reclassify(p) {
  ...
  if (n !== 1210 && /\b(hydrosol|hair spritz|solid shampoo|conditioner bar|active bar)\b/.test(t)) p.category = "hair";
...
async function main() {
  ...
  const cleaned = all
    .filter((p) => {
      ...
      return !(MERCH.test(p.name) || EXTRA_DROP.test(p.name) || EXTRA_DROP_WAVE38.test(p.name) || EXTRA_DROP_WAVE41.test(p.name) || EXTRA_DROP_WAVE42.test(p.name) || EXTRA_DROP_WAVE43.test(p.name) || EXTRA_DROP_WAVE46.test(p.name) || EXTRA_DROP_WAVE47.test(p.name) || EXTRA_DROP_WAVE51.test(p.name) || EXTRA_DROP_WAVE56.test(p.name));
    })
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave56Products: wave.length,
    wave56WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave56-ingredients-backfill.json"),
