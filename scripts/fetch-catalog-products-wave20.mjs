/**
 * Fetch Shopify catalogs for wave20 brands (c778+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave20.mjs";
...
function inferCategory(brandCategories, title, productType, tags) {
  ...
  const rules = [
    ...
    ["supplements", /\b(lion'?s mane|reishi|chaga|cordyceps|maitake|turkey tail|snow fungus|mushroom complex|mushroom extract|myco blend|synergy - \d+ mushrooms)\b/],
    ["electrolytes", /\b(electrolyt|oral rehydration|hydration (powder|packet|mix|stick|multiplier|pak)|rapid hydration|hydro pak|power pak|drink mix|wellness powder|immune support|hydust|organic hydration|servings|morning ritual|afternoon ritual)\b/],
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave20.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave20Products: waveCount,
        wave20BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave20-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave20=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
