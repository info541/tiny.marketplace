/**
 * Write src/lib/catalog-brands-wave28.ts from the wave28 seed.
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { brands } from "./catalog-brands-seed-wave28.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const out = path.join(__dirname, "../src/lib/catalog-brands-wave28.ts");
...
const rows = brands.map((b, i) => {
  const id = `c${String(i + 921).padStart(3, "0")}`;
  return `  {
    id: ${tsString(id)},
