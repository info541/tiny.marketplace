/**
 * Fetch Shopify catalogs for wave23 brands (c828+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave23.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave23.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave23Products: waveCount,
        wave23BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave23-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave23=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
