/**
 * Fetch Shopify catalogs for wave26 brands (c888+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave26.mjs";
...
function inferCategory(brandCategories, title, productType, tags) {
  ...
  const rules = [
    ["sunscreen", /\b(spf|sunscreen|sun\s?screen|sunblock|sun butter|sun paste|sunpaste|sun protection|sun stick|sun lotion|sun cream|after[- ]?sun|mineral defense|zinc stick)\b/],
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave26.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave26Products: waveCount,
        wave26BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave26-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave26=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
