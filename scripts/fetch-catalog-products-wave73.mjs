/**
 * Fetch Shopify catalogs for wave73 brands (c1307+).
 ...
 * - Converts cart.js currency (EUR × 1.08)
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { brands as seedBrands } from "./catalog-brands-seed-wave73.mjs";
...
const WAVE73_EXTRA =
  /\b(gift card|e-gift|gift pack|gift set|mystery box|shipping protection|build your (own )?bundle|bundle\b|duo\b|duet\b|trio\b|sets?\b|variety pack|holiday gift|shaker bottle|shakers?\b|sampler pack|ritual\b|makeover|bathroom cup|flavor pack|travel pack|2-pack|3-pack|4-pack|tinted\s*lip|ebook|directory|room spray|sunglasses|branded hat|sample pack|best sellers|lovers|ultimate bundle|family pack|power duo|starter kit|subscription|laundry|dish wash|pet bar|perfume|irregular|underweight|welcome gift|soap rest|soap saver|cotton gms|empty foamer|wood scoop|banner|replacement shipping|scalp massager|snuggle tray|shampoo bar tray|shampoo bar tin|lather bag|natural combs|hair brush|bundle builder|the humby club)\b/i;
...
function looksLikeInci(parts, raw) {
  ...
  if (
    parts.length >= 3 &&
    /water|aqua|glycerin|sodium|acid|extract|oil|oxide|protein|vitamin|tallow|wax|butter|zinc|salt|magnesium|potassium|gum|xylitol|hydroxyapatite|clay|shea|hydrobeef|cocoa|glycine|stevia|alcohol|honey|jojoba|goat milk|creatine|whey|theobromine/i.test(
      raw,
    )
  ) {
    return true;
  ...
  if (
    parts.length >= 2 &&
    /tallow|oil|wax|butter|zinc|beeswax|xylitol|hydroxyapatite|hydrobeef|alcohol|goat milk|whey/i.test(raw) &&
    /,/.test(raw)
  ) {
    return true;
  }
...
function extractIngredients(html) {
  ...
  const inci = text.match(
    /\b(?:Aqua|Water|Eau|Glycerin)\b(?:\s*\/\s*(?:Aqua|Water|Eau))?[^.]{0,40}(?:,\s*[A-Za-z0-9][^,]{1,100}){5,60}/i,
  );
  if (inci) {
...
function inferCategory(brandCategories, title, productType, tags) {
  ...
  const rules = [
    ["sunscreen", /\b(spf|sunscreen|sun\s?screen|sunblock|sun butter|sun[- ]?balm|sun cream|sun protectant|zinc stick|mineral zinc|mineral sun|mineral stick|mineral balm)\b/],
...
function refineCategory(category, title, brand) {
  ...
  if (brand?.slug === "grind-goods") return "oral";
  if (brand?.slug === "eirlyte") return "electrolytes";
  if (brand?.slug === "peachie-spoon") {
    if (/protein|whey/i.test(t)) return "protein";
    if (/electrolyt/i.test(t)) return "electrolytes";
    return "supplements";
  }
  if (brand?.slug === "fango") {
    if (/\b(lip)\b/.test(t)) return "skincare";
    return "sunscreen";
  }
  if (brand?.slug === "goat-milk-stuff") {
    if (/deodor/i.test(t)) return "deodorant";
    if (/\b(shampoo|hair|scalp)\b/.test(t)) return "hair";
    if (/tooth/i.test(t)) return "oral";
    return "skincare";
  }
  if (brand?.slug === "humby-organics") return "hair";
  return category;
}
...
function isOnNiche(raw, brand) {
  ...
  if (SKIP_TITLE.test(title) || WAVE73_EXTRA.test(title)) return false;
  if (brand.slug === "grind-goods") {
    if (/\b(3[- ]pack|2[- ]pack|adventurer|sweet tooth|the family|fresh & fruity|mint edition)\b/i.test(title)) {
      return false;
    }
    if (/fluoride free toothpaste/i.test(title)) return false;
    return /toothpaste|mint chip|achieve|advance|lemon love|cinnamon|purple pop|coastal|citrus|berry/i.test(
      hay,
    );
  }
  if (brand.slug === "eirlyte") {
    if (/\bbundle\b/i.test(title)) return false;
    return /electrolyt|eirlyte/i.test(hay);
  }
  if (brand.slug === "peachie-spoon") {
    if (/\b(bundle|t-shirt|tee|shirt|orchid|ivory)\b/i.test(title)) return false;
    return /protein|whey|creatine|collagen|electrolyt/i.test(hay);
  }
  if (brand.slug === "fango") {
    if (/\b(kit|family pack|power duo|bug|repellent|tanning oil)\b/i.test(title)) return false;
    return /\b(mineral stick|mineral balm|lip balm|sunscreen)\b/i.test(hay);
  }
  if (brand.slug === "goat-milk-stuff") {
    if (/landing page/i.test(type)) return false;
    if (
      /\b(naked|mega |luffa|travel goat|travel soap|tin goat milk lotion custom|tub goat milk lotion custom|small solid|lotion tub|super lip balm|refill liquid goat milk soap custom|soap savers?|cotton gms)\b/i.test(
        title,
      )
    ) {
      return false;
    }
    if (/\b(laundry|dish|pet|perfume|irregular|underweight|gift|banner|scoop|soap rest|shipping|foamer bottle)\b/i.test(title)) {
      return false;
    }
    return /\b(soap|deodorant|lotion|lip|balm|shampoo|tooth|salve|shaving|refill liquid)\b/i.test(hay);
  }
  if (brand.slug === "humby-organics") {
    if (/\b(bundle|subscription|the humby club|pack|tray|comb|brush|tin|bag|builder|massager|jumbo)\b/i.test(title)) {
      return false;
    }
    if (/shampoo\s*&\s*conditioner/i.test(title)) return false;
    if (/^(shampoo bars|conditioner bars)$/i.test(title)) return true;
    if (/humby man|anti-frizz oil|hair growth oil/i.test(title)) return true;
    return false;
  }
...
function flavorOptionIndex(raw, brand) {
  ...
  if (brand?.slug === "grind-goods" || brand?.slug === "peachie-spoon" || brand?.slug === "fango") {
    return -1;
  }
  if (brand?.slug === "eirlyte") {
  ...
  if (brand?.slug === "goat-milk-stuff") {
    if (/custom/i.test(raw.title || "")) {
      const scent = options.findIndex((o) => /scent|flavor|flavour/i.test(o.name || ""));
      if (scent >= 0) return scent;
    }
    return -1;
  }
  if (brand?.slug === "humby-organics") {
    if (/^(shampoo bars|conditioner bars|jumbo conditioner bar)$/i.test(raw.title || "")) {
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel, currency) {
  let baseTitle = (raw.title || "")
    ...
    .replace(/\s+Custom Goat Milk Lip Balms\s*$/i, "")
    .replace(/\s+Natural Tooth Care\s*$/i, "");
  ...
  if (
    flavorClean &&
    /^(default title|single|1 pack|2 pack|3 pack|30 servings|60 servings|90 servings|2oz|4oz|3oz|1oz|6g|scented|buy one|buy two|buy three|1 jar|2 jars|1 tub|2 tubs|3 tubs|250g|500g|1 lb\.?|2 lbs\.?|3 lbs\.?|12 pack|24 pack|36 pack|spf 30|spf 50|30 spf|50 spf)$/i.test(
      flavorClean,
    )
  ) {
    flavorClean = null;
...
function expandProducts(raw, brand, brandId, index, currency) {
  ...
  const mapped = mapOne(raw, brand, brandId, index, variants[0], null, currency);
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave73.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave73Products: waveCount,
        wave73BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave73-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave73=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
