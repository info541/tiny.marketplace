/**
 * Fetch Shopify catalogs for wave4 brands (c460+).
...
const WAVE_END = 460 + seedBrands.length - 1;

const SKIP_TITLE =
  /\b(gift card|e[- ]?gift|store credit|membership|subscription box|not for sale|tester only|wholesale|retail pack|family pack - retail|starter pack - retail|t-?shirt|tee\b|hoodie|sweatshirt|hat\b|cap\b|beanie|tote bag|sticker|mug\b|apparel|poster|patch|keychain|stuffed toy|plush|crew socks|socks\b|head wrap|book\b|journal\b|workshop|masterclass|reading|tarot|ticket|event|class with|immersion|social hour|astrology|gua sha class|oracle|akashic|initiation:|pocket altar|free class)\b/i;
...
const NICHE =
  /\b(spf\s*\d+|sunscreen|sun\s?screen|sunblock|sun butter|sun protection|deodor|antiperspir|underarm|\bdeo\b|toothpaste|toothbrush|floss|mouthwash|oral|hydroxyapatite|oil pull|tongue clean|shampoo|conditioner|hair (oil|mask|serum|care|cream)|scalp|leave[- ]?in|protein|whey|casein|collagen|pea protein|electrolyte|hydration|vitamin|supplement|capsule|softgel|probiotic|adaptogen|mushroom|tonic|powder|serum|moisturizer|moisturiser|cleanser|face wash|facial|toner|retinol|niacinamide|cream|lotion|oil|mask|balm|mist|essence|exfoliant|lip|body (lotion|butter|wash|cream|oil|bar)|soap)\b/i;
...
function isOnNiche(raw, brand) {
  ...
  // Single-category clean brands (Island Butter, Swellies) may omit the word "sunscreen"
...
async function main() {
  ...
  for (let i = 0; i < seedBrands.length; i++) {
    ...
    try {
      ...
      const variants = mapped.length - new Set(mapped.map((p) => p.slug.replace(/-[a-z0-9]+$/, ""))).size;
      console.log(
        `kept ${mapped.length}/${rawProducts.length} ingredients ${withIng} flavorExp ${Math.max(0, variants)}`,
      );
      perBrand.push({
        slug: brand.slug,
        brandId,
    ...
    JSON.stringify(
      {
        ...
        wave4Products: waveCount,
        wave4BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  console.log(
    `\nDone total=${all.length} wave4=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
