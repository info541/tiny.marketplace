/**
 * Drop leftover merch from wave20 and backfill missing ingredients from product pages.
 */
...
async function main() {
  ...
  const cleaned = all.filter((p) => !(isWave20(p) && MERCH.test(p.name)));
  const dropped = before - cleaned.length;

  const missing = cleaned.filter((p) => isWave20(p) && (!p.ingredients || p.ingredients.length === 0));
  let filled = 0;
  ...
  const wave = cleaned.filter(isWave20);
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave20Products: wave.length,
    wave20WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave20-ingredients-backfill.json"),
