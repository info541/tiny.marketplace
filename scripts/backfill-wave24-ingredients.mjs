/**
 * Drop leftover merch from wave24 and backfill missing ingredients from product pages.
 */
...
async function main() {
  ...
  const cleaned = all.filter((p) => !(isWave24(p) && MERCH.test(p.name)));
  const dropped = before - cleaned.length;

  const missing = cleaned.filter((p) => isWave24(p) && (!p.ingredients || p.ingredients.length === 0));
  let filled = 0;
  ...
  const wave = cleaned.filter(isWave24);
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave24Products: wave.length,
    wave24WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave24-ingredients-backfill.json"),
