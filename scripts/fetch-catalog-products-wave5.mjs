/**
 * Fetch Shopify catalogs for wave5 brands (c481+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave5.mjs";
...
const WAVE_START = 481;
const WAVE_END = 481 + seedBrands.length - 1;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave5.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave5Products: waveCount,
        wave5BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  console.log(
    `\nDone total=${all.length} wave5=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
