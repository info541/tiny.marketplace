/**
 * Fetch Shopify catalogs for wave16 brands (c701+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave16.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave16.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave16Products: waveCount,
        wave16BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave16-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave16=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
