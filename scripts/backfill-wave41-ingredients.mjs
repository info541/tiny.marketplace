/**
 * Drop leftover merch from wave41 and backfill missing ingredients from product pages.
 */
...
async function main() {
  ...
  const cleaned = all
    .filter((p) => {
      ...
      return !(MERCH.test(p.name) || EXTRA_DROP.test(p.name) || EXTRA_DROP_WAVE38.test(p.name) || EXTRA_DROP_WAVE41.test(p.name));
    })
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave41Products: wave.length,
    wave41WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave41-ingredients-backfill.json"),
