/**
 * Fetch Shopify catalogs for wave21 brands (c795+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave21.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave21.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave21Products: waveCount,
        wave21BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave21-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave21=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
