/**
 * Fetch Shopify catalogs for wave101 brands (c1475+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave101.mjs";
...
function refineCategory(category, title, brand) {
  ...
  if (brand?.slug === "smart-charlie-naturals") {
    if (/toothpaste|hydroxyapatite|oral/i.test(t)) return "oral";
    return "skincare";
  }
  if (brand?.slug === "warrior-salt") return "electrolytes";
  if (brand?.slug === "natural-nutrients") {
    if (/whey|protein/i.test(t)) return "protein";
    return "supplements";
  }
  if (brand?.slug === "miyazaki-tallow") {
    if (/sunscreen|spf|uv protection/i.test(t)) return "sunscreen";
    return "skincare";
  }
  if (brand?.slug === "toups-and-co") {
    if (/deodorant/i.test(t)) return "deodorant";
    if (/sun (stick|balm)|sunscreen|spf/i.test(t)) return "sunscreen";
    if (/dry shampoo|beard|shave soap|baby wash \+ shampoo/i.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "mama-shrooms") return "supplements";
  return category;
}

const TOUPS_KEEP = new Set([
  "deodorant",
  "extra-strength-deodorant-lavender",
  "sun-stick",
  "sun-balm",
  ...
  "glow-serum",
  "resurfacing-mask",
  "limited-edition-glow-face-mask",
...
const NATURAL_KEEP = new Set([
  ...
  "organic-wholefood-multivitamin",
  "zinc-citrate-supplement-15mg",
  "high-potency-milk-thistle-capsules",
  "natural-digestive-enzyme-complex-capsules",
  "creapure-creatine-monohydrate-powder",
  "bcaas-500mg-supplement-2-1-1",
  "zma-supplement-zinc-and-magnesium",
  "vegan-vitamin-d3-capsules",
  "super-magnesium-citrate-bisglycinate-capsules",
  "probiotic-complex-10bn-cfu-high-strength",
  "high-strength-optomega-3-fish-oil-capsules",
  "brain-power-natural-nootropic",
]);
...
function isOnNiche(raw, brand) {
  ...
  if (brand.slug === "smart-charlie-naturals") {
    return /^(peppermint|magnesium-butter-cream|smart-charlie-s-soap|smart-charlie-s-bee-healed-balm-overnight-rescue-for-dry-cracked-skin)$/.test(
      handle,
    );
  }
  if (brand.slug === "warrior-salt") {
    return handle === "warrior-salt-victory-electrolytes";
  }
  if (brand.slug === "natural-nutrients") {
    return NATURAL_KEEP.has(handle);
  }
  if (brand.slug === "miyazaki-tallow") {
    return /^(everyday-tallow-sunscreen-kyoto-green-tea|sweet-yuzu-miyazaki-tallow-balm|tallow-body-butter-lavender)$/.test(
      handle,
    );
  }
  if (brand.slug === "toups-and-co") {
    return TOUPS_KEEP.has(handle);
  }
  if (brand.slug === "mama-shrooms") {
    return /^(lions-mane-tincture|day|night|teen)$/.test(handle);
  }
  if (SKIP_TITLE.test(title) || WAVE101_EXTRA.test(title) || WAVE101_EXTRA.test(handle)) return false;
...
function flavorOptionIndex(raw, brand) {
  ...
  const handle = raw.handle || "";
  if (brand?.slug === "mama-shrooms") return -1;
  if (brand?.slug === "miyazaki-tallow") return -1;
  if (brand?.slug === "smart-charlie-naturals") {
    if (/peppermint/.test(handle)) return options.findIndex((o) => /flavou?r/i.test(o.name || ""));
    if (/magnesium-butter-cream/.test(handle)) return options.findIndex((o) => /scent/i.test(o.name || ""));
    if (/smart-charlie-s-soap/.test(handle)) return options.findIndex((o) => /type/i.test(o.name || ""));
    return -1;
  }
  if (brand?.slug === "warrior-salt") {
    return options.findIndex((o) => /flavou?r|title/i.test(o.name || "")) >= 0
      ? options.findIndex((o) => /flavou?r|title/i.test(o.name || ""))
      : 0;
  }
  if (brand?.slug === "natural-nutrients") {
    if (/grass-fed-whey-protein-(isolate|concentrate)/.test(handle)) {
  ...
  if (brand?.slug === "toups-and-co") {
    if (
      /^(deodorant|extra-strength-deodorant-lavender|body-wash|body-oil|dry-shampoo|facial-tonic|tallow-lip-love|new-tinted-lip-balm)$/.test(
        handle,
      )
    ) {
      return options.findIndex((o) =>
        /scent|flavor|flavour|colour|color|shade|purpose|dry shampoo/i.test(o.name || ""),
      );
    }
    return -1;
  }
...
function pickVariant(variants, brand, raw) {
  ...
  const scored = variants.map((v, i) => {
    ...
    if (/\b(two-pack|2-pack|three-pack|3-pack|four-pack|4-pack|2 boxes|4 boxes|6 boxes|0\.5oz|0\.5 oz|20 servings|3x90|250 g|250g|500 g|500g|1 kg|1kg|5g|sample size|refill only|2 jars|3 jars|5 jars|2 bars|3 bars|8 oz extra|16 oz|5lb|5 lb|family size|81 servings|15 servings|30 ct|3\.3 oz|refill cartridge|30g sample)\b/i.test(hay))
      score -= 8;
    if (brand?.slug === "natural-nutrients") {
      if (/\b300g\b/i.test(hay)) score += 14;
      if (/\b30g\b/i.test(hay)) score -= 14;
      if (/\b1kg\b|\b2kg\b/i.test(hay)) score -= 6;
    }
    if (brand?.slug === "toups-and-co") {
      if (/\b2\s*oz\b/i.test(hay)) score += 8;
      if (/\b4\s*oz\b/i.test(hay)) score -= 4;
    }
    if (brand?.slug === "smart-charlie-naturals") {
      if (/\b2\s*oz\b/i.test(hay)) score += 6;
      if (/\b4\s*oz\b/i.test(hay)) score -= 2;
      if (/soap/.test(handle)) {
        const o1 = String(v.option1 || "").toLowerCase();
        const o2 = String(v.option2 || "").toLowerCase();
        const token = o1.split(/\s+/)[0];
        if (token && o2.includes(token)) score += 20;
        if (Number(v.price) >= 15) score += 4;
      }
    }
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel, currency) {
  ...
  if (brand.slug === "smart-charlie-naturals") {
    const h = raw.handle || "";
    if (/peppermint/.test(h)) baseTitle = "Smart Charlie 10% Nano-Hydroxyapatite Toothpaste";
    else if (/magnesium-butter-cream/.test(h)) baseTitle = "Smart Charlie Magnesium Butter Cream";
    else if (/smart-charlie-s-soap/.test(h)) baseTitle = "Smart Charlie Cold Process Soap";
    else if (/bee-healed/.test(h)) baseTitle = "Smart Charlie Bee Healed Balm";
  }
  if (brand.slug === "warrior-salt") {
    baseTitle = "Warrior Salt Victory Electrolytes";
  }
  if (brand.slug === "natural-nutrients") {
    const h = raw.handle || "";
    if (/whey-protein-isolate/.test(h) && !/sample/.test(h))
      baseTitle = "Natural Nutrients Grass-Fed Whey Protein Isolate";
    else if (/whey-protein-concentrate/.test(h))
      baseTitle = "Natural Nutrients Grass-Fed Whey Protein Concentrate";
    else if (/vegan-protein-powder-chocolate/.test(h))
      baseTitle = "Natural Nutrients Vegan Protein — Chocolate";
    else if (/vegan-protein-powder-vanilla/.test(h))
      baseTitle = "Natural Nutrients Vegan Protein — Vanilla";
    else if (/creapure/.test(h)) baseTitle = "Natural Nutrients Creapure Creatine Monohydrate";
    else if (/supreme-greens/.test(h)) baseTitle = "Natural Nutrients Supreme Greens Powder";
    else if (!/^natural nutrients\b/i.test(baseTitle)) baseTitle = `Natural Nutrients ${baseTitle}`;
  }
  if (brand.slug === "miyazaki-tallow") {
    const h = raw.handle || "";
    if (/everyday-tallow-sunscreen/.test(h))
      baseTitle = "Miyazaki Green Tea Tallow Mineral Sunscreen";
    else if (/sweet-yuzu/.test(h)) baseTitle = "Miyazaki Sweet Yuzu Tallow Balm";
    else if (/tallow-body-butter-lavender/.test(h))
      baseTitle = "Miyazaki Lavender Tallow Body Butter";
  }
  if (brand.slug === "toups-and-co") {
    const h = raw.handle || "";
    if (h === "deodorant") baseTitle = "Toups and Co Natural Deodorant";
    else if (/extra-strength-deodorant/.test(h))
      baseTitle = "Toups and Co Extra-Strength Deodorant";
    else if (h === "sun-stick") baseTitle = "Toups and Co Mineral Sun Stick";
    else if (h === "sun-balm") baseTitle = "Toups and Co Mineral Sun Balm";
    else if (!/^toups\b/i.test(baseTitle)) baseTitle = `Toups and Co ${baseTitle}`;
  }
  if (brand.slug === "mama-shrooms") {
    const h = raw.handle || "";
    if (/lions-mane/.test(h)) baseTitle = "Mama Shrooms Lion's Mane Tincture";
    else if (h === "day") baseTitle = "Mama Shrooms Day Capsules";
    else if (h === "night") baseTitle = "Mama Shrooms Night Capsules";
    else if (h === "teen") baseTitle = "Mama Shrooms Teen Cacao Blend";
  }
  ...
  if (
    brand.slug === "smart-charlie-naturals" ||
    brand.slug === "natural-nutrients" ||
    brand.slug === "miyazaki-tallow"
  ) {
    ingredients = [];
  }
  if (brand.slug === "warrior-salt") {
    const fruit = /strawberry/i.test(title)
      ? ["Organic strawberry powder"]
      : ["Organic lemon powder", "Organic lime powder"];
    ingredients = [
      "Organic cane sugar",
      "Pink Himalayan salt",
      "Potassium chloride",
  ...
  if (brand.slug === "toups-and-co") {
    if (/extra-strength deodorant/i.test(title)) {
      ingredients = [
        "Grass-fed tallow",
    ...
    } else if (/natural deodorant/i.test(title)) {
      ingredients = [
        "Grass-fed tallow",
        "Organic arrowroot powder",
        "Beeswax",
...
function expandProducts(raw, brand, brandId, index, currency) {
  ...
  if (flavorIdx >= 0 && variants.length > 1) {
    ...
    for (const v of variants) {
      ...
      if (brand?.slug === "natural-nutrients" && /\b(30g|300g|1kg|2kg|150g|60 servings)\b/i.test(label) && !/unflavou?red|chocolate|vanilla|strawberry/i.test(label))
        continue;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave101.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave101Products: waveCount,
        wave101BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave101-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave101=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
