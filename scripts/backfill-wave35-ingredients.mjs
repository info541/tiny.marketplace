/**
 * Drop leftover merch from wave35 and backfill missing ingredients from product pages.
 */
...
function reclassify(p) {
  ...
  if (/\b(lion'?s mane|turkey tail|chaga|reishi|tincture|bitters|immunity tonic)\b/.test(t)) p.category = "supplements";
  ...
  if (/\b(ashwagandha|creatine|nac \+ ala|zinc \+ copper|nootropic caps|vitamin d|sleep caps|magnesium \+ potassium)\b/.test(t)) {
    p.category = "supplements";
  }
...
async function main() {
  ...
  const cleaned = all
    .filter((p) => {
      ...
      if (/\b(the lifeguard|mineral melt|mug guard|bod guard|whey protein|protein bag|beef isolate|smash melon)\b/.test(n)) {
        return !EXTRA_DROP.test(p.name);
      }
      return !(MERCH.test(p.name) || EXTRA_DROP.test(p.name));
    })
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave35Products: wave.length,
    wave35WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave35-ingredients-backfill.json"),
