/**
 * Fetch Shopify catalogs for wave52 brands (c1179+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave52.mjs";
...
const FX_TO_USD = {
  "thrive-made-simple": 0.65,
  "frankie-apothecary": 0.6,
  "zoes-kawakawa": 0.6,
};
...
function isOnNiche(raw, brand) {
  ...
  const skipped =
    ...
    WAVE52_SKIP.test(title) ||
    WAVE51_SKIP.test(title) ||
    WAVE50_SKIP.test(title) ||
    SKIP_TYPE.test(type) ||
    SKIP_TAGS.test(tags);
  if (WAVE52_SKIP.test(title)) return false;
  if (skipped) {
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave52.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave52Products: waveCount,
        wave52BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave52-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave52=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
