/**
 * Drop leftover merch from wave5 and backfill missing ingredients from product pages.
 */
...
async function main() {
  ...
  const cleaned = all.filter((p) => !(isWave5(p) && MERCH.test(p.name)));
  const dropped = before - cleaned.length;

  const missing = cleaned.filter((p) => isWave5(p) && (!p.ingredients || p.ingredients.length === 0));
  let filled = 0;
  ...
  const wave = cleaned.filter(isWave5);
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave5Products: wave.length,
    wave5WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave5-ingredients-backfill.json"),
