/**
 * Fetch Shopify catalogs for wave104 brands (c1493+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave104.mjs";
...
function refineCategory(category, title, brand) {
  ...
  if (brand?.slug === "huppy") return "oral";
  if (brand?.slug === "drinkwel") return "electrolytes";
  if (brand?.slug === "pulsin") return "protein";
  if (brand?.slug === "swellies") return "sunscreen";
  if (brand?.slug === "little-tallow-co") {
    if (/deodorant/i.test(t)) return "deodorant";
    if (/shampoo|scalp|hair|bear tallow treatment/i.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "gribb") return "supplements";
  return category;
}

const HUPPY_KEEP = new Set([
  "huppy-toothpaste-tablets-1-month-supply",
  "huppy-mouthwash-tablets",
  "huppy-dental-floss",
  "huppy-tongue-cleaner",
  "bamboo-travel-toothbrush",
]);

const DRINKWEL_KEEP = new Set([
  "lytepow-electrolyte-powder-lemon-lime-90-servings",
  "lytepow-lemon-lime-electrolyte-stick-packs",
  "lyteshow-4-oz-bottle-electrolyte-concentrate-for-rapid-rehydration",
  "lytecaps-electrolytes-capsules-for-serious-rehydration",
  "lytetrace-trace-mineral-concentrate",
  "lytemag-4oz-bottle",
]);

const PULSIN_KEEP = new Set([
  "complete-vegan-protein-blend-chocolate-280g-pouch",
  "complete-vegan-protein-blend-chocolate-hazelnut-280g-pouch",
  "complete-vegan-protein-blend-vanilla-270g-pouch",
  "complete-vegan-protein-blend-pumpkin-spice-270g-pouch-end-of-line",
  "rice-protein-powder-250g-pouch",
  "faba-bean-protein-powder-250g-pouch-pack-of-6-copy",
  "pulsin-pea-protein",
  "pulsin-soya-protein",
]);

const LITTLE_KEEP = new Set([
  "tallowdeodorant",
  "lavender-handmade-tallow-soap",
  "goat-milk-tallow-soap",
  "tallow-foaming-liquid-soap",
  "tallow-complexion-balm",
  "bear-tallow-treatment-hair-scalp-serum",
  "tallow-shampoo-bar",
  "mini-tallow-lip-balm",
  "tallow-lip-balm",
  "traditional-whipped-grassfed-tallow-balm",
  "everywhere-tallow-balm-for-eczema",
  "whipped-tallow",
  "gentle-herb-tallow-bar-soap",
  "soothing-tallow-chest-rub",
]);

const GRIBB_KEEP = new Set([
  "organic-lions-mane-mushroom-tincture",
  "organic-reishi-mushroom-tincture",
  "organic-cordyceps-mushroom-tincture",
  "organic-chaga-mushroom-tincture",
  "organic-shiitake-mushroom-tincture",
  "organic-maitake-mushroom-tincture",
  "organic-tremella-mushroom-tincture",
  "organic-agaricus-blazei-tincture",
  "mesima-tincture",
]);
...
function isOnNiche(raw, brand) {
  ...
  if (brand.slug === "huppy") return HUPPY_KEEP.has(handle);
  if (brand.slug === "drinkwel") return DRINKWEL_KEEP.has(handle);
  if (brand.slug === "pulsin") return PULSIN_KEEP.has(handle);
  if (brand.slug === "swellies") return handle === "mineral-sunscreen-spf-46";
  if (brand.slug === "little-tallow-co") return LITTLE_KEEP.has(handle);
  if (brand.slug === "gribb") return GRIBB_KEEP.has(handle);
  if (SKIP_TITLE.test(title) || WAVE104_EXTRA.test(title) || WAVE104_EXTRA.test(handle)) return false;
...
function flavorOptionIndex(raw, brand) {
  ...
  const handle = raw.handle || "";
  if (brand?.slug === "drinkwel") return -1;
  if (brand?.slug === "pulsin") return -1;
  if (brand?.slug === "swellies") return -1;
  if (brand?.slug === "gribb") return -1;
  if (brand?.slug === "huppy") {
    return options.findIndex((o) => /flavou?r/i.test(o.name || ""));
  }
  if (brand?.slug === "little-tallow-co") {
    return options.findIndex((o) => /scent|material/i.test(o.name || ""));
  }
...
function pickVariant(variants, brand, raw) {
  ...
  const scored = variants.map((v, i) => {
    ...
    if (/\b(two-pack|2-pack|three-pack|3-pack|four-pack|4-pack|2 boxes|4 boxes|6 boxes|0\.5oz|0\.5 oz|20 servings|3x90|250 g|250g|500 g|500g|1 kg|1kg|5g|sample size|refill only|2 jars|3 jars|5 jars|2 bars|3 bars|8 oz extra|16 oz|5lb|5 lb|family size|81 servings|15 servings|30 ct|3\.3 oz|refill cartridge|30g sample)\b/i.test(hay))
      score -= 8;
    if (brand?.slug === "pulsin") {
      if (/\b250g\b/i.test(hay)) score += 10;
      if (/\b(1kg|6x250g|6x1kg|1 kg)\b/i.test(hay)) score -= 8;
    }
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel, currency) {
  ...
  if (brand.slug === "huppy") {
    const h = raw.handle || "";
    if (/toothpaste-tablets-1-month/.test(h)) baseTitle = "Huppy Toothpaste Tablets";
    else if (/mouthwash-tablets/.test(h)) baseTitle = "Huppy Mouthwash Tablets";
    else if (/dental-floss/.test(h)) baseTitle = "Huppy Dental Floss";
    else if (/tongue-cleaner/.test(h)) baseTitle = "Huppy Tongue Cleaner";
    else if (/bamboo-travel-toothbrush/.test(h)) baseTitle = "Huppy Bamboo Travel Toothbrush";
    else if (!/^huppy\b/i.test(baseTitle)) baseTitle = `Huppy ${baseTitle}`;
  }
  if (brand.slug === "drinkwel") {
    const h = raw.handle || "";
    if (/lytepow-electrolyte-powder/.test(h)) baseTitle = "LytePow Electrolyte Powder — Lemon Lime";
    else if (/lytepow-lemon-lime-electrolyte-stick/.test(h))
      baseTitle = "LytePow Electrolyte Stick Packs — Lemon Lime";
    else if (/lyteshow-4-oz/.test(h)) baseTitle = "LyteShow Electrolyte Concentrate";
    else if (/lytecaps/.test(h)) baseTitle = "LyteCaps Electrolyte Capsules";
    else if (/lytetrace/.test(h)) baseTitle = "LyteTrace Trace Mineral Concentrate";
    else if (/lytemag/.test(h)) baseTitle = "LyteMAG Magnesium Concentrate";
  }
  if (brand.slug === "pulsin") {
    const h = raw.handle || "";
    if (/pumpkin-spice/.test(h)) baseTitle = "Pulsin Complete Vegan Protein — Pumpkin Spice";
    else if (/chocolate-hazelnut/.test(h)) baseTitle = "Pulsin Complete Vegan Protein — Chocolate Hazelnut";
    else if (/chocolate-280g/.test(h)) baseTitle = "Pulsin Complete Vegan Protein — Chocolate";
    else if (/vanilla-270g/.test(h)) baseTitle = "Pulsin Complete Vegan Protein — Vanilla";
    else if (/rice-protein/.test(h)) baseTitle = "Pulsin Rice Protein";
    else if (/faba-bean/.test(h)) baseTitle = "Pulsin Faba Bean Protein";
    else if (/pulsin-pea-protein/.test(h)) baseTitle = "Pulsin Pea Protein";
    else if (/pulsin-soya-protein/.test(h)) baseTitle = "Pulsin Soya Protein";
    else if (!/^pulsin\b/i.test(baseTitle)) baseTitle = `Pulsin ${baseTitle}`;
  }
  if (brand.slug === "swellies") baseTitle = "Swellies SPF 46 Mineral Sunscreen";
  if (brand.slug === "little-tallow-co") {
    if (!/^little tallow/i.test(baseTitle)) baseTitle = `Little Tallow Co ${baseTitle}`;
  }
  if (brand.slug === "gribb") {
    const h = raw.handle || "";
    if (/lions-mane/.test(h)) baseTitle = "Gribb Lion's Mane Fruiting-Body Tincture";
    else if (/reishi/.test(h)) baseTitle = "Gribb Reishi Fruiting-Body Tincture";
    else if (/cordyceps/.test(h)) baseTitle = "Gribb Cordyceps Fruiting-Body Tincture";
    else if (/chaga/.test(h)) baseTitle = "Gribb Chaga Fruiting-Body Tincture";
    else if (/shiitake/.test(h)) baseTitle = "Gribb Shiitake Fruiting-Body Tincture";
    else if (/maitake/.test(h)) baseTitle = "Gribb Maitake Fruiting-Body Tincture";
    else if (/tremella/.test(h)) baseTitle = "Gribb Tremella Fruiting-Body Tincture";
    else if (/agaricus/.test(h)) baseTitle = "Gribb Agaricus blazei Fruiting-Body Tincture";
    else if (/mesima/.test(h)) baseTitle = "Gribb Mesima Fruiting-Body Tincture";
  }
  ...
  if (brand.slug === "huppy" || brand.slug === "drinkwel") {
    ingredients = [];
  }
  if (brand.slug === "pulsin") {
    if (/pea protein$/i.test(title)) ingredients = ["Pea protein"];
    else if (/rice protein$/i.test(title)) ingredients = ["Rice protein"];
    else if (/faba bean protein$/i.test(title)) ingredients = ["Faba bean protein"];
    else if (/soya protein$/i.test(title)) ingredients = ["Soya protein"];
    else ingredients = [];
  }
  if (brand.slug === "swellies") {
    ingredients = [
      "Non-nano zinc oxide 21%",
  ...
  if (brand.slug === "little-tallow-co") {
    ...
    } else if (/everywhere tallow balm/i.test(title)) {
      ingredients = ["Grass-fed beef tallow", "Organic jojoba oil", "USA beeswax"];
    } else {
      ingredients = [];
    }
  }
  if (brand.slug === "gribb") {
    const mushroom = title.match(
      /Lion's Mane|Reishi|Cordyceps|Chaga|Shiitake|Maitake|Tremella|Agaricus blazei|Mesima/i,
    );
    if (mushroom) {
      ingredients = [`${mushroom[0]} fruiting body`, "Spring water", "Organic alcohol"];
    } else {
      ingredients = [];
    }
  }
  ...
  if (flavorIdx >= 0 && variants.length > 1) {
    ...
    for (const v of variants) {
      ...
      if (brand?.slug === "huppy") {
        ...
        if (/charcoal/i.test(label)) label = "Charcoal Mint";
      }
      if (brand?.slug === "little-tallow-co") {
        label = label.replace(/\s*\(Limited Edition\)/gi, "").trim();
      }
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave104.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave104Products: waveCount,
        wave104BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave104-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave104=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
