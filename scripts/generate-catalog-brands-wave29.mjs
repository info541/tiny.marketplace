/**
 * Write src/lib/catalog-brands-wave29.ts from the wave29 seed.
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { brands } from "./catalog-brands-seed-wave29.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const out = path.join(__dirname, "../src/lib/catalog-brands-wave29.ts");
...
const rows = brands.map((b, i) => {
  const id = `c${String(i + 937).padStart(3, "0")}`;
  return `  {
    id: ${tsString(id)},
