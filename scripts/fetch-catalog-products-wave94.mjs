/**
 * Fetch Shopify catalogs for wave94 brands (c1433+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave94.mjs";
...
function refineCategory(category, title, brand) {
  ...
  if (brand?.slug === "elims") return "oral";
  if (brand?.slug === "suprey") return "electrolytes";
  if (brand?.slug === "malama-mushrooms") return "supplements";
  if (brand?.slug === "clean-kind") {
    if (/\bspf\b|sunscreen/.test(t)) return "sunscreen";
    return "skincare";
  }
  if (brand?.slug === "muscle-feast") {
    if (/whey|casein|protein/.test(t)) return "protein";
    return "supplements";
  }
  if (brand?.slug === "willow-valley") {
    if (/deodorants?/.test(t)) return "deodorant";
    if (/shampoo/.test(t)) return "hair";
    return "skincare";
  }
  return category;
}
...
function isOnNiche(raw, brand) {
  ...
  if (SKIP_TITLE.test(title) || WAVE94_EXTRA.test(title)) return false;
  if (Number((raw.variants || [])[0]?.price || 0) <= 0) return false;
  if (brand.slug === "elims") {
    if (
      /\b(travel|2[- ]pack|3[- ]pack|5[- ]pack|home\s*&\s*away|home and away|sample|whitening|mask|replacement|heads?|ritual|bundle|electric)\b/i.test(
        title,
      ) ||
      /travel|2-pack|3-pack|5-pack|home-away|sample|whitening|mask|heads|ritual|bundle|electric/.test(handle)
    ) {
      return false;
    }
    if (/toothbrush/i.test(hay)) return /blue/i.test(hay);
    return /toothpaste/i.test(hay) && /4\s*oz/i.test(hay);
  }
  if (brand.slug === "clean-kind") {
    if (/\b(gift|bundle|kit)\b/i.test(title)) return false;
    return /\b(spf|sunscreen|lotion|serum|balm|tallow)\b/i.test(hay);
  }
  if (brand.slug === "suprey") {
    return /hydust|electrolytes?/i.test(hay);
  }
  if (brand.slug === "muscle-feast") {
    if (
      /\b(bottle|t-shirt|tee|oat|dextrin|candy|chew)\b/i.test(title) ||
      /water-bottle|t-shirt|oat|dextrin|candy|chew/.test(handle)
    ) {
      return false;
    }
    return /\b(whey|casein|creatine|protein)\b/i.test(hay);
  }
  if (brand.slug === "willow-valley") {
    if (
      /\b(gift|bundle|roll-?on|ritual mist|bug off|soap saver|kindness|simple joy|tranquil|morning ritual)\b/i.test(
        title,
      ) ||
      /gift|bundle|roll-on|ritual-mist|bug-off|soap-saver|kindness|simple-joy|tranquil|morning-ritual/.test(
        handle,
      )
    ) {
      return false;
    }
    return /\b(deodorant|shampoo|soap|bar)\b/i.test(hay);
  }
  if (brand.slug === "malama-mushrooms") {
    if (
      /\b(coffee|cacao|matcha|bundle|merch|hoodie|t-shirt|tee|hat|plushy|grow|pet|book|card|mug|gift wrap|hoodie|festival)\b/i.test(
        title,
      ) ||
      /coffee|cacao|matcha|bundle|merch|hoodie|t-shirt|hat|plushy|growkit|pet|guide|mug|gift-wrap|festival|subscriber/.test(
        handle,
      )
    ) {
      return false;
    }
    return /\b(tincture|powder extracts?|extract powder|superfood powder)\b/i.test(hay);
  }
...
function flavorOptionIndex(raw, brand) {
  ...
  const handle = raw.handle || "";
  if (brand?.slug === "elims") return -1;
  if (brand?.slug === "malama-mushrooms") return -1;
  if (brand?.slug === "willow-valley") return -1;
  if (brand?.slug === "suprey") {
    if (/hydust/i.test(handle)) {
      return options.findIndex((o) => /flavou?r/i.test(o.name || ""));
    }
    return -1;
  }
  if (brand?.slug === "clean-kind") {
    return options.findIndex((o) => /scent|tint/i.test(o.name || ""));
  }
  if (brand?.slug === "muscle-feast") {
...
function pickVariant(variants, brand, raw) {
  ...
  const scored = variants.map((v, i) => {
    ...
    if (/\b(two-pack|2-pack|three-pack|3-pack|four-pack|4-pack|2 boxes|4 boxes|6 boxes|0\.5oz|0\.5 oz|20 servings|3x90|250 g|250g|500 g|500g|1 kg|1kg|5g|sample size|refill only|2 jars|3 jars|5 jars|2 bars|3 bars|8 oz extra|16 oz|5lb|5 lb|family size|81 servings|15 servings|30 ct|3\.3 oz|refill cartridge)\b/i.test(hay))
      score -= 8;
    if (brand?.slug === "elims") {
      if (/4\s*oz/i.test(hay)) score += 14;
      if (/1\s*oz|travel/i.test(hay)) score -= 14;
    }
    if (brand?.slug === "muscle-feast") {
      if (/\b2lb|2 lb|300g|250g\b/i.test(hay)) score += 14;
      if (/\b5lb|5 lb|4lb|4 lb|2lb family\b/i.test(hay)) score -= 12;
    }
    if (brand?.slug === "willow-valley") {
      if (/4oz|4 oz/i.test(hay)) score += 14;
      if (/2oz|2 oz|1oz|1 oz/i.test(hay)) score -= 10;
    }
    if (brand?.slug === "malama-mushrooms") {
      if (/3\.5 oz|30 ml|30ml/i.test(hay)) score += 14;
      if (/16 oz|60 ml|60ml/i.test(hay)) score -= 14;
    }
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel, currency) {
  let baseTitle = (raw.title || "")
    ...
    .trim();
  if (brand.slug === "elims") {
    if (/pineapple/i.test(baseTitle)) baseTitle = "ELIMS 10% Nano-Hydroxyapatite Toothpaste Pineapple Orange Mint";
    else if (/lavender/i.test(baseTitle)) baseTitle = "ELIMS 10% Nano-Hydroxyapatite Toothpaste Lavender Vanilla Mint";
    else if (/toothbrush/i.test(baseTitle)) baseTitle = "ELIMS Premium Bamboo Toothbrush";
    else if (!/^elims\b/i.test(baseTitle)) baseTitle = `ELIMS ${baseTitle}`;
  }
  if (brand.slug === "clean-kind") {
    if (/spf 50/i.test(baseTitle)) baseTitle = "Clean + Kind Tallow Mineral Sunscreen SPF 50";
    else if (/spf 25/i.test(baseTitle)) baseTitle = "Clean + Kind Tallow Mineral Sunscreen SPF 25";
    else if (/kids/i.test(baseTitle)) baseTitle = "Clean + Kind Kids Mineral Sunscreen SPF 40";
    else if (/daily/i.test(baseTitle)) baseTitle = "Clean + Kind Daily Moisturizing Sunscreen SPF 40";
    else if (/retinol/i.test(baseTitle)) baseTitle = "Clean + Kind Retinol Tallow Body Lotion";
    else if (/body lotion/i.test(baseTitle)) baseTitle = "Clean + Kind Tallow Body Lotion";
    else if (/whipped/i.test(baseTitle)) baseTitle = "Clean + Kind Whipped Tallow Balm";
    else if (/baby/i.test(baseTitle)) baseTitle = "Clean + Kind Baby & Diaper Balm";
    else if (/copper|ghk/i.test(baseTitle)) baseTitle = "Clean + Kind Copper Peptide Serum";
    else if (/azelaic/i.test(baseTitle)) baseTitle = "Clean + Kind Azelaic Acid Serum 10%";
    else if (!/^clean/i.test(baseTitle)) baseTitle = `Clean + Kind ${baseTitle}`;
  }
  if (brand.slug === "suprey") {
    baseTitle = "Suprey HYDUST Electrolytes";
  }
  if (brand.slug === "muscle-feast") {
    if (/hydrolyzed/i.test(baseTitle)) baseTitle = "Muscle Feast Grass-Fed Hydrolyzed Whey";
    else if (/isolate/i.test(baseTitle)) baseTitle = "Muscle Feast Grass-Fed Whey Isolate";
    else if (/100% whey|100 whey/i.test(baseTitle)) baseTitle = "Muscle Feast 100% Grass-Fed Whey";
    else if (/concentrate/i.test(baseTitle)) baseTitle = "Muscle Feast Grass-Fed Whey Concentrate";
    else if (/casein/i.test(baseTitle)) baseTitle = "Muscle Feast Micellar Casein";
    else if (/creapure micro|micronized/i.test(baseTitle)) baseTitle = "Muscle Feast Creapure Micronized Creatine";
    else if (/creapure/i.test(baseTitle)) baseTitle = "Muscle Feast Creapure Creatine Monohydrate";
    else if (!/^muscle feast\b/i.test(baseTitle)) baseTitle = `Muscle Feast ${baseTitle}`;
  }
  if (brand.slug === "willow-valley") {
    if (/deodorant/i.test(baseTitle)) baseTitle = "Willow Valley Perfectly Neutral Deodorant Paste";
    else if (!/^willow/i.test(baseTitle)) baseTitle = `Willow Valley ${baseTitle}`;
  }
  if (brand.slug === "malama-mushrooms") {
    if (!/^m[āa]lama/i.test(baseTitle)) baseTitle = `Mālama ${baseTitle}`;
  }
  ...
  if (brand.slug === "elims" || brand.slug === "clean-kind" || brand.slug === "suprey" || brand.slug === "muscle-feast") {
    ingredients = [];
  }
  if (brand.slug === "willow-valley") {
    ...
    } else if (ingredients.some((s) => /helping reduce|water-free alternative|travel-friendly|more manageable/i.test(s))) {
      ingredients = [];
    }
  }
  if (brand.slug === "malama-mushrooms") {
    if (/lion'?s mane.*powder|powder.*lion'?s mane/i.test(title)) {
      ingredients = ["Organic Lion's Mane mushroom fruiting-body extract"];
    } else if (/reishi.*powder|powder.*reishi/i.test(title)) {
      ingredients = ["Organic Reishi mushroom fruiting-body extract"];
    } else if (/cordyceps.*powder|powder.*cordyceps/i.test(title)) {
      ingredients = ["Organic Cordyceps mushroom fruiting-body extract"];
    } else if (/turkey tail.*powder|powder.*turkey tail/i.test(title)) {
      ingredients = ["Organic Turkey Tail mushroom fruiting-body extract"];
    } else if (/chaga.*powder|powder.*chaga/i.test(title)) {
      ingredients = ["Organic Chaga mushroom fruiting-body extract"];
    } else if (/8 mushroom|superfood powder/i.test(title)) {
      ingredients = ["Organic 8-mushroom fruiting-body extract blend"];
    } else {
      ingredients = [];
    }
  }
...
function expandProducts(raw, brand, brandId, index, currency) {
  ...
  if (flavorIdx >= 0 && variants.length > 1) {
    ...
    for (const v of variants) {
      ...
      if (brand?.slug === "muscle-feast" && /\b5lb|5 lb\b/i.test(label)) continue;
      if (brand?.slug === "suprey" && /\bmix\b/i.test(label)) continue;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave94.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave94Products: waveCount,
        wave94BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave94-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave94=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
