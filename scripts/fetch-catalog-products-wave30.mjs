/**
 * Fetch Shopify catalogs for wave30 brands (c956+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave30.mjs";
...
const SKIP_TITLE_EXTRA =
  /\b(consultations?|guide to|water-based lubricant|\blubricant\b|bone meal|wash tabs|bug off|vanilla (bean )?extract|migraine roller|wax wraps?|eyelash|drinkware|hand mixer|active bottle|elderberry syrup kits?|foaming tallow hand soap|6 pack|refillery|pearl powder|eco action|laundry detergent strips|nectar scent|natural perfume|free gifts|egyptian loofah)\b/i;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave30.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave30Products: waveCount,
        wave30BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave30-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave30=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
