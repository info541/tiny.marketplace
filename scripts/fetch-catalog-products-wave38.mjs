/**
 * Fetch Shopify catalogs for wave38 brands (c1019+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave38.mjs";
...
const FX_TO_USD = {
  "high-end-hippie": 0.73,
  "margaret-river-made": 0.65,
  "fat-cow-skin": 1.32,
  "grin-natural": 0.58,
};
...
function isOnNiche(raw, brand) {
  ...
  if (brand.slug === "surface-sunscreen" && /\bdry touch\b/i.test(title) && !/\bmineral\b/i.test(title)) return false;
  if (brand.slug === "surface-sunscreen" && /\b(anti chafe|golf sun|sports bundle|grab & go|kids go-to|variety pack)\b/i.test(title)) return false;
  if (brand.slug === "wellah" && /\bpre-workout\b/i.test(title)) return false;
  if (brand.slug === "saint-oral" && /\b(whitening strips|discount_hidden|hidden.product|whitening essentials|whitening starter)\b/i.test(title)) return false;
  if (brand.slug === "tallow-balm-co" && /\bshirt\b/i.test(title)) return false;
  if (brand.slug === "chagrin-valley" && /\b(subscription|room spray|shower mist|bug|pet|apparel|gift card|soap club|bulk bath|hand sanitizer|massage candle|accessory)\b/i.test(title)) return false;
  if (brand.slug === "haus-of-sage" && /\b(48 pack|white label|wholesale|customizable)\b/i.test(title)) return false;
  if (brand.slug === "wellah" && /\bshaker\b/i.test(title)) return false;
  if (brand.slug === "sugar-pine" && /\b(hat|hair bundle|hot girls)\b/i.test(title)) return false;
  if (brand.slug === "manna-tallow" && /\b(blush|starter kit|reset trio|lip & blush|lip and blush)\b/i.test(title)) return false;
  if (brand.slug === "high-end-hippie" && /\b(travel case|shampoo bag|storage|shower-to-travel|better together|try me bundle)\b/i.test(title)) return false;
  if (brand.slug === "margaret-river-made" && /\b(eau de|parfum|perfume)\b/i.test(title)) return false;
  if (brand.slug === "fat-cow-skin" && /\b(gift card|gua sha|bundle offer|buy two|glow & flow|gentle bundle|small starter|starter pack|fat cow bundle)\b/i.test(title)) return false;
  if (brand.slug === "grin-natural" && /\b(silicone|holder|travel kit|on the go kit)\b/i.test(title)) return false;
  if (brand.slug === "sb-botanicals" && /\b(gift card|room spray|recipe|directory|supplier)\b/i.test(title)) return false;
  if (brand.slug === "twice-oral" && /\b(stadium|whitening kit|led|electric)\b/i.test(title)) return false;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave38.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave38Products: waveCount,
        wave38BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave38-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave38=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
