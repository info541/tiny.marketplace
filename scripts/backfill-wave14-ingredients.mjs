/**
 * Drop leftover merch from wave14 and backfill missing ingredients from product pages.
 */
...
async function main() {
  ...
  const cleaned = all.filter((p) => !(isWave14(p) && MERCH.test(p.name)));
  const dropped = before - cleaned.length;

  const missing = cleaned.filter((p) => isWave14(p) && (!p.ingredients || p.ingredients.length === 0));
  let filled = 0;
  ...
  const wave = cleaned.filter(isWave14);
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave14Products: wave.length,
    wave14WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave14-ingredients-backfill.json"),
