/**
 * Drop leftover merch from wave49 and backfill missing ingredients from product pages.
 */
...
function reclassify(p) {
  ...
  if (n === 1159) {
    ...
    else if (/\b(beard|after shave|aftershave)\b/.test(t)) p.category = "hair";
    else if (/\bmagnesium (magic )?spray\b/.test(t)) p.category = "supplements";
    else p.category = "skincare";
  }
  if (n === 1160) p.category = /\bsun balm\b/.test(t) ? "sunscreen" : "skincare";
  if (n === 1161) p.category = "sunscreen";
  if (n === 1162) p.category = "electrolytes";
  if (n === 1163) p.category = "skincare";
  return p;
}
...
async function main() {
  ...
  const cleaned = all
    .filter((p) => {
      ...
      return !(MERCH.test(p.name) || EXTRA_DROP.test(p.name) || EXTRA_DROP_WAVE38.test(p.name) || EXTRA_DROP_WAVE41.test(p.name) || EXTRA_DROP_WAVE42.test(p.name) || EXTRA_DROP_WAVE43.test(p.name) || EXTRA_DROP_WAVE46.test(p.name) || EXTRA_DROP_WAVE47.test(p.name));
    })
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave49Products: wave.length,
    wave49WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave49-ingredients-backfill.json"),
