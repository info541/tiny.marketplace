/**
 * Fetch Shopify catalogs for wave88 brands (c1397+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave88.mjs";
...
const WAVE88_EXTRA =
  /\b(gift card|e-gift|gift pack|gift set|gift basket|mystery (box|gift)|shipping protection|package protection|savedby|navidium|recura|onward|covered returns|build your (own )?bundle|bundles?\b|duo\b|duet\b|trio\b|variety pack|holiday gift|shaker bottle|shakers?\b|whisk\b|sampler pack|cashback|enamel pin|empty bottle|frother|mixer|water bottle|sachet|sample kit|sweatshirt|hoodie|\bhat\b|scrunchie|pillowcase|keychain|tote|socks|tumbler|straws?|complimentary|e-?book|masterclass|workshop|audiobook|consultation|sponsorship|sisal|soap dish|stainless steel bottle|wholesale|subscription gift|joint support bundle|ritual|collection|discovery (box|set|kit)|glove|eyelash|yoga mat|blue light|dry brush|hori hori|gardening|t-shirt|\btee\b|gnar tar 4-set|synergistic 4 set|tiktok|do not use|vip bundle|caddie|measuring scoop|checkout\+|protocol|glp-1)\b/i;
...
function refineCategory(category, title, brand) {
  ...
  if (brand?.slug === "living-well-dr-michelle") {
    if (/electrolyte|hydrat/.test(t)) return "electrolytes";
    if (/deodorants?/.test(t)) return "deodorant";
    if (/toothpaste|toothbrush|floss|mouthwash|tooth powder|dry mouth|re-?mints|mint spray|tongue|oral|remineral/.test(t))
      return "oral";
    return "supplements";
  }
  if (brand?.slug === "hydrant") return "electrolytes";
  if (brand?.slug === "centenarius") {
    if (/whey|protein/.test(t)) return "protein";
    return "supplements";
  }
  if (brand?.slug === "oak-and-sage") {
    if (/hair oil|beard/.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "tus-tallow") {
    if (/sun|spf|zinc|sunblock/.test(t)) return "sunscreen";
    if (/deodorants?/.test(t)) return "deodorant";
    return "skincare";
  }
  if (brand?.slug === "mycolove-farm") return "supplements";
  return category;
}
...
function isOnNiche(raw, brand) {
  ...
  if (SKIP_TITLE.test(title) || WAVE88_EXTRA.test(title)) return false;
  if (Number((raw.variants || [])[0]?.price || 0) <= 0) return false;
  if (brand.slug === "living-well-dr-michelle") {
    if (
      /\b(bundle|kit|year supply|1 year|discovery box|gift card|ebook|audiobook|masterclass|workshop|course|yoga|shaker|glasses|hori|gardening|book\b|protocol|duo|toiletry|applicator|pump & cup|four pack|two pack|2 pack|4 pack|digital|real food for real families|healthy mouth, healthy you|be prepared|self-sufficient living|100 days from the garden|lymphatic|remineralization supplements|floss refills?)\b/i.test(
        title,
      )
    ) {
      return false;
    }
    return /\b(toothpaste|toothbrush|floss|mouthwash|tooth powder|dry mouth|re-?mints|mint spray|tongue|deodorants?|electrolyte|elderberry|digest|tribiotic|bloat|vita d|tooth\/bone|chewable|oregano|immunity)\b/i.test(
      hay,
    );
  }
  if (brand.slug === "hydrant") {
    if (
      /\b(tiktok|do not use|vip bundle|buy 2|gift card|checkout|scoop|8-pack|8 pack|12-pack|12 pack|caddie|bundle|all flavors|cold traffic)\b/i.test(
        title,
      ) ||
      handle === "hydrant-energy-lemon-squeeze" ||
      handle === "hydrant-sleep-rapid-hydration-mix-cosmic"
    ) {
      return false;
    }
    return /\b(hydrate|energy|sleep|immunity|unflavored|electrolyte|hydration)\b/i.test(hay);
  }
  if (brand.slug === "centenarius") {
    if (/\b(protocol|bundle|shaker|shipping|returns|strada)\b/i.test(title)) return false;
    return /\b(whey|protein|collagen|creatine|glycine|magnesium|sleep|mood)\b/i.test(hay);
  }
  if (brand.slug === "oak-and-sage") {
    if (/\b(bundle|gift card|tanning oil|tropic like)\b/i.test(title)) return false;
    return /\b(cream|balm|oil|serum|scrub|hydrosol|lip|glow|salve|butter)\b/i.test(hay);
  }
  if (brand.slug === "tus-tallow") {
    if (/\b(4-pack|4 pack|set)\b/i.test(title)) return false;
    return /\b(sun|spf|zinc|deodorants?|cream|balm|soap|lip|tallow)\b/i.test(hay);
  }
  if (brand.slug === "mycolove-farm") {
    if (/\b(4-set|4 set|t-shirt|tee)\b/i.test(title)) return false;
    return /\b(tincture|gnar tar|lion.?s mane|reishi|chaga|cordyceps|turkey tail|powder)\b/i.test(
      hay,
    );
  }
...
function flavorOptionIndex(raw, brand) {
  ...
  const handle = raw.handle || "";
  if (
    brand?.slug === "living-well-dr-michelle" ||
    brand?.slug === "centenarius" ||
    brand?.slug === "tus-tallow" ||
    brand?.slug === "mycolove-farm"
  )
    return -1;
  if (brand?.slug === "hydrant") {
    if (
      handle === "hydration-mix" ||
      handle === "rapid-hydration-mix-caffeine" ||
      handle === "sleep-drink-mix" ||
      handle === "hydrant-energy-lemon-squeeze"
    ) {
  ...
  if (brand?.slug === "oak-and-sage") {
    if (handle === "everywhere-cream") {
      return options.findIndex((o) => /^scent$/i.test(o.name || ""));
    }
    if (handle === "glow-drops") {
      return options.findIndex((o) => /^shade$/i.test(o.name || ""));
    }
    return -1;
  }
...
function pickVariant(variants, brand) {
  ...
  const scored = variants.map((v, i) => {
    ...
    if (/\b(two-pack|2-pack|three-pack|3-pack|four-pack|4-pack|2 boxes|4 boxes|6 boxes|0\.5oz|0\.5 oz|20 servings|3x90|250 g|250g|500 g|500g|1 kg|1kg|5g|sample size|refill only|2 jars|3 jars|5 jars|2 bars|3 bars|4 oz\. amber|8 oz|plus 4 oz|plus 8 oz|2\.5lb|5lb|small\b)\b/i.test(hay))
      score -= 8;
    if (brand?.slug === "living-well-dr-michelle") {
      if (/default title|single/i.test(hay)) score += 8;
      if (/year|4 pack|2 pack/i.test(hay)) score -= 12;
    }
    if (brand?.slug === "hydrant") {
      if (/\b(30|default title)\b/i.test(hay)) score += 6;
      if (/variety pack/i.test(hay)) score -= 14;
    }
    if (brand?.slug === "oak-and-sage") {
      if (/\b8\s*oz\b/i.test(hay) && !/16/.test(hay)) score += 12;
      if (/16 oz|glow drops bundle|travel size|2 oz/i.test(hay)) score -= 10;
    }
    if (brand?.slug === "tus-tallow") {
      if (/\b(2 oz|default title)\b/i.test(hay)) score += 6;
      if (/\b1 oz\b/i.test(hay)) score -= 4;
    }
    if (brand?.slug === "mycolove-farm") {
      if (/1 oz|30 mL|default title/i.test(hay)) score += 8;
      if (/1\/2 oz|15 mL|2 oz|59 mL|60 mL/i.test(hay)) score -= 6;
    }
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel, currency) {
  let baseTitle = (raw.title || "")
    ...
    .trim();
  if (brand.slug === "living-well-dr-michelle") {
    if (/remineralizing toothpaste – hydroxyapatite/i.test(baseTitle))
      baseTitle = "Living Well Remineralizing Toothpaste";
    else if (/kids remineralizing toothpaste/i.test(baseTitle))
      baseTitle = "Living Well Kids Remineralizing Toothpaste Mixed Berry";
    else if (/sensitive remineralizing tooth powder/i.test(baseTitle))
      baseTitle = "Living Well Sensitive Remineralizing Tooth Powder";
    else if (/whitening & remineralizing tooth powder/i.test(baseTitle))
      baseTitle = "Living Well Whitening Remineralizing Tooth Powder";
    else if (/kids tooth powder/i.test(baseTitle)) baseTitle = "Living Well Kids Tooth Powder Fresh Citrus";
    else if (/gum health remineralizing tooth powder/i.test(baseTitle))
      baseTitle = "Living Well Gum Health Tooth Powder";
    else if (/^remineralizing tooth powder$/i.test(baseTitle))
      baseTitle = "Living Well Remineralizing Tooth Powder";
    else if (/remineralizing \+ gum health mouthwash/i.test(baseTitle))
      baseTitle = "Living Well Gum Health Mouthwash Soothing Mint";
    else if (/mouthwash \(cool mint\)/i.test(baseTitle))
      baseTitle = "Living Well Remineralizing Mouthwash Cool Mint";
    else if (/mouthwash \(fresh citrus\)/i.test(baseTitle))
      baseTitle = "Living Well Remineralizing Mouthwash Fresh Citrus";
    else if (/probiotic deodorant stick/i.test(baseTitle))
      baseTitle = "Living Well Probiotic Deodorant Stick";
    else if (/probiotic deodorant/i.test(baseTitle)) baseTitle = "Living Well Probiotic Deodorant";
    else if (/electrolyte drink mix/i.test(baseTitle))
      baseTitle = "Living Well Cocoa Recharge Electrolytes";
    else if (/kids bamboo toothbrush/i.test(baseTitle)) baseTitle = "Living Well Kids Bamboo Toothbrush";
    else if (/bamboo toothbrush/i.test(baseTitle)) baseTitle = "Living Well Bamboo Toothbrush";
    else if (/copper tongue/i.test(baseTitle)) baseTitle = "Living Well Copper Tongue Scraper";
    else if (!/^living well\b/i.test(baseTitle)) baseTitle = `Living Well ${baseTitle}`;
  }
  if (brand.slug === "hydrant") {
    if (/unflavored/i.test(baseTitle)) baseTitle = "Hydrant Unflavored Electrolyte Powder";
    else if (/immunity/i.test(baseTitle)) baseTitle = "Hydrant Immunity Elderberry";
    else if (/energy/i.test(baseTitle)) baseTitle = "Hydrant Energy";
    else if (/sleep/i.test(baseTitle)) baseTitle = "Hydrant Sleep";
    else if (/hydrate/i.test(baseTitle)) baseTitle = "Hydrant Hydrate";
    else if (!/^hydrant\b/i.test(baseTitle)) baseTitle = `Hydrant ${baseTitle}`;
  }
  if (brand.slug === "centenarius") {
    if (/whey/i.test(baseTitle)) baseTitle = "Centenarius Grass-Fed Whey Isolate";
    else if (/collagen/i.test(baseTitle)) baseTitle = "Centenarius Hydrolyzed Marine Collagen";
    else if (/creatine/i.test(baseTitle)) baseTitle = "Centenarius Creatine Monohydrate";
    else if (/glycine/i.test(baseTitle)) baseTitle = "Centenarius Glycine";
    else if (/dual mag/i.test(baseTitle)) baseTitle = "Centenarius Dual Mag Power";
    else if (/sleep/i.test(baseTitle)) baseTitle = "Centenarius Sleep & Mood";
    else if (!/^centenarius\b/i.test(baseTitle)) baseTitle = `Centenarius ${baseTitle}`;
  }
  if (brand.slug === "oak-and-sage") {
    if (!/^oak & sage\b/i.test(baseTitle) && !/^oak and sage\b/i.test(baseTitle))
      baseTitle = `Oak & Sage ${baseTitle}`;
  }
  if (brand.slug === "tus-tallow") {
    if (/mineral sunscreen|sun block/i.test(baseTitle)) baseTitle = "Tu's Tallow Mineral Sunscreen";
    else if (!/^tu'?s\b/i.test(baseTitle)) baseTitle = `Tu's ${baseTitle}`;
  }
  if (brand.slug === "mycolove-farm") {
    if (!/^mycolove\b/i.test(baseTitle)) baseTitle = `Mycolove ${baseTitle}`;
  }
  ...
  if (
    brand.slug === "living-well-dr-michelle" ||
    brand.slug === "hydrant" ||
    brand.slug === "oak-and-sage"
  ) {
    ingredients = [];
  }
  if (brand.slug === "centenarius") {
    if (/whey/i.test(title)) {
      ingredients = [
        "Certified Grass-Fed Whey Protein Isolate",
  ...
  if (brand.slug === "tus-tallow") {
    if (/sunscreen|sun block/i.test(title)) {
      ingredients = [
        "Grass-Fed Tallow",
        "Non-Nano Zinc Oxide",
        "Organic Oils",
  ...
  if (brand.slug === "mycolove-farm") {
    const species = /lion/i.test(title)
      ...
      : /reishi/i.test(title)
        ...
        : /cordyceps/i.test(title)
          ...
          : /turkey tail/i.test(title)
            ? "Turkey Tail (Trametes versicolor) Fruiting Body"
            : /chaga/i.test(title)
              ? "Chaga (Inonotus obliquus)"
              : null;
    if (/alcohol-free|glycer/i.test(title) && species) {
      ingredients = [species, "Vegetable Glycerine", "Distilled Water"];
    } else if (/tincture|traditional alcohol|gnar tar/i.test(title) && species) {
      ingredients = [species, "Organic Cane Alcohol", "Distilled Water"];
    } else if (/powder/i.test(title) && species) {
      ingredients = [`Fermented Freeze-Dried ${species} Powder`];
    } else if (!looksLikeInci(ingredients, ingredients.join(", "))) {
      ingredients = [];
    }
  }
...
function expandProducts(raw, brand, brandId, index, currency) {
  ...
  if (flavorIdx >= 0 && variants.length > 1) {
    ...
    for (const v of variants) {
      ...
      if (brand?.slug === "hydrant" && /variety pack|bulk powder|unflavored electrolyte/i.test(label))
        continue;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave88.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave88Products: waveCount,
        wave88BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave88-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave88=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
