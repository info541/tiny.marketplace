/**
 * Fetch Shopify catalogs for wave60 brands (c1229+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave60.mjs";
...
function isOnNiche(raw, brand) {
  ...
  const skipped =
    ...
    SKIP_TYPE.test(type) ||
    SKIP_TAGS.test(tags);
  if (WAVE60_EXTRA.test(title) || WAVE59_EXTRA.test(title) || WAVE58_EXTRA.test(title) || WAVE57_EXTRA.test(title) || WAVE56_EXTRA.test(title) || WAVE54_SKIP.test(title) || WAVE54_EXTRA.test(title)) return false;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave60.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave60Products: waveCount,
        wave60BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave60-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave60=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
