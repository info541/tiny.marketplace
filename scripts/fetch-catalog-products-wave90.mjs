/**
 * Fetch Shopify catalogs for wave90 brands (c1409+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave90.mjs";
...
function refineCategory(category, title, brand) {
  ...
  if (brand?.slug === "primal-life-organics") {
    if (/deodorants?|stick up/.test(t)) return "deodorant";
    if (/sun[- ]?up|spf|sun protection|after[- ]?sun/.test(t)) return "sunscreen";
    if (
      /toothpowder|toothpaste|toothbrush|floss|oil pull|tongue|mouthwash|gum (drops|serum)|chews|dental boost/.test(
        t,
      )
    )
      return "oral";
  ...
  if (brand?.slug === "adapt-electrolytes") return "electrolytes";
  if (brand?.slug === "go-good") {
    if (/creatine/.test(t)) return "supplements";
    return "protein";
  }
  if (brand?.slug === "tallow-cosmetics") {
    if (/sunscreen|spf/.test(t)) return "sunscreen";
    if (/deodorants?/.test(t)) return "deodorant";
    if (/hairwax|hair wax/.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "royal-tallow") {
    if (/deodorants?/.test(t)) return "deodorant";
    return "skincare";
  }
  if (brand?.slug === "organicules") {
...
function isOnNiche(raw, brand) {
  ...
  if (SKIP_TITLE.test(title) || WAVE90_EXTRA.test(title)) return false;
  if (Number((raw.variants || [])[0]?.price || 0) <= 0) return false;
  if (brand.slug === "primal-life-organics") {
    if (
      /\b(bundle|kit|trio|package|pack(?!age)|subscription|loyal customer|price test|onward|route|shipping|gift card|whitening (system|pen|strip|duo|refill)|led|sonic|mouth guard|zap|wooden|taster|bug spray|hand-it-izer|replacement brush|travel hack|summer essentials|smile starter|family smile|gum health essentials|refill & refresh|whitening power|daily detox|mini stick up|black pack)\b/i.test(
        title,
      ) ||
      /bundle|kit|trio|package|subscription|onward|routeins|v5-|v4-|whitening|sonic|mouth-guard|zap-|wooden-toothpowder|taster|boot-a-pest|hand-it-izer|replacement-brush|travel-hack|smile-starter|family-smile|essentials-kit|refill-refresh|black-pack|mini-stick|power-pack|detox-set|his-hers/.test(
        handle,
      )
    ) {
      return false;
    ...
    if (/toothbrush/i.test(title)) {
      return handle === "bamboo-charcoal-ion-toothbrush";
    }
    if (
      handle === "stick-up-mini-100-natural-deodorant-2-oz" ||
      handle === "ignite-cinnamon-shine-toothpowder" ||
      handle === "cinnamon-shine-toothpowder" ||
      handle === "charcoal-cinnamon-shine-toothpowder" ||
      handle === "cinnamon-collection-powdered-mineral-toothpastes"
    ) {
      return false;
    }
    return /\b(toothpowder|toothpaste|chews|floss|oil[- ]?pull\w*|pulling oil|tongue|deodorants?|stick up|sun-up|sun protection|lip|body butter|soap|serum|cleanser|after-shave|beard|hair|after[- ]?sun|balm|gum drops|foot repair|hand repair)\b/i.test(
      hay,
    );
  }
  if (brand.slug === "adapt-electrolytes") {
    if (/\b(bundle|hat|bottle|tee|tank)\b/i.test(title)) return false;
    return /electrolytes?/.test(hay);
  }
  if (brand.slug === "go-good") {
    if (
      /\b(shaker|sample|bundle|10 pack|10x)\b/i.test(title) ||
      /shaker|sample|bundle/.test(handle)
    ) {
      return false;
    }
    return /\b(whey|protein|creatine|wpi|wpc|plant protein)\b/i.test(hay);
  }
  if (brand.slug === "tallow-cosmetics") {
    if (
      /\b(bundle|gift|candle|geurkaars|cadeaukaart|travel box|facial cups?|kit)\b/i.test(title) ||
      /bundle|cadeaukaart|geurkaars|facial-cups|neem-je-natuurlijke/.test(handle)
    ) {
      return false;
    }
    return /\b(sunscreen|spf|deodorants?|cream|creme|creame|balm|soap|hairwax|zinc)\b/i.test(hay);
  }
  if (brand.slug === "royal-tallow") {
    if (/\b(shipping protection)\b/i.test(title) || handle === "shipping-protection") return false;
    return /\b(deodorants?|soap|lip|balm)\b/i.test(hay);
  }
  if (brand.slug === "organicules") {
    if (
      /\b(bath bomb|shower steamer|gift card|e-gift)\b/i.test(title) ||
      /bath-bombs|shower-steamer|e-gift|gift-card/.test(handle)
    ) {
      return false;
    }
    return /\b(shampoo|conditioner|deodorants?|facial oil|jojoba|argan|castor|scrub)\b/i.test(hay);
  }
...
function flavorOptionIndex(raw, brand) {
  ...
  const handle = raw.handle || "";
  if (brand?.slug === "adapt-electrolytes" || brand?.slug === "go-good") return -1;
  if (brand?.slug === "tallow-cosmetics" || brand?.slug === "organicules") return -1;
  if (brand?.slug === "primal-life-organics") {
    if (
      handle === "dirty-mouth-primal-toothpowder" ||
      handle === "limited-edition-dirty-mouth-toothpowder" ||
      handle === "cinnamon-collection-powdered-mineral-toothpastes" ||
      handle === "kids-toothpowder-powdered-mineral-toothpaste" ||
      handle === "toothpaste-chews" ||
      handle === "natural-toothpaste" ||
      handle === "deodorant" ||
      handle === "cinnamon-shine-lip-balm"
    ) {
      return options.findIndex((o) => /flavor|scent|type|flavor\/size/i.test(o.name || ""));
    }
    return -1;
  }
  if (brand?.slug === "royal-tallow") {
...
function pickVariant(variants, brand) {
  ...
  const scored = variants.map((v, i) => {
    ...
    if (/\b(two-pack|2-pack|three-pack|3-pack|four-pack|4-pack|2 boxes|4 boxes|6 boxes|0\.5oz|0\.5 oz|20 servings|3x90|250 g|250g|500 g|500g|1 kg|1kg|5g|sample size|refill only|2 jars|3 jars|5 jars|2 bars|3 bars|4 oz\. amber|8 oz|plus 4 oz|plus 8 oz|2\.5lb|5lb|small\b)\b/i.test(hay))
      score -= 8;
    if (brand?.slug === "primal-life-organics") {
      if (/\b4 oz|4oz|3 oz|default title\b/i.test(hay)) score += 10;
      if (/\b1 oz|1oz\b/i.test(hay)) score += 2;
      if (/0\.25|one month|0\.75|mini|family|refill/i.test(hay)) score -= 10;
    }
    if (brand?.slug === "go-good") {
      if (/500g|500 g/i.test(hay)) score += 12;
      if (/1kg|1 kg|2kg|2 kg/i.test(hay)) score -= 10;
    }
    if (brand?.slug === "royal-tallow") {
      if (/default title|2 oz|small/i.test(hay)) score += 6;
      if (/4 oz|large/i.test(hay)) score -= 4;
    }
    if (brand?.slug === "organicules") {
      if (/compostable bag|default title|15 ml|15ml/i.test(hay)) score += 12;
      if (/aluminium tin|aluminum tin|30 ml|30ml|50 ml|50ml|100 ml|100ml/i.test(hay)) score -= 8;
    }
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel, currency) {
  let baseTitle = (raw.title || "")
    ...
    .trim();
  if (brand.slug === "primal-life-organics") {
    if (/toothpowder \/|powdered mineral toothpaste/i.test(baseTitle) && /kids/i.test(baseTitle))
      baseTitle = "Primal Life Organics Kids Toothpowder";
    else if (/toothpowder \/|powdered mineral toothpaste/i.test(baseTitle))
      baseTitle = "Primal Life Organics Toothpowder";
    else if (/limited edition dirty mouth/i.test(baseTitle))
      baseTitle = "Primal Life Organics Dirty Mouth Toothpowder";
    else if (/cinnamon collection/i.test(baseTitle))
      baseTitle = "Primal Life Organics Cinnamon Toothpowder";
    else if (/deodorant 3 oz|stick up \(3/i.test(baseTitle))
      baseTitle = "Primal Life Organics Stick Up Deodorant";
    else if (/real white pulling oil/i.test(baseTitle))
      baseTitle = "Primal Life Organics Pulling Oil";
    else if (/sun-up sun protection stick/i.test(baseTitle))
      baseTitle = "Primal Life Organics Sun-Up Stick";
    else if (/sun-up sun protection 3oz/i.test(baseTitle))
      baseTitle = "Primal Life Organics Sun-Up Tin";
    else if (!/^primal life\b/i.test(baseTitle)) baseTitle = `Primal Life Organics ${baseTitle}`;
  }
  if (brand.slug === "adapt-electrolytes") {
    if (/mixed-berry/i.test(baseTitle)) baseTitle = "ADAPT Mixed-Berry Electrolytes";
    else if (/orange/i.test(baseTitle)) baseTitle = "ADAPT Orange Electrolytes";
    else if (/lemon-lime/i.test(baseTitle)) baseTitle = "ADAPT Lemon-Lime Electrolytes";
    else if (!/^adapt\b/i.test(baseTitle)) baseTitle = `ADAPT ${baseTitle}`;
  }
  if (brand.slug === "go-good") {
    if (/whey protein isolate \+ organic vanilla/i.test(baseTitle))
      baseTitle = "Go Good Whey Isolate Organic Vanilla";
    else if (/whey protein isolate \+ organic chocolate/i.test(baseTitle))
      baseTitle = "Go Good Whey Isolate Organic Chocolate";
    else if (/whey protein isolate unflavoured/i.test(baseTitle))
      baseTitle = "Go Good Whey Isolate Unflavoured";
    else if (/whey protein powder \+ organic banana/i.test(baseTitle))
      baseTitle = "Go Good Whey Concentrate Organic Banana";
    else if (/whey protein powder \+ organic chocolate/i.test(baseTitle))
      baseTitle = "Go Good Whey Concentrate Organic Chocolate";
    else if (/whey protein powder \+ organic vanilla/i.test(baseTitle))
      baseTitle = "Go Good Whey Concentrate Organic Vanilla";
    else if (/whey protein powder \+ organic strawberry/i.test(baseTitle))
      baseTitle = "Go Good Whey Concentrate Organic Strawberry";
    else if (/whey protein powder unflavoured/i.test(baseTitle))
      baseTitle = "Go Good Whey Concentrate Unflavoured";
    else if (/plant protein isolate \+ organic coffee/i.test(baseTitle))
      baseTitle = "Go Good Plant Isolate Organic Coffee";
    else if (/plant protein isolate \+ organic vanilla/i.test(baseTitle))
      baseTitle = "Go Good Plant Isolate Organic Vanilla";
    else if (/plant protein isolate \+ organic chocolate/i.test(baseTitle))
      baseTitle = "Go Good Plant Isolate Organic Chocolate";
    else if (/creatine monohydrate unflavoured/i.test(baseTitle))
      baseTitle = "Go Good Creatine Monohydrate Unflavoured";
    else if (/creatine complete \+ wildberry/i.test(baseTitle))
      baseTitle = "Go Good Creatine Complete Wildberry";
    else if (/creatine complete \+ tropical/i.test(baseTitle))
      baseTitle = "Go Good Creatine Complete Tropical";
    else if (!/^go good\b/i.test(baseTitle)) baseTitle = `Go Good ${baseTitle}`;
  }
  if (brand.slug === "tallow-cosmetics") {
    if (/tallow natural sunscreen/i.test(baseTitle))
      baseTitle = "Tallow Cosmetics Mineral Sunscreen SPF 30";
    else if (/geranium/i.test(baseTitle))
      baseTitle = "Tallow Cosmetics Deodorant Geranium & Vanilla";
    else if (/lemongrass/i.test(baseTitle))
      baseTitle = "Tallow Cosmetics Deodorant Lemongrass & Patchouli";
    else if (!/^tallow cosmetics\b/i.test(baseTitle))
      baseTitle = `Tallow Cosmetics ${baseTitle}`;
  }
  if (brand.slug === "royal-tallow") {
    if (/tallow deodorant/i.test(baseTitle)) baseTitle = "Royal Tallow Deodorant";
    else if (/tallow soap/i.test(baseTitle)) baseTitle = "Royal Tallow Soap";
    else if (/lip balm/i.test(baseTitle)) baseTitle = "Royal Tallow Lip Balm";
    else if (/tallow balm/i.test(baseTitle)) baseTitle = "Royal Tallow Balm";
    else if (!/^royal tallow\b/i.test(baseTitle)) baseTitle = `Royal Tallow ${baseTitle}`;
  }
  if (brand.slug === "organicules") {
    if (!/^organicules\b/i.test(baseTitle)) baseTitle = `Organicules ${baseTitle}`;
  }
  ...
  if (brand.slug === "primal-life-organics") {
    if (/toothpowder/i.test(title)) {
      ...
      if (/charcoal|whitening/i.test(title)) ingredients.splice(3, 0, "Activated Charcoal");
    } else if (/deodorants?|stick up/i.test(title)) {
      ingredients = [
  ...
  if (brand.slug === "go-good") {
    if (/whey isolate unflavoured/i.test(title)) {
      ingredients = ["New Zealand Whey Protein Isolate", "Sunflower Lecithin"];
    } else if (/whey isolate organic vanilla/i.test(title)) {
      ingredients = [
        "New Zealand Whey Protein Isolate",
        "Sunflower Lecithin",
        "Organic Vanilla",
      ];
    } else if (/whey isolate organic chocolate/i.test(title)) {
      ingredients = [
        "New Zealand Whey Protein Isolate",
        "Sunflower Lecithin",
    ...
    } else if (/creatine monohydrate unflavoured/i.test(title)) {
  ...
  if (brand.slug === "royal-tallow") {
    ...
    } else if (/soap/i.test(title)) {
      ingredients = ["Saponified beef tallow"];
    } else {
      if (!looksLikeInci(ingredients, ingredients.join(", "))) ingredients = [];
    }
  }
  if (brand.slug === "organicules") {
    if (/shampoo bar \| normal/i.test(title)) {
      ingredients = [
    ...
    } else if (!looksLikeInci(ingredients, ingredients.join(", "))) {
      ingredients = [];
    }
  }
...
function expandProducts(raw, brand, brandId, index, currency) {
  ...
  if (flavorIdx >= 0 && variants.length > 1) {
    ...
    for (const v of variants) {
      ...
      if (brand?.slug === "primal-life-organics" && /0\.25|one month|family|refill/i.test(label))
        continue;
      if (brand?.slug === "organicules" && /tin|packaging/i.test(label)) continue;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave90.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave90Products: waveCount,
        wave90BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave90-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave90=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
