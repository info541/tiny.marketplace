/**
 * Fetch Shopify catalogs for wave50 brands (c1164+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave50.mjs";
...
const FLAVOR_OPTION = /flavor|flavour|scent|fragrance|smell|aroma|colour|color|shade|scented|finish|constitutive|herb|fluord|fluoride|the taste|dry shampoo colour|style|tint|skin tone|formula|whip|blend|infusi[oó]n|\bmaku\b|botanical bars|choose your scent|^product$|^variants$/i;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave50.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave50Products: waveCount,
        wave50BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave50-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave50=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
