/**
 * Drop leftover merch from wave43 and backfill missing ingredients from product pages.
 */
...
async function main() {
  ...
  const cleaned = all
    .filter((p) => {
      ...
      return !(MERCH.test(p.name) || EXTRA_DROP.test(p.name) || EXTRA_DROP_WAVE38.test(p.name) || EXTRA_DROP_WAVE41.test(p.name) || EXTRA_DROP_WAVE42.test(p.name) || EXTRA_DROP_WAVE43.test(p.name));
    })
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave43Products: wave.length,
    wave43WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave43-ingredients-backfill.json"),
