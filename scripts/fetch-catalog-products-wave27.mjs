/**
 * Fetch Shopify catalogs for wave27 brands (c907+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave27.mjs";
...
function inferCategory(brandCategories, title, productType, tags) {
  ...
  const rules = [
    ["sunscreen", /\b(spf|sunscreen|sun\s?screen|sunblock|sun butter|sun paste|sunpaste|sunbalm|sun[- ]?balm|sunguard|sun protection|sun stick|sun lotion|sun cream|after[- ]?sun|mineral defense|zinc stick|tinted zinc|mineral zinc)\b/],
...
function refineCategory(category, title) {
  const t = title.toLowerCase();
  if (/\b(sunbalm|sun[- ]?balm|sunguard|tinted zinc|mineral zinc|zinc tallow)\b/.test(t)) {
    return "sunscreen";
  }
  if (/\b(toothpaste|tooth powder|mouth rinse|pulling oil|hydroxyapatite)\b/.test(t)) {
    return "oral";
  }
  if (/\b(deodorant|deo\b)\b/.test(t)) return "deodorant";
  if (/\b(shampoo|conditioner|hair (oil|mask|serum)|scalp)\b/.test(t) && !/\blip\b/.test(t)) {
    return "hair";
  }
  if (
    /\b(lip balm|bar soap|sphere soap|toner|salt scrub|diaper cream|face (oil|cream|balm|serum)|body (oil|balm|butter)|tallow balm|whipped tallow)\b/.test(
      t,
    )
  ) {
    return "skincare";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave27.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave27Products: waveCount,
        wave27BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave27-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave27=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
