/**
 * Write src/lib/catalog-brands-wave39.ts from the wave39 seed.
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { brands } from "./catalog-brands-seed-wave39.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const out = path.join(__dirname, "../src/lib/catalog-brands-wave39.ts");
...
const rows = brands.map((b, i) => {
  const id = `c${String(i + 1060).padStart(4, "0")}`;
  return `  {
    id: ${tsString(id)},
