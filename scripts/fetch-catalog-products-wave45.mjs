/**
 * Fetch Shopify catalogs for wave45 brands (c1129+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave45.mjs";
...
const FLAVOR_OPTION = /flavor|flavour|scent|fragrance|smell|aroma|colour|color|shade|scented|finish|constitutive|herb|fluord|fluoride|the taste|dry shampoo colour|style|tint|formula|whip|blend|infusi[oó]n|\bmaku\b|botanical bars|choose your scent|^product$/i;

/** Convert shop-native prices that would otherwise look like USD luxury SKUs. */
const FX_TO_USD = {
  // Halo's products.json already returns USD (price_currency: USD). Do not apply GBP FX.
};
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave45.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave45Products: waveCount,
        wave45BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave45-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave45=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
