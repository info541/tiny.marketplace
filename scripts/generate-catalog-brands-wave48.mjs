/**
 * Write src/lib/catalog-brands-wave48.ts from the wave48 seed.
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { brands } from "./catalog-brands-seed-wave48.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const out = path.join(__dirname, "../src/lib/catalog-brands-wave48.ts");
...
const rows = brands.map((b, i) => {
  const id = `c${String(i + 1153).padStart(4, "0")}`;
  return `  {
    id: ${tsString(id)},
