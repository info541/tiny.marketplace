/**
 * Drop leftover merch from wave61 and backfill missing ingredients from product pages.
 */
...
const WAVE_START = 1235;
const WAVE_END = 1241;
// 7 brands: c1235–c1241
...
function reclassify(p) {
  ...
  const n = Number(String(p.brandId).replace(/^c/, ""));
  if (n === 1235) p.category = "sunscreen";
  if (n === 1236) p.category = "oral";
  if (n === 1237) {
  ...
  if (n === 1238) p.category = "protein";
  if (n === 1239) {
    p.category = /\b(sunscreen|zinc|field day)\b/.test(t) ? "sunscreen" : "skincare";
  }
  if (n === 1240) {
    if (/\b(zonnebalsem|spf|zinc|sun)\b/.test(t)) p.category = "sunscreen";
    else if (/\b(tandpoeder|tooth)\b/.test(t)) p.category = "oral";
    else p.category = "skincare";
  }
  if (n === 1241) p.category = "supplements";
  if (n === 1229) p.category = "sunscreen";
  if (n === 1230) p.category = "oral";
  if (n === 1231) p.category = "oral";
  if (n === 1232) {
    p.category = /\b(shampoo|conditioner|hair)\b/.test(t) ? "hair" : "skincare";
  }
  if (n === 1233) {
    p.category = /\b(crush|toothpaste|toothbrush|tongue|mouthwash|oral|floss|hydroxyapatite|spearmint|citrus spice|moringa|fennel)\b/.test(t)
      ? "oral"
      : "deodorant";
  }
  if (n === 1234) p.category = "supplements";
  if (n === 1223) p.category = "electrolytes";
  if (n === 1224) {
    if (/\b(toothpaste|toothbrush|tongue|oral|tablet)\b/.test(t)) p.category = "oral";
    else if (/\b(shampoo|conditioner|hair)\b/.test(t)) p.category = "hair";
    else p.category = "skincare";
  }
  if (n === 1225) p.category = "deodorant";
  if (n === 1226) {
    if (/\b(sunscreen|spf|zinc)\b/.test(t)) p.category = "sunscreen";
    else if (/\bdeodorant\b/.test(t)) p.category = "deodorant";
  ...
  if (n === 1228) p.category = "supplements";
  if (n === 1218) p.category = "oral";
  if (n === 1219) p.category = "protein";
  if (n === 1220) p.category = "electrolytes";
  if (n === 1221) {
    p.category = /\b(coffee|mushroom)\b/.test(t) && !/electrolyt|hydrat/.test(t)
      ? "supplements"
      : "electrolytes";
  }
  if (n === 1222) {
...
async function main() {
    ...
    return (parts || [])
      ...
      .filter(
        (s) =>
          ...
          !/happiness guarantee|satisfaction guarantee|email our friendly|e mail our friendly|take our free hair quiz|add to cart|that's it|no bs|stored in an amber|it can be used|gentle enough for baby|packed with the goodness|idyll|net weight|helping you achieve|this natural hair growth|including batana|topped with dried|tallow me honey is our|for more information|see our page on essential|if you buy a product|return it to us|the cut may vary|immerse yourself|sensory journey|just push it up|providing a calming|our infusion of these|deeply hydrating and soothing|it provides a gentle|moisturizing marvel|its nourishing properties|leaves hair nourished|vegan and palm oil-free|replaces up to|plastic-free packaging|gentle ingredients|so we're here|bar saver|planning to travel|full of nourishing|leaves hair feeling|perfect travel|made with nourishing|salty scent|great for hands|reducing your environmental|outer papers contain|recycled or composted|british journal|components of essential oil|handcrafted the way your grandmother|used for centuries|saponification|inhaltsstoffe|mundmilieu|fluoridfrei|does not contain|sls or fluoride|love & gratitude|bestandteile natürlicher|für wen ist|zahngesundheit|orales mikrobiom|weniger sauren|^(free from|scent:|eco tube:|all of the papers|the interior|it's easy to use|instead of twisting|soft|honest|and natural|protect|and restore|this is the)$/i.test(
            s,
          ),
      );
  }
  ...
  const cleaned = all
    .filter((p) => {
      ...
      const keepWave61 =
        nId >= 1235 && nId <= 1241 &&
        !/\b(whitening strips|electric toothbrush|vibrosonic|water flosser|instruments kit|eugenol|dental care kit|beeswax wraps|soap dish|big bundle|five pack|woodsy bundle|floral bundle|range rinse|crusader|hard on target|field hygiene|soap set|farmer'?s lot|working skincare|fat kiss bundle|dog shampoo|handdoek|zeepbakje|badzout|paardenhaar|umami|sun dried)\b/i.test(p.name) &&
        (
          (nId === 1235 && /\b(spf|sunscreen|sun barrier)\b/i.test(p.name)) ||
          (nId === 1236 && /\b(toothpaste|tablets?|tongue scraper|hydroxyapatite|n[- ]?ha)\b/i.test(p.name) && !/\b(whitening strips|electric|flosser|instruments|eugenol)\b/i.test(p.name)) ||
          (nId === 1237 && /\b(whipped tallow|tallow body balm|sun balm|tallow deodorant|tallow shampoo|tallow soap|liquid tallow|freedom bar)\b/i.test(p.name) && !/\b(bundle|five pack|kit|tee|candle|wraps|dish)\b/i.test(p.name)) ||
          (nId === 1238 && /\b(whey|protein)\b/i.test(p.name)) ||
          (nId === 1239 && /\b(tallow balm|soap bar|sunscreen|sun protection|fat kiss|lip balm)\b/i.test(p.name) && !/\b(bundle|farmer'?s lot|dog)\b/i.test(p.name)) ||
          (nId === 1240 && /\b(tallow|zonnebalsem|tandpoeder|zeep|lippenbalsem|aleppo|zink)\b/i.test(p.name) && !/\b(handdoek|zeepbakje|badzout|paardenhaar|komijn)\b/i.test(p.name)) ||
          (nId === 1241 && /\b(tincture|muddy waterz|daily dose|chaga powder|lion'?s mane|mushroom coffee|mushroom blend)\b/i.test(p.name) && !/\b(umami|sun dried|seasoning)\b/i.test(p.name))
        );
      if (keepWave61) return true;
      if (nId >= 1235 && nId <= 1241) return false;
      const keepWave60 =
        nId >= 1229 && nId <= 1234 &&
        !WAVE60_MERCH.test(p.name) &&
        (
          (nId === 1229 && /\b(spf|sunscreen|wattz)\b/i.test(p.name)) ||
          (nId === 1230 && /\b(zahnpulver|hydroxylapatit|hydroxyapatite)\b/i.test(p.name) && !/\b(set|bundle|tiegel|4er)\b/i.test(p.name)) ||
          (nId === 1231 && /\b(zero pro|zero toothpaste|halo|nanofloss|versa floss|braces floss)\b/i.test(p.name) && !/\b(kit|aquaclean|charger|nozzle|lumismart|3 pack)\b/i.test(p.name)) ||
          (nId === 1232 && /\b(bar soap|tallow balm|glow face oil|face & body moisturizer|shampoo bar|conditioner bar|soap)\b/i.test(p.name) && !/\b(brush|comb|dish|sisal|bundle|collection|gift card|aromatherapy|set with|fugly|combo|variety set|subscription)\b/i.test(p.name) && !/\|/.test(p.name)) ||
          (nId === 1233 && /\b(deodorant|crush & brush|crush & swish|tongue scraper|toothbrush|toothpaste|floss|hydroxyapatite|spearmint|citrus spice|moringa|fennel)\b/i.test(p.name) && !/\b(wholesale|bulk|raven roast|sample|5 pack|4 pack|duo|application tool|imperfect|full case|tablet refill|best sellers|hydroxyapatite set)\b/i.test(p.name)) ||
          (nId === 1234 && /\b(lion'?s mane|reishi|cordyceps|chaga|turkey tail|rise mushroom|rest mushroom|mushroom coffee|mushroom extract|mushroom complex)\b/i.test(p.name) && !/\b(bundle|dried)\b/i.test(p.name))
        );
      if (keepWave60) return true;
      if (nId >= 1229 && nId <= 1234) return false;
      const keepWave59 =
        nId >= 1223 && nId <= 1228 &&
        !WAVE59_MERCH.test(p.name) &&
        (
          (nId === 1223 && /\b(starter pack|full tub|elektra|berry splash)\b/i.test(p.name)) ||
          (nId === 1224 && /\b(toothpaste tablets|tongue scraper|bamboo toothbrush|shampoo|conditioner|body wash|body lotion)\b/i.test(p.name) && !/\b(electric|guide|shower|air pod|boxers|glasses|wool)\b/i.test(p.name)) ||
          (nId === 1225 && !/\bbundle\b/i.test(p.name)) ||
          (nId === 1226 && /\b(sunscreen|deodorant|balm|cleanser|cream|lip|beard)\b/i.test(p.name) && !/\b(gift card|bundle)\b/i.test(p.name)) ||
          (nId === 1227 && /\b(tallow|deodorant|shampoo|soap|lip|beard|rosanera)\b/i.test(p.name) && !/\b(bath salt|bug off|tray|saver|pet)\b/i.test(p.name)) ||
          (nId === 1228 && /\b(lion'?s mane|reishi|cordyceps|extract)\b/i.test(p.name))
        );
      if (keepWave59) return true;
      if (nId >= 1223 && nId <= 1228) return false;
      const keepWave57 =
        nId >= 1212 && nId <= 1217 &&
        (
  ...
  const WAVE58_KNOWN = [
    ...
    {
      test: (p) => p.brandId === "c1221" && /lemon lime/i.test(p.name),
      ingredients: [
        "Himalayan rock salt",
        "Potassium chloride",
        "Magnesium glycinate",
        "Dicalcium phosphate",
        "Organic Irish sea moss powder",
        "Taurine",
        "Organic lemon and lime juice powder",
        "Non-GMO citric acid (from fermented cassava root)",
        "Organic monk fruit extract",
      ],
    },
    {
      test: (p) => p.brandId === "c1221" && /mixed berry/i.test(p.name),
      ingredients: [
        "Himalayan rock salt",
        "Potassium chloride",
        "Magnesium glycinate",
    ...
    {
      test: (p) => p.brandId === "c1221" && /tropical/i.test(p.name),
      ingredients: [
        "Himalayan rock salt",
        "Potassium chloride",
        "Magnesium glycinate",
        ...
        "Organic Irish sea moss powder",
        "Taurine",
        "Organic pineapple / guava / passionfruit flavor",
        "Non-GMO citric acid (from fermented cassava root)",
        "Organic monk fruit extract",
      ],
    },
    {
      test: (p) => p.brandId === "c1221" && /mushroom coffee/i.test(p.name),
      ingredients: [
        ...
        "Alpha GPC",
        "L-Theanine",
        "Organic ginkgo biloba leaf powder",
        "Organic cordyceps mushroom 8:1 fruiting body extract",
        "Organic tremella mushroom 8:1 fruiting body extract",
        "Organic chaga mushroom 8:1 fruiting body extract",
        "Organic lion's mane mushroom 8:1 fruiting body extract",
      ],
    },
    {
      test: (p) => p.brandId === "c1222" && /lotus seed cleansing oil/i.test(p.name),
      ingredients: [
        "Lotus seed oil",
        "Olive oil",
        "Coconut oil",
    ...
    {
      test: (p) => p.brandId === "c1222" && /strawberry clouds/i.test(p.name),
      ingredients: [
        "Aqua",
        "Aloe",
        "Glycerin",
    ...
    {
      test: (p) => p.brandId === "c1222" && /itch ease/i.test(p.name),
      ingredients: [
    ...
    {
      test: (p) => p.brandId === "c1222" && /cranberry aspen soap/i.test(p.name),
      ingredients: [
        "Shea butter",
        "Coconut oil",
        "Olive oil",
  ...
  const WAVE60_KNOWN = [
    ...
    {
      test: (p) => p.brandId === "c1230" && /crisp mint/i.test(p.name),
      ingredients: [
        "Calcium Carbonate",
        "Xylitol",
        "Sodium Cocoyl Glutamate",
        "Ananas Comosus Fruit Extract (Bromelain)",
        "Hydroxyapatite",
        "Mentha Piperita Oil",
        "Menthol",
    ...
    {
      test: (p) => p.brandId === "c1230" && /soothing mint/i.test(p.name),
      ingredients: [
        "Xylitol",
        "Hydrated Silica",
        "Sodium Cocoyl Glutamate",
    ...
    {
      test: (p) => p.brandId === "c1230" && /orange-mandarine/i.test(p.name),
      ingredients: [
        "Xylitol",
        "Hydrated Silica",
        "Sodium Cocoyl Glutamate",
    ...
    {
      test: (p) => p.brandId === "c1231" && /^zero toothpaste$/i.test(p.name),
      ingredients: [
        "Water (Aqua)",
        "Glycerin",
        "Hydrated Silica",
    ...
    {
      test: (p) => p.brandId === "c1231" && /zero pro toothpaste/i.test(p.name),
      ingredients: [
        "Water (Aqua)",
        "Glycerin",
        "Hydrated Silica",
    ...
    {
      test: (p) => p.brandId === "c1231" && /halo/i.test(p.name),
      ingredients: [
        "Xylitol",
        "Gum Base (Chicle)",
        "Nano-Hydroxyapatite",
    ...
    {
      test: (p) => p.brandId === "c1231" && /nanofloss/i.test(p.name),
      ingredients: ["Nano-hydroxyapatite coating", "Vegan wax"],
    },
    {
      test: (p) => p.brandId === "c1233" && /crush & brush mint$/i.test(p.name),
      ingredients: [
        "Calcium carbonate",
        "Sodium bicarbonate",
        "Xylitol",
    ...
    {
      test: (p) => p.brandId === "c1233" && /crush & brush with hydroxyapatite mint/i.test(p.name),
      ingredients: [
        "Calcium carbonate",
        "Sodium bicarbonate",
        "Xylitol",
    ...
    {
      test: (p) => p.brandId === "c1233" && /crush & brush grape/i.test(p.name),
      ingredients: [
        "Calcium carbonate",
        "Sodium bicarbonate",
        "Xylitol",
    ...
    {
      test: (p) => p.brandId === "c1233" && /crush & brush rootbeer/i.test(p.name),
      ingredients: [
        "Calcium carbonate",
        "Sodium bicarbonate",
        "Xylitol",
        ...
        "Citric acid",
        "Sea salt",
        "Natural rootbeer flavour",
        "Magnesium stearate",
        "Silica",
      ],
    },
    {
      test: (p) => p.brandId === "c1233" && /crush & swish/i.test(p.name),
      ingredients: [
        "Xylitol",
        "Sodium bicarbonate",
        "Calcium carbonate",
    ...
    {
      test: (p) => p.brandId === "c1233" && /dom'?s deodorant - fresh/i.test(p.name),
      ingredients: [
    ...
    {
      test: (p) => p.brandId === "c1233" && /dom'?s deodorant - bold/i.test(p.name),
      ingredients: [
        "Helianthus annuus (sunflower) seed oil",
        "Zea mays (corn) starch",
        "Sodium bicarbonate",
    ...
    {
      test: (p) => p.brandId === "c1233" && /dom'?s deodorant - nude/i.test(p.name),
      ingredients: [
        "Helianthus annuus (sunflower) seed oil",
        "Zea mays (corn) starch",
        "Sodium bicarbonate",
    ...
    {
      test: (p) => p.brandId === "c1233" && /vanilla\/mint stick/i.test(p.name),
      ingredients: [
    ...
    {
      test: (p) => p.brandId === "c1233" && /black pepper\/lime stick/i.test(p.name),
      ingredients: [
        "Maranta arundinacea (arrowroot) powder",
        "Cocos nucifera (coconut) oil",
        "Theobroma cacao (cocoa) butter",
        ...
        "Copaifera officinalis (balsam copaiba) oil",
      ],
    },
    {
      test: (p) => p.brandId === "c1234" && /rise mushroom complex/i.test(p.name),
      ingredients: ["Lion's Mane", "Cordyceps", "Maitake", "Chaga"],
    },
  ];
  ...
  for (const p of cleaned.filter(isWave26)) {
    ...
    for (const row of WAVE58_KNOWN) {
      if (row.test(p)) p.ingredients = row.ingredients;
    }
    for (const row of WAVE60_KNOWN) {
      if (row.test(p)) p.ingredients = row.ingredients;
    }
    if (p.brandId && Number(String(p.brandId).replace(/^c/, "")) >= 1229) {
    ...
    if (p.brandId === "c1222" && /cream deodorant/i.test(p.name)) {
      p.ingredients = [];
    }
    if (
      (p.ingredients || []).some((s) =>
        /your cart|press the space key|new subtotal|close \(esc\)|full page refresh|powered by shopify/i.test(
          s,
        ),
      )
    ) {
    ...
    const nId = Number(String(p.brandId).replace(/^c/, ""));
    if (nId >= 1235 && nId <= 1241 && p.ingredients?.length) {
      p.ingredients = p.ingredients
        .map((s) =>
          s
            .replace(/^(activated charcoal|fennel|citrus spice|fresh mint|moringa forest fresh|spearmint|vanilla orange)\s*:\s*/i, "")
            .replace(/^&\s+/, "")
            .replace(/\s*\(our bentonite clay is food-grade\)\s*$/i, "")
            .replace(/\s*\(organic & vegan\)\s*$/i, "")
            .trim(),
        )
        .filter(
          (s) =>
            ...
            s.length < 90 &&
            !/^(and|or|&)$/i.test(s) &&
            !/\b(how to use|directions|massage gently|rub the shampoo|two extractions|one reason|most lion|we run two|we will work|once your order|delivery times|full spectrum|no shortcuts|we use dual|rise mushroom complex combines|without caffeine|mental clarity|immune health|food-grade|wet hair thoroughly|you will receive an email|further information)\b/i.test(
              s,
            ),
        );
      if (
        /we will work quickly|two extractions|full spectrum|no shortcuts|rise mushroom complex combines|delivery times vary/i.test(
          (p.ingredients || []).join(" "),
        )
      ) {
        p.ingredients = [];
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave61Products: wave.length,
    wave61WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave61-ingredients-backfill.json"),
