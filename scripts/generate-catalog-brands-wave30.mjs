/**
 * Write src/lib/catalog-brands-wave30.ts from the wave30 seed.
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { brands } from "./catalog-brands-seed-wave30.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const out = path.join(__dirname, "../src/lib/catalog-brands-wave30.ts");
...
const rows = brands.map((b, i) => {
  const id = `c${String(i + 956).padStart(3, "0")}`;
  return `  {
    id: ${tsString(id)},
