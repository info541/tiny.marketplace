/**
 * Drop leftover merch from wave6 and backfill missing ingredients from product pages.
 */
...
const MERCH =
  /\b(stuffed toy|plush|crew socks|socks\b|head wrap|tote bag|statement tote|silk scarf|t-?shirt|tee\b|hoodie|sweatshirt|beanie|sticker pack|sticker\b|mug\b|apparel|poster|patch\b|keychain|book\b|journal\b|candle holder|gift card|e[- ]?gift|shaker bottle|water bottle|empty bottle|fanny pack|luggage tag|enamel pin|marketing card|deodorant scoop|cream applicator|aluminum bottle|plastic bottle|blue bottle|brown bottle|clear bottle|clear jar|roll-on bottle|for pets?|daily dawg|there's a mushroom)\b/i;
...
async function main() {
  ...
  const cleaned = all.filter((p) => !(isWave6(p) && MERCH.test(p.name)));
  const dropped = before - cleaned.length;

  const missing = cleaned.filter((p) => isWave6(p) && (!p.ingredients || p.ingredients.length === 0));
  let filled = 0;
  ...
  const wave = cleaned.filter(isWave6);
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave6Products: wave.length,
    wave6WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave6-ingredients-backfill.json"),
