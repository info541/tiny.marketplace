/**
 * Download brand logos into public/brands/{slug}.png (or .svg fallback)
 * Primary: logo.uplead.com
 * Fallback: Google favicon (128)
 * Last resort: generated SVG monogram
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { brands } from "./catalog-brands-seed.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const outDir = path.join(__dirname, "../public/brands");
const catalogPath = path.join(__dirname, "../src/lib/catalog-brands.ts");
...
fs.writeFileSync(
  path.join(__dirname, "../data/brand-logo-manifest.json"),
