/**
 * Fetch Shopify catalogs for wave35 brands (c1019+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave35.mjs";
...
const FLAVOR_OPTION = /flavor|flavour|scent|fragrance|smell|aroma|colour|color|shade|scented|finish|constitutive|herb|fluord|fluoride|the taste|dry shampoo colour/i;

/** Convert shop-native prices that would otherwise look like USD luxury SKUs. */
const FX_TO_USD = {
  "green-routine": 1.27, // GBP
  "myhydro": 2.65, // BHD
};
...
function refineCategory(category, title, brand) {
  ...
  if (/\b(sunbalms?|sun[- ]?balms?|sunguard|tinted zinc|mineral zinc|zinc tallow|safe sunscreen|sun protectant|after[- ]?sun)\b/.test(t)) {
    return "sunscreen";
  }
  ...
  if (/\b(hydration drops?|electrolytes?|electrolyte powder|yuzu citrus|asian pear|white peach|lychee|watermelon electrolyte|jamaica electrolyte|acai electrolyte|pog electrolyte|electrolyte (tub|singles|caps|mix)|hydration & recovery|sleep & hydration|raspberry electrolyte|lemon and lime electrolyte|adapt variety|adapt sleep|adapt 30pk|adapt sample)\b/.test(t) && !/\bprotein\b/.test(t)) {
    return "electrolytes";
  }
  if (/\b(sun balm|mineral melt|mug guard|bod guard|sporto spray|melt stick|slip stick|golden guard|gold standard|the lifeguard|the gang|daily double|mineral spf|miracle jelly|miracle mask)\b/.test(t)) {
    return "sunscreen";
  }
  ...
  if (/\b(nootropic energy|energy powder)\b/.test(t) && !/\belectrolyt\b/.test(t)) return "supplements";
  if (/\b(marine collagen|collagen peptides|lion'?s mane|reishi|chaga|turkey tail|tincture|bitters|immunity tonic)\b/.test(t)) {
    return "supplements";
  }
  if (/\b(whey protein|plant protein|pea protein|whey isolate|clear (whey )?protein|milk tea whey|mushroom latte whey)\b/.test(t)) {
    return "protein";
  }
  ...
  if (/\b(shampoo|conditioner|hair (oil|mask|serum|cleanser|gummies)|scalp|curl (cleanser|cream|conditioner)|protein booster|oil blend|dry shampoo)\b/.test(t) && !/\blip\b/.test(t)) {
    return "hair";
  }
  if (/\b(recovery drink|plant protein|hydration hero|blg)\b/.test(t)) return "protein";
  if (
    /\b(lip balm|lip elixir|love your lips|bar soap|sphere soap|toner|salt scrub|diaper cream|face (oil|cream|balm|serum)|body (oil|balm|butter|wash|cream)|tallow balm|tallow whip|whipped tallow|moisturi[sz]er|cleanser|cleansing oil|face mist|bath melt)\b/.test(
      t,
    )
  ) {
    return "skincare";
...
function isOnNiche(raw, brand) {
  ...
  if (skipped) {
    ...
    const keepDuneLifeguard = brand.slug === "dune-suncare" && /\b(lifeguard|mineral melt|mug guard|bod guard)\b/i.test(title);
    if (!keepDespiteEssentialOil && !keepSample && !keepTallowAftershave && !keepProteinBag && !keepDuneLifeguard) return false;
  }
  if (NICHE.test(hay) || /\bscrub\b/i.test(hay)) return true;
  if (brand.slug === "aelia" && /\bmineral spf\b/i.test(title)) return true;
  if (brand.slug === "dune-suncare" && /\b(lifeguard|mineral melt|mug guard|bod guard|sporto|melt stick|slip stick|golden guard|gold standard|miracle|daily double|the gang)\b/i.test(title)) {
    return true;
  }
  if (brand.slug === "ecofam" && /\b(toothpaste|mouthwash|hydroxyapatite)\b/i.test(title)) return true;
  if (brand.slug === "pioneer-tallow" && /\b(tallow|goat.?s milk|lotion|moisturizer)\b/i.test(title)) return true;
  if (brand.slug === "field-tallow" && /\b(tallow|balm|vanilla sky|quiet|range)\b/i.test(title)) return true;
  if (brand.slug === "good-flower-farm" && /\b(deodorant|lip balm|face cream|serum|toner|mist|mask|cleanser|body (oil|cream)|salve|beauty balm|everywhere balm|glide balm)\b/i.test(title)) {
    return true;
  }
  if (brand.slug === "sunniemade" && /\b(shampoo|conditioner|lip balm|bar soap)\b/i.test(title)) return true;
  if (brand.slug === "green-routine" && /\b(toothpaste|hydroxyapatite|floss|toothbrush|shampoo|conditioner|body wash|body bar|soap|body scrub)\b/i.test(title)) {
    return true;
  }
  if (brand.slug === "smash-melon" && /\bprotein\b/i.test(title)) return true;
  if (brand.slug === "kellemen" && /\b(deodorant|body fuel|warmup)\b/i.test(title)) return true;
  if (brand.slug === "santa-cruz-paleo" && /\b(electrolyte|whey|protein|tallow|sun balm|moisturizer|balm|creatine|magnesium|ashwagandha|nootropic|vitamin|sleep caps|nac)\b/i.test(title)) {
    return true;
  }
  if (brand.slug === "adapt-electrolytes" && /\b(adapt|electrolyte|hydration|drink mix)\b/i.test(hay)) return true;
  if (brand.slug === "myhydro" && /\belectrolyte\b/i.test(title)) return true;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave35.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave35Products: waveCount,
        wave35BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave35-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave35=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
