/**
 * Fetch Shopify catalogs for wave62 brands (c1242+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave62.mjs";
...
const WAVE62_EXTRA =
  /\b(insect repellent|egift|e-gift|the duo|bamboo toothbrush|power pack|gift pack|refill pack|starter pack|nightgown|face tape|apothecary'?s trio|evening heirloom|lip balm - 3 pack|3 pack|culinary tallow|protein oats|wafer bar|xray-ted|gift sample|mesh soap|soap pouch)\b/i;
...
function isOnNiche(raw, brand) {
  ...
  const skipped =
    ...
    SKIP_TYPE.test(type) ||
    SKIP_TAGS.test(tags);
  if (WAVE62_EXTRA.test(title) || WAVE61_EXTRA.test(title) || WAVE60_EXTRA.test(title) || WAVE59_EXTRA.test(title) || WAVE58_EXTRA.test(title) || WAVE57_EXTRA.test(title) || WAVE56_EXTRA.test(title) || WAVE54_SKIP.test(title) || WAVE54_EXTRA.test(title)) return false;
  if (
    brand.slug === "eezysun" &&
  ...
  if (brand.slug === "arame" && /\b(the paste|the polish)\b/i.test(title) && !WAVE62_EXTRA.test(title)) {
    return true;
  }
  if (
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave62.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave62Products: waveCount,
        wave62BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave62-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave62=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
