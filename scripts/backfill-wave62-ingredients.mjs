/**
 * Drop leftover merch from wave62 and backfill missing ingredients from product pages.
 */
...
const EXTRA_DROP =
  /\b(insect repellent|egift|the duo|bamboo toothbrush|power pack|gift pack|refill pack|starter pack|nightgown|face tape|apothecary|heirloom collection|3 pack|culinary tallow|protein oats|wafer bar|xray-ted|gift sample|mesh soap)\b/i;
...
function looksLikeMarketing(raw) {
  return /\b(how to (use|apply)|directions?|why (we|tallow)|add to cart|subscription|if you haven'?t tried|love & gratitude|shipping|refund|free from|ours doesn'?t|four actives|sports.activity)\b/i.test(
    raw || "",
  );
}
...
function reclassify(p) {
  ...
  const n = Number(String(p.brandId).replace(/^c/, ""));
  if (n === 1242) p.category = "sunscreen";
  if (n === 1243) p.category = "oral";
  if (n === 1244) p.category = "deodorant";
  if (n === 1245) p.category = "electrolytes";
  if (n === 1246) p.category = "protein";
  if (n === 1247) p.category = /\b(hair oil|beard)\b/.test(t) ? "hair" : "skincare";
  if (n === 1248) {
...
async function main() {
  ...
  fs.writeFileSync(path.join(root, "data/wave62-ingredients-backfill.json"), JSON.stringify(report, null, 2));
  console.log(report);
}
