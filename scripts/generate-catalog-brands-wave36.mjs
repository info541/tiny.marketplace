/**
 * Write src/lib/catalog-brands-wave36.ts from the wave36 seed.
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { brands } from "./catalog-brands-seed-wave36.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const out = path.join(__dirname, "../src/lib/catalog-brands-wave36.ts");
...
const rows = brands.map((b, i) => {
  const id = `c${String(i + 1032).padStart(4, "0")}`;
  return `  {
    id: ${tsString(id)},
...
/** Wave 36: small organic / toxic-free brands across marketplace categories (c1019+). */
