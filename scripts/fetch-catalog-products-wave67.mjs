/**
 * Fetch Shopify catalogs for wave67 brands (c1272+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave67.mjs";
...
const WAVE67_EXTRA =
  /\b(gift card|e-gift|mystery box|soap of the month|monthly mystery|odds & ends|clearance products|wax melt|healthy pup|dog shampoo|sample size|sample (packet|bag)|top 3 bundle|grow kit|grow bag|grain spawn|liquid culture|bulk substrate|vermiculite|gypsum|innoculated|inoculated|whole dried|perfume oil|natural perfume|solid natural perfume|concealer|eye shadow|eyeshadow|cream blush|minerals foundation|cc beauty|aint no bugs|tinted lip|lip & cheek|cheek tint|mascara|flawless finish powder|mattifying powder|primer serum)\b/i;

const WAVE66_EXTRA =
  /\b(gift (card|set|box)|trio gift|4 months?|2 people|3 people|4 people|ritual kit|protein balls?|protein brioche|primal surge|primal core|panache|perfume oil|grooming kit|clay and spray|grow kit|dried (lion|shiitake|morel)|all-in-one grow|konjac|toothbrush holder|toothbrush twin|pearl serum|solid beads)\b/i;

const WAVE64_EXTRA =
  /\b(bundle|gift card|e-gift|shipping protection|whitening pen|pitmaster|chef'?s tallow|raw honey|wax melt|t-melt|candle|provisions set|gentleman'?s|nurtured provisions|beehive provisions|essentials provisions|basic provisions|full provisions|standard provisions|clarify bundle|t-stick bundle|t-soap bundle|syrup|room spray|loose leaf tea|short-sleeve tee|\btee\b|zipper pouch|gift set|gift wrapping|gift certificate|perfume|cologne|dog bar|gua sha|dry brush|soap dish|sisal|soap shreds|breast milk|custom order|bath salts|tinted lip|stack|mastery|kit|ultimate ancestral|radiance bundle|glow and levity|women'?s radiance|ancestral nutrition stacks|wax melt|car bliss|diffuser|essential oil|soap rack|soap draining|bath sprinkle|in store only|misc\.|gift card|toothbrush set|local pickup|his & her|his and her)\b/i;

const WAVE63_EXTRA =
  /\b(gift card|gift certificate|pulse point|regeneration protocol|barrier repair system|hair & scalp oil and shampoo bar|beauty bundle|tinted tallow makeup|tallow cheek|cheek tint|cheek balm|being cheeky|just glow|complete set|0\.50 oz samples|lip balm - 4 pack|tinted lip|nude lip|pink lip|bronze lip|hot mess|first date|mama bear|wallflower|brass knuckles|art prints|chocolate|gummies bundle|gummies \(trio\)|essence book|trio bundle|multi-mush|e-gift|mushroom essence|fly agaric|green death|liberty cap)\b/i;

const WAVE61_EXTRA =
  /\b(whitening strips|oral hygiene instruments|dental care kit|zinc oxide eugenol|electric toothbrush|vibrosonic|uv smart|water flosser|tallow \+ besswax|beeswax wraps|soap dish|big bundle|grass-fed five pack|woodsy bundle|floral bundle|range rinse|crusader pocket|hard on target|field hygiene|soap set \(7|farmer'?s lot|working skincare bundle|fat kiss bundle|dog shampoo|katoenen handdoek|hemu houten|badzout|paardenhaar|umami seasoning|sun dried (oyster|shiitake|lions? mane))\b/i;

const WAVE60_EXTRA =
  /\b(kinder zahnpflege|familien-set|kennenlern-bundle|refill-set 3|einsteiger-set|braunglas-tiegel|4er-set|whitening kit|bentist kit|braces care kit|smart start kit|daily care kit|aquaclean|usb-c charger|nozzle refill|biofuse™? kit|essentials kit|complete kit|zero pro 3 pack|lumismart|facial cleansing brush|massage comb|soap dish|sisal bag|gift card|bundle subscription|aromatherapy|travel tins only|shampoo & conditioner set|fugly \+|wholesale|bulk 1 ?l|raven roast|dried (lions? mane|reishi)|rise by day|extract bundle|support bundle|energy & endurance bundle|immune support and inflammation|focus & clarity bundle|bundle:|crush & swish sample|crush & brush sample|variety set|moisturizer combo|glow face oil with|toothpaste 5 pack|best sellers pack|hydroxyapatite set|tablet refills|application tool|imperfectly|toothpaste and mouthwash|full case|adult toothbrushes- 4)\b/i;
...
function inferCategory(brandCategories, title, productType, tags) {
  ...
  const rules = [
    ...
    ["oral", /\b(toothpaste|toothbrush|floss|mouthwash|oral care|oral health|hydroxyapatite|hydroxylapatit|zahnpulver|zahnbürste|n[- ]?ha|theobromine|oil pull|tongue clean|teeth|tooth (powder|serum|paste)|enamelizer|zahnputz|nanofloss|advanced gum)\b/],
...
function refineCategory(category, title, brand) {
  ...
  if (brand?.slug === "gudyu") return "oral";
  if (brand?.slug === "natrilyte") return "electrolytes";
  if (brand?.slug === "rainwater-botanicals") {
    if (/\b(spf|sunscreen|sun balm|zinc sunscreen|mango sun)\b/.test(t)) return "sunscreen";
    if (/\bdeodorant\b/.test(t)) return "deodorant";
    if (/\bsoap\b/.test(t)) return "skincare";
    if (/\b(shampoo|conditioner|detangler|dry shampoo|hair (spray|serum|mist|repair)|hair and body mist|salt spray)\b/.test(t)) {
      return "hair";
    }
    return "skincare";
  }
  if (brand?.slug === "pure-food-company") {
    if (/\bcollagen\b/.test(t)) return "supplements";
    return "protein";
  }
  if (brand?.slug === "emilys-homestead") {
    if (/\bdeodorant\b/.test(t)) return "deodorant";
    if (/\b(shampoo|conditioner|hair)\b/.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "wildspore") return "supplements";
  if (brand?.slug === "tidalove") {
    if (/\b(facial|body wash|cleanser|serum)\b/.test(t)) return "skincare";
    return "oral";
  }
  if (brand?.slug === "enact-hydration") return "electrolytes";
  if (brand?.slug === "flora-fauna-skin") {
    if (/\b(sun balm|zinc|sunscreen|spf)\b/.test(t)) return "sunscreen";
    return "skincare";
  }
  if (brand?.slug === "oxford-hill") {
    if (/\bdeodorant\b/.test(t)) return "deodorant";
    return "hair";
  }
  if (brand?.slug === "mindful-crumb") return "protein";
  if (brand?.slug === "mmm-farms") return "supplements";
  if (brand?.slug === "mineral-daily") return "oral";
  if (brand?.slug === "aura-glow-tallow") {
    if (/\b(spf|sunscreen|mineral spf|sun balm)\b/.test(t)) return "sunscreen";
    if (/\bdeodorant\b/.test(t)) return "deodorant";
    if (/\bbeard\b/.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "ofp-farms") {
    if (/\bdeodorant\b/.test(t)) return "deodorant";
    if (/\b(shampoo|beard|hair)\b/.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "untamed-mother") {
    if (/\b(sun stick|sunscreen|spf|zinc)\b/.test(t)) return "sunscreen";
    return "skincare";
  }
  if (brand?.slug === "tallow-and-hive") return "skincare";
  if (brand?.slug === "keeko") return "oral";
  if (brand?.slug === "oovii") return "sunscreen";
  if (brand?.slug === "bison-protein") return "protein";
  if (brand?.slug === "massie-provisions") {
    if (/\b(sun balm|sunscreen|spf|zinc)\b/.test(t)) return "sunscreen";
    if (/\bdeodorant\b/.test(t)) return "deodorant";
    if (/\b(pomade|beard|t-style|hair)\b/.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "hemsworth-homestead") {
    if (/\b(after[- ]?sun|sunscreen|spf)\b/.test(t)) return "sunscreen";
    if (/\bdeodorant\b/.test(t)) return "deodorant";
    if (/\b(shampoo|conditioner|hair|dry shampoo|scalp|detangler)\b/.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "longevity-power") {
    if (/electrolyt|hydrat|coconut water|vanilla café|vanilla cafe/i.test(t) && !/\b(lion|reishi|mushroom|maca|liver|feast|adaptogen|goji|pearl|radiance|levity|glow)\b/.test(t)) {
      return "electrolytes";
    }
    return "supplements";
  }
  if (brand?.slug === "essee-farms") {
    if (/\b(shampoo|conditioner|hair)\b/.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "klyyr") return "oral";
  if (brand?.slug === "baili-essentials") {
    if (/\bdeodorant\b/.test(t)) return "deodorant";
    return "skincare";
  }
  if (brand?.slug === "without-compromise") {
    if (/\b(sun tallow|signature sun|mineral tallow|zinc)\b/.test(t)) return "sunscreen";
    if (/\b(shampoo|scalp|hair|3-in-1|3 in 1)\b/.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "i-lyte-you") return "electrolytes";
  if (brand?.slug === "fruiting-bodies") return "supplements";
  if (brand?.slug === "eezysun") return "sunscreen";
  if (brand?.slug === "arame") return "oral";
  if (brand?.slug === "elemental-deodorant") return "deodorant";
  if (brand?.slug === "superieur") return "electrolytes";
  if (brand?.slug === "caprifit") return "protein";
  if (brand?.slug === "white-sparrow") {
    if (/\b(hair oil|beard)\b/.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "ancient-suds") {
    if (/\bdeodorant\b/.test(t)) return "deodorant";
    if (/\b(shampoo|beard)\b/.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "wattz") return "sunscreen";
  if (brand?.slug === "dentafari") return "oral";
  if (brand?.slug === "something-nice") return "oral";
  if (brand?.slug === "aspen-kay") {
    if (/\b(shampoo|conditioner|hair)\b/.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "doms-deodorant") {
    if (/\b(crush|toothpaste|toothbrush|tongue|mouthwash|oral|floss|hydroxyapatite|spearmint|citrus spice|moringa|fennel)\b/.test(t)) {
      return "oral";
    }
    return "deodorant";
  }
  if (brand?.slug === "lone-star-mushrooms") return "supplements";
  ...
  if (/\b(tandpoeder|remineraliserend)\b/.test(t)) return "oral";
  if (/\b(zonnebalsem|sun barrier|field day)\b/.test(t)) return "sunscreen";
  if (/\b(goat whey|gotpwr)\b/.test(t)) return "protein";
  if (/\b(muddy waterz|daily dose mushroom)\b/.test(t)) return "supplements";
  if (brand?.slug === "plain-jane-tallow" && /\b(sunscreen|zinc|field day)\b/.test(t)) return "sunscreen";
  if (brand?.slug === "tandlers-tallow" && /\bsun balm\b/.test(t)) return "sunscreen";
  if (brand?.slug === "tandlers-tallow" && /\bdeodorant\b/.test(t)) return "deodorant";
  if (brand?.slug === "tandlers-tallow" && /\bshampoo\b/.test(t)) return "hair";
  if (brand?.slug === "tandlers-tallow" && /\b(tallow soap|liquid tallow|freedom bar)\b/.test(t)) return "skincare";
...
function isOnNiche(raw, brand) {
  ...
  if (brand.slug === "gudyu") {
    return /toothpaste tablets?/i.test(title);
  }
  if (brand.slug === "natrilyte") {
    return /electrolyte|natrilyte/i.test(`${title} ${type}`) && !WAVE67_EXTRA.test(title);
  }
  if (brand.slug === "rainwater-botanicals") {
    if (WAVE67_EXTRA.test(title) || /tinted\s*lip/i.test(title) || /\b(perfume|gift card|bug|concealer|eyeshadow|eye shadow|blush|foundation|cc beauty)\b/i.test(title)) {
      if (!/\b(spf|sunscreen|sun balm|zinc sunscreen)\b/i.test(title)) return false;
    }
    if (/\b(spf|sunscreen|sun balm|zinc sunscreen|mango sun)\b/i.test(title)) return true;
    if (/\b(deodorant|shampoo|conditioner|soap|balm|lotion|cream|serum|cleanser|toner|detangler|dry shampoo|salt spray|hair and body mist|after[- ]?shave|salve|foot|baby powder|body powder|shave soap|body oil|bath oil|body butter|facial|moisturizer|mask|eye cream|pimple|healing waters|cuticle)\b/i.test(`${title} ${type}`) && !/tinted\s*lip/i.test(title) && !/\b(perfume|concealer|eyeshadow|blush|foundation|bug|gift|primer)\b/i.test(title)) {
      return true;
    }
    return false;
  }
  if (brand.slug === "pure-food-company") {
    if (WAVE67_EXTRA.test(title) || /\bsample\b/i.test(title)) return false;
    return /\b(plant (based )?protein|collagen|meal replacement)\b/i.test(title);
  }
  if (brand.slug === "emilys-homestead") {
    if (WAVE67_EXTRA.test(title) || /\b(dog|wax melt|odds|clearance|gift|mystery|club|sample size|healthy pup)\b/i.test(title)) {
      return false;
    }
    if (/\b(deodorant|shampoo|soap|lip balm|conditioner|lotion|sugar scrub|shaving soap)\b/i.test(`${title} ${type}`)) {
      return true;
    }
    return false;
  }
  if (brand.slug === "wildspore") {
    if (WAVE67_EXTRA.test(title) || /\b(grow|spawn|culture|substrate|vermiculite|gypsum|bundle|whole dried|injection port)\b/i.test(title)) {
      return false;
    }
    return /\b(tincture|lion'?s mane mushroom powder|reishi mushroom powder)\b/i.test(title);
  }
  if (brand.slug === "tidalove") {
    if (WAVE66_EXTRA.test(title) || /\b(gift|4 months|holder|konjac|pearl|trio|tablet set|5% nhap|fluoride \+ nhap)\b/i.test(title)) return false;
    if (/\b(toothpaste tablets?|mouthwash|floss|toothbrush|facial (wash|beans)|body wash)\b/i.test(`${title} ${type}`) && !(/\btwin pack\b/i.test(title) && /\btoothbrush\b/i.test(title))) {
      return true;
    }
    return false;
  }
  if (brand.slug === "enact-hydration") {
    return /\belectrolyt|drink mix|unflavored|lemon bliss\b/i.test(`${title} ${type}`);
  }
  if (brand.slug === "flora-fauna-skin") {
    return /\b(sun balm|tallow glow|enzyme cleanser|hydrating mist|desert daze)\b/i.test(title);
  }
  if (brand.slug === "oxford-hill") {
    if (WAVE66_EXTRA.test(title) || /\b(gift|kit|perfume|panache|bundle)\b/i.test(title)) return false;
    if (/\b(deodorant|hair (paste|cream|clay)|pomade|sea salt spray|texture)\b/i.test(`${title} ${type}`)) {
      return true;
    }
    return false;
  }
  if (brand.slug === "mindful-crumb") {
    if (WAVE66_EXTRA.test(title) || /\b(ball|brioche|kit|gift|shilajit|surge|ritual)\b/i.test(title)) return false;
    return /\bwhey protein concentrate\b/i.test(title);
  }
  if (brand.slug === "mmm-farms") {
    if (WAVE66_EXTRA.test(title) || /\b(grow|dried|gift|kit|spore)\b/i.test(title)) return false;
    return /tincture|lion'?s mane powder/i.test(title);
  }
  if (brand.slug === "mineral-daily") {
    return /\b(remineralizing gum|chewing gum|mineral)\b/i.test(`${title} ${type}`) && !/\b(bundle builder|gift)\b/i.test(title);
  }
  if (brand.slug === "aura-glow-tallow") {
    if (WAVE66_EXTRA.test(title) || /\b(tallow tails|nose, paw|paw & dry|dog)\b/i.test(title)) return false;
    if (/\b(spf|sunscreen|lotion|deodorant|beard|soap|lip|eye corrector|vapor rub|whip)\b/i.test(`${title} ${type}`)) {
      return true;
    }
    return false;
  }
  if (brand.slug === "ofp-farms") {
    if (WAVE66_EXTRA.test(title) || /\b(bundle|gift box|gift set|smack pack|4-pack|soap sack|perfume|beard comb|wellness box|ritual kit|deluxe|sample)\b/i.test(title)) {
      return false;
    }
    if (/\b(soap|lotion|deodorant|shampoo|lip balm|goat kisses|beard (balm|oil|wash)|magnesium|face bar|salve|balm)\b/i.test(`${title} ${type}`) && !/\b(4 pack|smack|sack|comb|perfume|bundle|gift|kit)\b/i.test(title)) {
      return true;
    }
    return false;
  }
  if (brand.slug === "untamed-mother") {
    if (WAVE66_EXTRA.test(title) || /\b(bug|skeeter|hound|dish soap|gua sha|soap saver|bundle)\b/i.test(title)) {
      return false;
    }
    if (/\b(sun stick|lip balm|whipped tallow|everyday balm|goat milk lotion|drawing salve|tattoo|honey \+ hide|golden glow|breathe easy|mud \+ hide|sand \+ hide|hive \+ hide|lemon poppy|rosemary|naked bar|milk \+ honey|golden sunrise|oops bars|creamsicle)\b/i.test(`${title} ${type}`)) {
      return true;
    }
    return false;
  }
  if (brand.slug === "tallow-and-hive") {
    if (/\bmini\b/i.test(title)) return false;
    if (/\b(bar|balm|whip)\b/i.test(`${title} ${type}`)) return true;
    return false;
  }
  const wave66Drop = WAVE64_EXTRA.test(title);
  if (brand.slug === "keeko") {
    if (wave66Drop) return false;
    if (/\b(toothpaste|oil pull|toothbrush|tongue (cleaner|scraper)|floss|dream clean|super clean|morning mint)\b/i.test(`${title} ${type}`) && !/\b(set|bundle|pen)\b/i.test(title)) {
      return true;
    }
    return false;
  }
  if (brand.slug === "oovii") {
    return /\b(face first|spf|sunscreen)\b/i.test(`${title} ${type}`) && !wave66Drop;
  }
  if (brand.slug === "bison-protein") {
    return /\bprotein\b/i.test(title) && !wave66Drop && Number(raw.variants?.[0]?.price || 0) > 0;
  }
  if (brand.slug === "massie-provisions") {
    if (wave66Drop) return false;
    if (/\b(pitmaster|chef'?s tallow|raw honey|wax melt|candle|provisions set|bundle|set)\b/i.test(title)) return false;
    if (/\b(sun balm|deodorant|t-balm|t-soap|t-stick|t-salve|t-scrub|combat crème|combat creme|pomade|beard|home mist|hydrosol)\b/i.test(`${title} ${type}`)) {
      return true;
    }
    return false;
  }
  if (brand.slug === "hemsworth-homestead") {
    if (wave66Drop) return false;
    if (/\b(oil cleanser|makeup remover|nourish cleansing)\b/i.test(title) && !/\b(gift|bundle|kit)\b/i.test(title)) {
      return true;
    }
    if (/\b(tongue (cleaner|scraper)|after[- ]?sun|tallow|deodorant|shampoo|conditioner|hair (mist|balm|serum)|dry shampoo|lip balm|lotion|body butter|sugar scrub|bar soap|face (mask|serum|cream)|cleansing|eye cream|body oil|diaper balm|baby body|magnesium body|arnica|manuka salve)\b/i.test(`${title} ${type}`) && !/\b(gift|bundle|kit|tee|candle|syrup|room spray|tea|perfume|cologne|dog|gua sha|dish|sisal|tinted|custom|breast)\b/i.test(title)) {
      return true;
    }
    return false;
  }
  if (brand.slug === "longevity-power") {
    if (wave66Drop || /\b(stack|bundle|kit|gift|pack)\b/i.test(title)) return false;
    if (/\b(epic electrolytes|epic lion|epic reishi|epic mushrooms|epic maca|epic goji|epic beef|ancestral feast|pearl beauty|longevity adaptogens|levity|glow he shou|radiance beauty)\b/i.test(title)) {
      return true;
    }
    return false;
  }
  if (brand.slug === "essee-farms") {
    if (wave66Drop) return false;
    if (/\b(shampoo bar|lotion|lotion bar|bar soap|beauty bar|goat milk soap|anti-aging)\b/i.test(`${title} ${type}`) && !/\b(wax melt|diffuser|essential oil|rack|sprinkle|gift|misc|in store|sample size|with soap rack)\b/i.test(title)) {
      return true;
    }
    return false;
  }
  const skipped =
    ...
    WAVE64_EXTRA.test(title) ||
    WAVE63_EXTRA.test(title) ||
    WAVE61_EXTRA.test(title) ||
    WAVE60_EXTRA.test(title) ||
    ...
    SKIP_TYPE.test(type) ||
    SKIP_TAGS.test(tags);
  if (WAVE64_EXTRA.test(title) || WAVE63_EXTRA.test(title) || WAVE61_EXTRA.test(title) || WAVE60_EXTRA.test(title) || WAVE59_EXTRA.test(title) || WAVE58_EXTRA.test(title) || WAVE57_EXTRA.test(title) || WAVE56_EXTRA.test(title) || WAVE54_SKIP.test(title) || WAVE54_EXTRA.test(title)) return false;
  if (brand.slug === "klyyr" && /one\+|toothpaste|hydroxyapatite/i.test(`${title} ${type}`) && !WAVE63_EXTRA.test(title)) {
    return true;
  }
  if (
    brand.slug === "baili-essentials" &&
    /\b(deodorant|lotion bar|lip balm)\b/i.test(title) &&
    !WAVE63_EXTRA.test(title) &&
    !/\b(gift|pulse point)\b/i.test(title)
  ) {
    return true;
  }
  if (
    brand.slug === "without-compromise" &&
    /\b(tallow|shampoo|scalp|lip balm|unscented lip|scrub bar|calm waters|sun tallow|signature sun|hand & foot|hand and foot)\b/i.test(title) &&
    !WAVE63_EXTRA.test(title) &&
    !/\b(gift|bundle|protocol|makeup|cheek|sample|4 pack|certificate|tinted lip|nude lip|pink lip|bronze lip)\b/i.test(title)
  ) {
    return true;
  }
  if (brand.slug === "i-lyte-you" && /\b(hydrat|electrolyt|sticks?)\b/i.test(title) && !WAVE63_EXTRA.test(title)) {
    return true;
  }
  if (
    brand.slug === "fruiting-bodies" &&
    /\b(lion'?s mane|reishi|cordyceps|chaga|turkey tail|synergy|gummies)\b/i.test(title) &&
    !WAVE63_EXTRA.test(title) &&
    !/\b(chocolate|art print|essence|book|trio|bundle)\b/i.test(title)
  ) {
    return true;
  }
  if (
  ...
  if (
    brand.slug === "eezysun" &&
    /\b(spf|sunscreen|aftersun|after-sun|bare tinted)\b/i.test(title) &&
    !WAVE63_EXTRA.test(title) &&
    !/\b(insect|gift)\b/i.test(title)
  ) {
    return true;
  }
  if (brand.slug === "arame" && /\b(the paste|the polish)\b/i.test(title) && !WAVE63_EXTRA.test(title)) {
    return true;
  }
  if (
    brand.slug === "elemental-deodorant" &&
    /\b(nur|air|refill pouch|fragrance-free|all-natural deodorant)\b/i.test(title) &&
    !WAVE63_EXTRA.test(title) &&
    !/\b(power pack|gift pack|refill pack|starter pack)\b/i.test(title)
  ) {
    return true;
  }
  if (
    brand.slug === "superieur" &&
    /\b(canister|packets?|sunrise|raspberry|grape|lemonade|lemon|blueberry|watermelon|citrus)\b/i.test(title) &&
    !WAVE63_EXTRA.test(title)
  ) {
    return true;
  }
  if (
    brand.slug === "caprifit" &&
    /\bgoat whey protein\b/i.test(title) &&
    !/\b(oats|wafer|bar)\b/i.test(title) &&
    !WAVE63_EXTRA.test(title)
  ) {
    return true;
  }
  if (
    brand.slug === "white-sparrow" &&
    /\b(tallow balm|tallow lip|tallow soap|tallow hair|tallow cold cream|liberty balm)\b/i.test(title) &&
    !WAVE63_EXTRA.test(title) &&
    !/\b(nightgown|face tape|culinary|3 pack|trio|heirloom)\b/i.test(title) &&
    !/^(tallow balm|tallow lip balm)$/i.test(title)
  ) {
    return true;
  }
  if (
    brand.slug === "ancient-suds" &&
    /\b(tallow|deodorant|shampoo|soap|cream & coal|terrazzo)\b/i.test(title) &&
    !WAVE63_EXTRA.test(title) &&
    !/\b(kit|sample box|mesh|pouch)\b/i.test(title)
  ) {
    return true;
  }
  if (
  ...
  if (brand.slug === "sea-summit" && /\b(spf|sunscreen|sun barrier)\b/i.test(title) && !WAVE61_EXTRA.test(title)) {
    return true;
  }
  if (
    brand.slug === "mastermedi" &&
    /\b(toothpaste|tablets?|tongue scraper|hydroxyapatite|n[- ]?ha)\b/i.test(title) &&
    !WAVE61_EXTRA.test(title) &&
    !/\b(whitening strips|electric|vibrosonic|water flosser|instruments kit|eugenol|dental care kit)\b/i.test(title)
  ) {
    return true;
  }
  if (
    brand.slug === "tandlers-tallow" &&
    /\b(whipped tallow|tallow body balm|sun balm|tallow deodorant|tallow shampoo|tallow soap|liquid tallow|freedom bar)\b/i.test(title) &&
    !WAVE61_EXTRA.test(title) &&
    !/\b(bundle|five pack|kit|tee|candle|wraps|dish|set \()\b/i.test(title)
  ) {
    return true;
  }
  if (brand.slug === "gotpwr" && /\b(whey|protein)\b/i.test(title) && !/\bsubscription\b/i.test(title)) {
    return true;
  }
  if (
    brand.slug === "plain-jane-tallow" &&
    /\b(tallow balm|soap bar|sunscreen|sun protection|fat kiss|lip balm)\b/i.test(title) &&
    !WAVE61_EXTRA.test(title) &&
    !/\b(bundle|farmer'?s lot|dog)\b/i.test(title)
  ) {
    return true;
  }
  if (
    brand.slug === "primal-glow" &&
    /\b(tallow|zonnebalsem|tandpoeder|zeep|lippenbalsem|aleppo|zink)\b/i.test(title) &&
    !WAVE61_EXTRA.test(title) &&
    !/\b(handdoek|zeepbakje|badzout|paardenhaar|komijn)\b/i.test(title)
  ) {
    return true;
  }
  if (
    brand.slug === "arizona-mushroom" &&
    /\b(tincture|muddy waterz|daily dose|chaga powder|lion'?s mane|mushroom coffee|mushroom blend)\b/i.test(title) &&
    !WAVE61_EXTRA.test(title) &&
    !/\b(umami|sun dried|seasoning)\b/i.test(title)
  ) {
    return true;
  }
  if (
    brand.slug === "sea-summit" ||
    brand.slug === "mastermedi" ||
    brand.slug === "tandlers-tallow" ||
    brand.slug === "gotpwr" ||
    brand.slug === "plain-jane-tallow" ||
  ...
  if (brand.slug === "wattz" && /\b(spf|sunscreen|wattz)\b/i.test(title)) return true;
  if (
    brand.slug === "dentafari" &&
    /\b(zahnpulver|hydroxylapatit|hydroxyapatite)\b/i.test(title) &&
    !WAVE60_EXTRA.test(title) &&
    !/\b(set|bundle|tiegel|4er)\b/i.test(title)
  ) {
    return true;
  }
  if (
    brand.slug === "something-nice" &&
    /\b(zero pro|zero toothpaste|halo|nanofloss|versa floss|braces floss)\b/i.test(title) &&
    !WAVE60_EXTRA.test(title) &&
    !/\b(kit|aquaclean|charger|nozzle|lumismart|3 pack)\b/i.test(title)
  ) {
    return true;
  }
  if (
    brand.slug === "aspen-kay" &&
    /\b(bar soap|tallow balm|glow face oil|face & body moisturizer|shampoo bar|conditioner bar|soap)\b/i.test(title) &&
    !WAVE60_EXTRA.test(title) &&
    !/\b(brush|comb|dish|sisal|bundle|collection|gift card|aromatherapy|set with|fugly|combo|variety set|subscription)\b/i.test(title) &&
    !/\|/.test(title)
  ) {
    return true;
  }
  if (
    brand.slug === "doms-deodorant" &&
    /\b(deodorant|crush & brush|crush & swish|tongue scraper|toothbrush|toothpaste|floss|hydroxyapatite|spearmint|citrus spice|moringa|fennel)\b/i.test(title) &&
    !WAVE60_EXTRA.test(title) &&
    !/\b(wholesale|bulk|raven roast|sample|5 pack|4 pack|duo|application tool|imperfect|full case|tablet refill|best sellers|hydroxyapatite set)\b/i.test(title)
  ) {
    return true;
  }
  if (
    brand.slug === "lone-star-mushrooms" &&
    /\b(lion'?s mane|reishi|cordyceps|chaga|turkey tail|rise mushroom|rest mushroom|mushroom coffee|mushroom extract|mushroom complex)\b/i.test(
      title,
    ) &&
    !WAVE60_EXTRA.test(title) &&
    !/\b(bundle|dried)\b/i.test(title)
  ) {
    return true;
  }
  if (
...
function flavorOptionIndex(raw, brand) {
  ...
  if (brand?.slug === "white-sparrow") {
    ...
    if (scent >= 0 && /soap collection/i.test(raw.title || "")) return scent;
    return -1;
  }
  if (brand?.slug === "mill-creek-tallow") {
  ...
  const scored = options.map((o, i) => {
    ...
    let score = 1;
    if (/flavor|flavour|scent|fragrance|smell|aroma|geur|smaak|colour|color|shade|tint/i.test(name)) score = 3;
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel) {
  ...
  if (brand.slug === "gudyu") {
    ...
    if (/childrens-strawberry/.test(handle)) {
      title = /tablets-1/.test(handle)
        ? "Children's Strawberry Toothpaste Tablets — Starter"
        : "Children's Strawberry Toothpaste Tablets";
    } else if (/remineralising|remineralizing/.test(handle)) {
      title = /tablets-1/.test(handle)
        ? "Remineralising and Whitening Toothpaste Tablets — Starter"
        : "Remineralising and Whitening Toothpaste Tablets";
    }
  }
  if (brand.slug === "caprifit") {
    const handle = raw.handle || "";
    if (/vanilla-goat/.test(handle)) title = "Goat Whey Protein — Vanilla";
    else if (/dark-chocolate/.test(handle)) title = "Goat Whey Protein — Dark Chocolate";
    else if (/cookies-cream/.test(handle)) title = "Goat Whey Protein — Cookies & Cream";
  }
  title = title
    .replace(/&amp;/g, "&")
    .replace(/&nbsp;/g, " ")
...
function longevityFlavorKey(label) {
  const s = String(label || "");
  if (/naked coconut/i.test(s)) return "Naked Coconut Water";
  if (/vanilla caf[eé]/i.test(s)) return "Vanilla Café Ritual";
  if (/pure carnivore/i.test(s)) return "Pure Carnivore";
  if (/garlic/i.test(s)) return "Garlic & Herb";
  if (/tex-mex|zesty/i.test(s)) return "Zesty Tex-Mex";
  return null;
}
...
function expandProducts(raw, brand, brandId, index) {
  ...
  if (brand.slug === "longevity-power") {
    ...
    for (const v of variants) {
      const key = longevityFlavorKey(v.option1 || v.title);
      if (!key || seen.has(key)) continue;
      if (!Number(v.price) || Number(v.price) <= 0) continue;
      seen.add(key);
      const mapped = mapOne(raw, brand, brandId, index, v, key);
  ...
  const skipFlavorExpand =
    brand.slug === "gudyu" ||
    brand.slug === "rainwater-botanicals" ||
    brand.slug === "pure-food-company" ||
    brand.slug === "wildspore" ||
    (brand.slug === "emilys-homestead" && /lotion bar/i.test(raw.title || "")) ||
    brand.slug === "tidalove" ||
    brand.slug === "enact-hydration" ||
    brand.slug === "flora-fauna-skin" ||
    brand.slug === "oxford-hill" ||
    brand.slug === "mindful-crumb" ||
    brand.slug === "mmm-farms" ||
    brand.slug === "mineral-daily" ||
    brand.slug === "aura-glow-tallow" ||
    brand.slug === "tallow-and-hive" ||
    brand.slug === "keeko" ||
    brand.slug === "oovii" ||
    brand.slug === "bison-protein" ||
    brand.slug === "essee-farms" ||
    brand.slug === "klyyr" ||
    brand.slug === "without-compromise" ||
    brand.slug === "fruiting-bodies" ||
    brand.slug === "eezysun" ||
    ...
    brand.slug === "caprifit" ||
    brand.slug === "ancient-suds" ||
    (brand.slug === "white-sparrow" && !/soap collection/i.test(raw.title || "")) ||
    brand.slug === "wattz" ||
    brand.slug === "dentafari" ||
    (brand.slug === "something-nice" && !/nanofloss/i.test(raw.title || "")) ||
    brand.slug === "aspen-kay" ||
    brand.slug === "doms-deodorant" ||
    brand.slug === "lone-star-mushrooms" ||
    brand.slug === "elektra" ||
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave67.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave67Products: waveCount,
        wave67BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave67-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave67=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
