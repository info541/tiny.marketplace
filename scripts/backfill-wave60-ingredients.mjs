/**
 * Drop leftover merch from wave60 and backfill missing ingredients from product pages.
 */
...
const WAVE_START = 1229;
const WAVE_END = 1234;
// 6 brands: c1229–c1234
...
async function main() {
  ...
  for (const p of cleaned.filter(isWave26)) {
    ...
    const nId = Number(String(p.brandId).replace(/^c/, ""));
    if (nId >= 1229 && nId <= 1234 && p.ingredients?.length) {
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave60Products: wave.length,
    wave60WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave60-ingredients-backfill.json"),
