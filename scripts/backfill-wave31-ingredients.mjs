/**
 * Drop leftover merch from wave31 and backfill missing ingredients from product pages.
 */
...
function reclassify(p) {
  ...
  if (/\b(vegan vanilla protein|protein shake|protein powder)\b/.test(t)) p.category = "protein";
  if (/\b(mineral zinc sunscreen|mineral sunstick|sun balm|daydream mineral sunscreen)\b/.test(t)) {
...
async function main() {
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave31Products: wave.length,
    wave31WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave31-ingredients-backfill.json"),
