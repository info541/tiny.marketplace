/**
 * Fetch Shopify catalogs for wave29 brands (c937+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave29.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave29.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave29Products: waveCount,
        wave29BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave29-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave29=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
