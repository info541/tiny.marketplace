/**
 * Fetch Shopify catalogs for wave39 brands (c1060+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave39.mjs";
...
const FLAVOR_OPTION = /flavor|flavour|scent|fragrance|smell|aroma|colour|color|shade|scented|finish|constitutive|herb|fluord|fluoride|the taste|dry shampoo colour|style|tint|formula|whip|blend|infusi[oó]n/i;

/** Convert shop-native prices that would otherwise look like USD luxury SKUs. */
const FX_TO_USD = {
  "aus-natural-protein": 0.65,
  "boeuf-tallow": 0.73,
  "freo-living": 0.73,
  "tallow-and-me": 0.00105,
};
...
function expandProducts(raw, brand, brandId, index) {
  ...
  if (flavorIdx >= 0 && variants.length > 1) {
    ...
    for (const v of variants) {
      ...
      if (/\b(variety pack|staple flavors?|limited flavors?|mix of|mix vanilla|duo\b)\b/i.test(label)) continue;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave39.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave39Products: waveCount,
        wave39BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave39-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave39=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
