/**
 * Write src/lib/catalog-brands-wave45.ts from the wave45 seed.
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { brands } from "./catalog-brands-seed-wave45.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const out = path.join(__dirname, "../src/lib/catalog-brands-wave45.ts");
...
const rows = brands.map((b, i) => {
  const id = `c${String(i + 1129).padStart(4, "0")}`;
  return `  {
    id: ${tsString(id)},
