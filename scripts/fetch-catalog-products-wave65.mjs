/**
 * Fetch Shopify catalogs for wave65 brands (c1261+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave65.mjs";
...
const WAVE65_EXTRA =
  /\b(bundle|gift (box|set|card)|smack pack|4-pack|soap sack|soap saver|solid (all natural )?perfume|wooden beard comb|wellness box|ritual kit|deluxe|dog paw|tallow tails|skeeter|bug (spray|stick)|dish soap|gua sha|crocheted|drawing salve bundle|day \+ night|lake day|mini sun|whip mini|balm mini)\b/i;
...
function isOnNiche(raw, brand) {
  ...
  if (brand.slug === "aura-glow-tallow") {
    if (WAVE65_EXTRA.test(title) || /\b(tallow tails|nose, paw|paw & dry|dog)\b/i.test(title)) return false;
  ...
  if (brand.slug === "ofp-farms") {
    if (WAVE65_EXTRA.test(title) || /\b(bundle|gift box|gift set|smack pack|4-pack|soap sack|perfume|beard comb|wellness box|ritual kit|deluxe|sample)\b/i.test(title)) {
      return false;
    }
  ...
  if (brand.slug === "untamed-mother") {
    if (WAVE65_EXTRA.test(title) || /\b(bug|skeeter|hound|dish soap|gua sha|soap saver|bundle)\b/i.test(title)) {
      return false;
    }
  ...
  const wave65Drop = WAVE64_EXTRA.test(title);
  if (brand.slug === "keeko") {
    if (wave65Drop) return false;
  ...
  if (brand.slug === "oovii") {
    return /\b(face first|spf|sunscreen)\b/i.test(`${title} ${type}`) && !wave65Drop;
  }
  if (brand.slug === "bison-protein") {
    return /\bprotein\b/i.test(title) && !wave65Drop && Number(raw.variants?.[0]?.price || 0) > 0;
  }
  if (brand.slug === "massie-provisions") {
    if (wave65Drop) return false;
  ...
  if (brand.slug === "longevity-power") {
    if (wave65Drop || /\b(stack|bundle|kit|gift|pack)\b/i.test(title)) return false;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave65.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave65Products: waveCount,
        wave65BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave65-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave65=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
