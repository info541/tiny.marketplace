/**
 * Drop leftover merch from wave58 and backfill missing ingredients from product pages.
 */
...
const WAVE_START = 1218;
const WAVE_END = 1222;
// 5 brands: c1218–c1222
...
async function main() {
  ...
  const cleaned = all
    .filter((p) => {
      ...
      const keepWave58 =
        nId >= 1218 && nId <= 1222 &&
        (
          (nId === 1218 && /\b(toothpaste|mouthwash|mint|chai|kids|berry)\b/i.test(p.name) && !EXTRA_DROP_WAVE58.test(p.name)) ||
          (nId === 1219 && /\b(whey|protein)\b/i.test(p.name) && !/\bsubscription\b/i.test(p.name)) ||
          (nId === 1220 && /\b(intro|daily|servings|electrolyt|lemon)\b/i.test(p.name)) ||
          (nId === 1221 && /\b(electrolyte|hydrat|mushroom coffee)\b/i.test(p.name) && !EXTRA_DROP_WAVE58.test(p.name)) ||
          (nId === 1222 && /\b(deodorant|shampoo|conditioner|serum|soap|lotion|lip balm|cleanser|mask|cream|wash|scrub|oil|facial)\b/i.test(p.name) && !EXTRA_DROP_WAVE58.test(p.name) && !/\b(bug|tray|gloss|spicule)\b/i.test(p.name))
        );
      if (keepWave58) return !EXTRA_DROP_WAVE58.test(p.name);
      const keepWave57 =
        nId >= 1212 && nId <= 1217 &&
        (
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave58Products: wave.length,
    wave58WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave58-ingredients-backfill.json"),
