/**
 * Fetch Shopify catalogs for wave6 brands (c501+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave6.mjs";
...
const WAVE_START = 501;
const WAVE_END = 501 + seedBrands.length - 1;

const SKIP_TITLE =
  /\b(gift card|e[- ]?gift|store credit|membership|subscription box|not for sale|tester only|wholesale|100%\s*off|do not use|tiktok shop|credit card payment|price test|employee wellness|shaker bottle|water bottle|empty bottle|pump holder|marketing card|fanny pack|luggage tag|t-?shirt|tee\b|hoodie|sweatshirt|\bhat\b|ball cap|beanie|tote bag|statement tote|silk scarf|sticker|mug\b|apparel|poster|patch|keychain|stuffed toy|plush|crew socks|socks\b|head wrap|book\b|journal\b|workshop|masterclass|reading|tarot|ticket|event|class with|immersion|social hour|astrology|gua sha class|oracle|akashic|initiation:|pocket altar|free class|free gift|enamel pin|deodorant scoop|cream applicator|aluminum bottle|plastic bottle|blue bottle|brown bottle|clear bottle|clear jar|roll-on bottle|for pets?|daily dawg|there's a mushroom)\b/i;
...
const NICHE =
  /\b(spf\s*\d+|sunscreen|sun\s?screen|sunblock|sun butter|sun protection|deodor|antiperspir|underarm|\bdeo\b|toothpaste|toothbrush|floss|mouthwash|oral|hydroxyapatite|oil pull|tongue clean|shampoo|conditioner|hair (oil|mask|serum|care|cream)|scalp|leave[- ]?in|protein|whey|casein|collagen|pea protein|chocho|electrolyte|hydration|daily (calm|energy|hydration)|vitamin|supplement|capsule|softgel|probiotic|adaptogen|mushroom|tonic|powder|serum|moisturizer|moisturiser|cleanser|face wash|facial|toner|retinol|niacinamide|cream|lotion|oil|mask|balm|mist|essence|exfoliant|lip|body (lotion|butter|wash|cream|oil|bar)|soap)\b/i;
...
function inferCategory(brandCategories, title, productType, tags) {
  ...
  const rules = [
    ...
    ["oral", /\b(toothpaste|toothbrush|floss|mouthwash|oral|hydroxyapatite|oil pull|tongue clean|teeth)\b/],
    ["hair", /\b(shampoo|conditioner|hair|scalp|curl|leave[- ]?in|edge control|twist)\b/],
    ["protein", /\b(protein|whey|casein|pea protein|plant protein|chocho)\b/],
    ["electrolytes", /\b(electrolyte|hydration (powder|packet|mix|stick|multiplier)|rapid hydration)\b/],
    ["supplements", /\b(vitamin|supplement|capsule|softgel|probiotic|adaptogen|mushroom|tonic herb|collagen|prenatal|greens?)\b/],
    ["skincare", /\b(serum|moisturizer|moisturiser|cleanser|cream|toner|mask|facial|skin|body (lotion|butter|wash|cream|oil|bar)|lip|soap|balm|oil|mist)\b/],
  ];
...
async function main() {
  ...
  for (let i = 0; i < seedBrands.length; i++) {
    ...
    try {
      ...
      console.log(`kept ${mapped.length}/${rawProducts.length} ingredients ${withIng}`);
      perBrand.push({
        slug: brand.slug,
        brandId,
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave6.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave6Products: waveCount,
        wave6BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  console.log(
    `\nDone total=${all.length} wave6=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
