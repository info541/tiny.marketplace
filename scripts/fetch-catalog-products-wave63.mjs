/**
 * Fetch Shopify catalogs for wave63 brands (c1249+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave63.mjs";
...
function isOnNiche(raw, brand) {
  ...
  const skipped =
    ...
    SKIP_TYPE.test(type) ||
    SKIP_TAGS.test(tags);
  if (WAVE63_EXTRA.test(title) || WAVE61_EXTRA.test(title) || WAVE60_EXTRA.test(title) || WAVE59_EXTRA.test(title) || WAVE58_EXTRA.test(title) || WAVE57_EXTRA.test(title) || WAVE56_EXTRA.test(title) || WAVE54_SKIP.test(title) || WAVE54_EXTRA.test(title)) return false;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave63.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave63Products: waveCount,
        wave63BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave63-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave63=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
