/**
 * Drop leftover merch from wave38 and backfill missing ingredients from product pages.
 */
...
async function main() {
  ...
  const cleaned = all
    .filter((p) => {
      ...
      return !(MERCH.test(p.name) || EXTRA_DROP.test(p.name) || EXTRA_DROP_WAVE38.test(p.name));
    })
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave38Products: wave.length,
    wave38WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave38-ingredients-backfill.json"),
