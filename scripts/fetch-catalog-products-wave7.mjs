/**
 * Fetch Shopify catalogs for wave7 brands (c521+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave7.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave7.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave7Products: waveCount,
        wave7BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  console.log(
    `\nDone total=${all.length} wave7=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
