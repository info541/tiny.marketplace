/**
 * Fetch Shopify catalogs for wave44 brands (c1118+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave44.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave44.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave44Products: waveCount,
        wave44BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave44-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave44=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
