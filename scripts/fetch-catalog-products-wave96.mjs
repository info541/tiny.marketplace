/**
 * Fetch Shopify catalogs for wave96 brands (c1445+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave96.mjs";
...
function refineCategory(category, title, brand) {
  ...
  if (brand?.slug === "gutology") return "oral";
  if (brand?.slug === "plan-d") return "electrolytes";
  if (brand?.slug === "forall-nutrition") return "protein";
  if (brand?.slug === "maui-mana-skincare") {
    if (/\bspf\b|sunscreen|sun soothe|after[- ]?sun/.test(t)) return "sunscreen";
    return "skincare";
  }
  if (brand?.slug === "cowlo") {
    if (/deodorants?/.test(t)) return "deodorant";
    if (/beard/.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "noble-mushrooms") return "supplements";
  return category;
}
...
function isOnNiche(raw, brand) {
  ...
  if (SKIP_TITLE.test(title) || WAVE96_EXTRA.test(title)) return false;
  if (Number((raw.variants || [])[0]?.price || 0) <= 0) return false;
  if (brand.slug === "gutology") {
    if (
      /\b(2[- ]?pack|3[- ]?pack|starter|family|bundle|kit)\b/i.test(title) ||
      /kids-tp$|kids-toothpaste$|toothpaste-120$|floss$|tongue-scraper$/.test(handle)
    ) {
      return false;
    }
    return /toothpaste-120-us|kids-tp-us|floss-us|tongue-scraper-us/.test(handle);
  }
  if (brand.slug === "plan-d") {
    return /hydration-sticks-strawberry-lemonade/.test(handle);
  }
  if (brand.slug === "forall-nutrition") {
    return /european-grass-fed-whey-protein-isolate/.test(handle);
  }
  if (brand.slug === "maui-mana-skincare") {
    return /\b(sunscreen|spf|sun soothe|after[- ]?sun|lip balm|lotion bar|salt scrub|go-balm|anti-chafing)\b/i.test(
      hay,
    );
  }
  if (brand.slug === "cowlo") {
    if (/\b(bug|insect|gift|bundle|3 soaps)\b/i.test(title) || /bug-balm|gift/.test(handle)) {
      return false;
    }
    return /\b(deodorant|whipped|lip balm|soap|beard|makeup remover|baby balm)\b/i.test(hay);
  }
  if (brand.slug === "noble-mushrooms") {
    if (
      /\b(gift|class|workshop|csa|kit|spawn|culture|trio|ready-to-fruit|grow)\b/i.test(title) ||
      /gift|class|workshop|csa|kit|spawn|culture|trio|ready-to-fruit|grow-/.test(handle)
    ) {
      return false;
    }
    return /\b(tincture|dried mushrooms)\b/i.test(hay);
  }
...
function flavorOptionIndex(raw, brand) {
  ...
  const handle = raw.handle || "";
  if (brand?.slug === "gutology") return -1;
  if (brand?.slug === "plan-d") return -1;
  if (brand?.slug === "forall-nutrition") return -1;
  if (brand?.slug === "maui-mana-skincare") return -1;
  if (brand?.slug === "cowlo") return -1;
  if (brand?.slug === "noble-mushrooms") {
    if (/dried mushrooms|functional-mushroom-powders/i.test(handle)) {
      return options.findIndex((o) => /mushroom/i.test(o.name || ""));
    }
    return -1;
  }
...
function pickVariant(variants, brand, raw) {
  ...
  const scored = variants.map((v, i) => {
    ...
    if (/\b(two-pack|2-pack|three-pack|3-pack|four-pack|4-pack|2 boxes|4 boxes|6 boxes|0\.5oz|0\.5 oz|20 servings|3x90|250 g|250g|500 g|500g|1 kg|1kg|5g|sample size|refill only|2 jars|3 jars|5 jars|2 bars|3 bars|8 oz extra|16 oz|5lb|5 lb|family size|81 servings|15 servings|30 ct|3\.3 oz|refill cartridge)\b/i.test(hay))
      score -= 8;
    if (brand?.slug === "gutology") {
      if (/\b1 pack\b/i.test(hay)) score += 14;
    }
    if (brand?.slug === "plan-d") {
      if (/10 sticks/i.test(hay)) score += 14;
      if (/sample|3 stick/i.test(hay)) score -= 12;
    }
    if (brand?.slug === "forall-nutrition") {
      if (/1 pack|20 scoop/i.test(hay)) score += 14;
      if (/2 pack|3 pack|2x|3x/i.test(hay)) score -= 12;
    }
    if (brand?.slug === "noble-mushrooms") {
      if (/0\.5 oz|0.5 oz/i.test(hay)) score += 8;
    }
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel, currency) {
  let baseTitle = (raw.title || "")
    ...
    .trim();
  if (brand.slug === "gutology") {
    if (/kids/i.test(baseTitle)) baseTitle = "Gutology Kids Hydroxyapatite Toothpaste";
    else if (/toothpaste/i.test(baseTitle)) baseTitle = "Gutology Hydroxyapatite Toothpaste";
    else if (/floss/i.test(baseTitle)) baseTitle = "Gutology Bamboo Charcoal Dental Floss";
    else if (/tongue/i.test(baseTitle)) baseTitle = "Gutology Copper Tongue Scraper";
    else if (!/^gutology\b/i.test(baseTitle)) baseTitle = `Gutology ${baseTitle}`;
  }
  if (brand.slug === "plan-d") {
    baseTitle = "Plan D Electrolyte Drink Mix — Strawberry Lemonade";
  }
  if (brand.slug === "forall-nutrition") {
    baseTitle = "FORALL European Grass-Fed Whey Protein Isolate";
  }
  if (brand.slug === "maui-mana-skincare") {
    if (!/^maui mana/i.test(baseTitle)) baseTitle = `Maui Mana ${baseTitle}`;
  }
  if (brand.slug === "cowlo") {
    if (!/^cowlo\b/i.test(baseTitle)) baseTitle = `COWLO ${baseTitle}`;
  }
  if (brand.slug === "noble-mushrooms") {
    if (/dried mushrooms/i.test(baseTitle)) baseTitle = "Noble Mushrooms Dried Fruiting Body";
    else if (!/^noble/i.test(baseTitle)) baseTitle = `Noble ${baseTitle}`;
  }
  ...
  if (brand.slug === "gutology" || brand.slug === "plan-d" || brand.slug === "maui-mana-skincare") {
    ingredients = [];
  }
  if (brand.slug === "forall-nutrition") {
    ingredients = ["European Grass-Fed Whey Protein Isolate"];
  }
  if (brand.slug === "cowlo") {
    if (/deodorant/i.test(title)) {
      const scent = [];
      if (/lavender/i.test(title)) scent.push("Lavandula Angustifolia (Lavender) Oil");
      else if (/vanilla mint|sweet vanilla/i.test(title)) {
        scent.push("Vanilla Planifolia Oil", "Mentha Piperita (Peppermint) Oil");
      } else if (/cedarwood/i.test(title)) {
        scent.push("Cedrus Atlantica (Cedarwood) Oil", "Mentha Piperita (Peppermint) Oil");
      }
      ingredients = [
    ...
    } else if (/pure unscented|unscented \+ jojoba|jojoba oil whipped/i.test(title)) {
      ingredients = ["Grass-fed and finished suet tallow", "Organic Simmondsia Chinensis (Jojoba) Oil"];
    } else if (/baby balm/i.test(title)) {
      ingredients = [
        "Grass-fed and finished suet tallow",
        "Calendula-infused organic Olea Europaea (Olive) Oil",
      ];
    } else if (/lavender whipped/i.test(title)) {
      ingredients = [
        "Grass-fed and finished suet tallow",
        "Organic extra virgin Olea Europaea (Olive) Oil",
        "Organic Lavandula Angustifolia (Lavender) Oil",
      ];
    } else if (/cedarwood vanilla whipped/i.test(title)) {
      ingredients = [
        "Grass-fed and finished suet tallow",
        "Organic extra virgin Olea Europaea (Olive) Oil",
        "Organic Cedrus Atlantica (Cedarwood) Oil",
        "Organic Vanilla Planifolia Oil",
      ];
    } else if (/orange vanilla whipped/i.test(title)) {
      ingredients = [
        "Grass-fed and finished beef tallow",
        "Organic extra virgin Olea Europaea (Olive) Oil",
        "Organic Citrus Sinensis (Orange) Oil",
        "Organic Vanilla Planifolia Oil",
      ];
    } else if (/vanilla latte whipped/i.test(title)) {
      ingredients = [
        "Grass-fed and finished beef tallow",
        "Organic coffee- and vanilla-infused Simmondsia Chinensis (Jojoba) Oil",
      ];
    } else if (/herb-infused/i.test(title)) {
      ingredients = [
        "Grass-fed and finished suet tallow",
        "Calendula, jasmine green tea, and frankincense infused organic extra virgin Olea Europaea (Olive) Oil",
      ];
    } else if (/lip balm/i.test(title)) {
      const scent = [];
      if (/vanilla/i.test(title) && /orange/i.test(title)) {
        scent.push("Citrus Sinensis (Orange) Oil", "Vanilla Planifolia Oil");
      } else if (/vanilla/i.test(title)) scent.push("Vanilla Planifolia Oil");
      else if (/peppermint/i.test(title)) scent.push("Mentha Piperita (Peppermint) Oil");
      ingredients = [
  ...
  if (brand.slug === "noble-mushrooms") {
    if (/dried/i.test(title)) {
      const mushroom = flavorClean || title;
      let bodyName = "USDA Certified Organic fruiting body";
      if (/lion/i.test(mushroom)) bodyName = "USDA Certified Organic Hericium erinaceus (lion's mane) fruiting body";
      else if (/albino/i.test(mushroom)) bodyName = "USDA Certified Organic albino Cordyceps militaris fruiting body";
      else if (/cordyceps/i.test(mushroom)) bodyName = "USDA Certified Organic Cordyceps militaris fruiting body";
      else if (/reishi/i.test(mushroom)) bodyName = "USDA Certified Organic Ganoderma lucidum (reishi) fruiting body";
      ingredients = [bodyName];
    } else if (/tincture/i.test(title)) {
      let bodyName = "USDA Certified Organic fruiting body";
      if (/lion/i.test(title)) bodyName = "USDA Certified Organic Hericium erinaceus (lion's mane) fruiting body";
      else if (/reishi/i.test(title)) bodyName = "USDA Certified Organic Ganoderma lucidum (reishi) fruiting body";
      else if (/cordyceps/i.test(title)) bodyName = "USDA Certified Organic Cordyceps militaris fruiting body";
      else if (/turkey/i.test(title)) bodyName = "USDA Certified Organic Trametes versicolor (turkey tail) fruiting body";
      else if (/shiitake/i.test(title)) bodyName = "USDA Certified Organic Lentinula edodes (shiitake) fruiting body";
      else if (/oyster/i.test(title)) bodyName = "USDA Certified Organic Pleurotus ostreatus (blue oyster) fruiting body";
      ingredients = [bodyName, "USDA Certified Organic grain alcohol", "Water"];
    } else {
      ingredients = [];
    }
  }
...
function expandProducts(raw, brand, brandId, index, currency) {
  ...
  if (flavorIdx >= 0 && variants.length > 1) {
    ...
    for (const v of variants) {
      ...
      if (brand?.slug === "noble-mushrooms" && /^\s*\d+(\.\d+)?\s*oz/i.test(label)) continue;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave96.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave96Products: waveCount,
        wave96BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave96-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave96=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
