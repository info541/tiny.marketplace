/**
 * Fetch Shopify catalogs for wave40 brands (c1077+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave40.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave40.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave40Products: waveCount,
        wave40BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave40-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave40=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
