/**
 * Drop leftover merch from wave51 and backfill missing ingredients from product pages.
 */
...
async function main() {
  ...
  const cleaned = all
    .filter((p) => {
      ...
      if (keepGarageOilSoap || keepDakotaScrub) return true;
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave51Products: wave.length,
    wave51WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave51-ingredients-backfill.json"),
