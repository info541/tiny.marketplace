/**
 * Fetch Shopify catalogs for wave70 brands (c1289+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave70.mjs";
...
const WAVE70_EXTRA =
  /\b(gift card|e-gift|gift pack|mystery box|shipping protection|build your (own )?bundle|bundle\b|duo\b|trio\b|sets?\b|variety pack|holiday gift|shaker bottle|sampler pack|everything but|chewies|ortho wax|stay\b|tinted\s*lip|lip serum|grapple guard shirt|canvas tote|body spray|slug.?a.?bug|\bticks?\b|bug spray|insect|survival kit|t-shirt|faith revival|ministry merch|liquid culture|agar plate|spore swab|colonized|grow kit|dried (blue )?oyster|research plate|dog mushroom|ultimate mushroom|complete wellness|directory|recipes for beginners|room spray)\b/i;
...
function looksLikeInci(parts, raw) {
  ...
  if (
    parts.length >= 3 &&
    /water|aqua|glycerin|sodium|acid|extract|oil|oxide|protein|vitamin|tallow|wax|butter|zinc|salt|magnesium|potassium|gum|xylitol|hydroxyapatite|clay|shea|hydrobeef|cocoa|glycine|stevia|alcohol|honey|jojoba/i.test(
      raw,
    )
  ) {
    return true;
  }
  if (parts.length >= 2 && /tallow|oil|wax|butter|zinc|beeswax|xylitol|hydroxyapatite|hydrobeef|alcohol/i.test(raw) && /,/.test(raw)) {
    return true;
  }
...
function inferCategory(brandCategories, title, productType, tags) {
  ...
  const rules = [
    ["sunscreen", /\b(spf|sunscreen|sun\s?screen|sunblock|sun butter|sun[- ]?balm|sungaze|sun protectant|zinc stick|mineral zinc|mineral sun)\b/],
    ["deodorant", /deodor|antiperspir|underarm|\bdeo\b/],
    ["supplements", /\b(lion'?s mane|reishi|chaga|cordyceps|shiitake|maitake|oyster mushroom|mushroom (complex|tincture|extract)|adaptogen|creatine|vitamin d|magnesium|shilajit|collagen|maca|chlorella)\b/],
...
function refineCategory(category, title, brand) {
  ...
  if (brand?.slug === "fizzy-oral") return "oral";
  if (brand?.slug === "cypher") return "electrolytes";
  if (brand?.slug === "living-love-superfoods") {
    if (/\b(maca|chlorella)\b/.test(t)) return "supplements";
    return "protein";
  }
  if (brand?.slug === "tallow-balm-company") {
    if (/\b(sun cream|sunscreen|spf|zinc|sun balm)\b/.test(t)) return "sunscreen";
    return "skincare";
  }
  if (brand?.slug === "sunny-soil") {
    if (/\b(sungaze|sun balm|sunblock|sunscreen|spf)\b/.test(t)) return "sunscreen";
    if (/\bdeodorant\b/.test(t)) return "deodorant";
    if (/\b(toothpaste|mouthwash|tooth powder|oral)\b/.test(t)) return "oral";
    if (/\b(shampoo|hair juice|hair)\b/.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "spruce-creek-mushrooms") return "supplements";
  return category;
}
...
function isOnNiche(raw, brand) {
  ...
  if (SKIP_TITLE.test(title) || WAVE70_EXTRA.test(title)) return false;
  if (brand.slug === "fizzy-oral") {
    if (/\b(shipping|bundle|everything but|variety|chewies|ortho wax|stay)\b/i.test(title)) return false;
    if (/mouthwash-concentrate-stay/i.test(handle)) return false;
    if (Number(raw.variants?.[0]?.price || 0) <= 0) return false;
    return /\b(mouthwash|floss|tongue|toothbrush|retainer cleaner)\b/i.test(hay);
  }
  if (brand.slug === "cypher") {
    return /electrolyt|hydrat/i.test(hay);
  }
  if (brand.slug === "living-love-superfoods") {
    if (/\b(sample|01 serving|1 serving)\b/i.test(title)) return false;
    return /\b(protein|whey|maca|chlorella)\b/i.test(hay);
  }
  if (brand.slug === "tallow-balm-company") {
    if (/\b(shirt|tee|duo|2 pack|tinted|lip serum|grapple guard shirt)\b/i.test(title)) return false;
    return /\b(sun cream|cloud moisturizer|original balm|body butter|tallow guard|pocket tallow)\b/i.test(hay);
  }
  if (brand.slug === "sunny-soil") {
    if (
      /\b(tote|t-shirt|gift card|bundle|kit|bug|body spray|survival|jesus|yahweh|ministry|3-pack|pack of 3|all-clean|hocl|hypochlorous)\b/i.test(
        title,
      )
    ) {
      return false;
    }
    return /\b(deodorant|sungaze|sun balm|toothpaste|mouthwash|tooth powder|shampoo|hair juice|tallow|soap|balm|cream|castile|body powder|magnesium|lip)\b/i.test(
      hay,
    );
  }
  if (brand.slug === "spruce-creek-mushrooms") {
    if (/\b(dog|bundle|tee|t-shirt|gift|culture|agar|spore|dried|colonized|grow|plate|syringe)\b/i.test(title)) {
      return false;
    }
    return /\btincture\b/i.test(hay);
  }
...
function flavorOptionIndex(raw, brand) {
  ...
  if (brand?.slug === "fizzy-oral") {
    const flavor = options.findIndex((o) => /flavor/i.test(o.name || ""));
    if (flavor >= 0) return flavor;
  }
  if (brand?.slug === "cypher") {
    const flavour = options.findIndex((o) => /flavour|flavor/i.test(o.name || ""));
    if (flavour >= 0) return flavour;
  }
  if (brand?.slug === "sunny-soil") {
    const scent = options.findIndex((o) => /scent|inci/i.test(o.name || ""));
  ...
  if (brand?.slug === "living-love-superfoods" || brand?.slug === "spruce-creek-mushrooms") {
    return -1;
  }
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel) {
  ...
  if (
    flavorClean &&
    /^(default title|single|30 servings|60 servings|90 servings|2oz|4oz|3oz|1oz|6g|scented|buy one|buy two|buy three|1 jar|2 jars|cloud like|cloud moisturizer)$/i.test(
      flavorClean,
    )
  ) {
    flavorClean = null;
...
function expandProducts(raw, brand, brandId, index) {
  ...
  const skipFlavorExpand =
    brand.slug === "living-love-superfoods" || brand.slug === "spruce-creek-mushrooms";
  if (!skipFlavorExpand && flavorIdx >= 0 && variants.length > 1) {
    ...
    for (const v of variants) {
      ...
      if (/^(scented|buy one|buy two|buy three|1 jar|2 jars)$/i.test(label)) continue;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave70.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave70Products: waveCount,
        wave70BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave70-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave70=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
