/**
 * Fetch Shopify catalogs for wave15 brands (c681+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave15.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave15.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave15Products: waveCount,
        wave15BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave15-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave15=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
