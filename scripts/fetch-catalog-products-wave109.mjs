/**
 * Fetch Shopify catalogs for wave109 brands (c1523+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave109.mjs";
...
function refineCategory(category, title, brand) {
  ...
  if (brand?.slug === "theodent") return "oral";
  if (brand?.slug === "voli-wellness") return "electrolytes";
  if (brand?.slug === "hela-nutrition") {
    if (/collagen|inulin|creatine/i.test(t)) return "supplements";
    return "protein";
  }
  if (brand?.slug === "indie-paradiso") return "sunscreen";
  if (brand?.slug === "ancestral-cosmetics") {
    if (/deodorant/i.test(t)) return "deodorant";
    if (/sun balm|sunscreen|spf/i.test(t)) return "sunscreen";
    if (/beard/i.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "yamabushi-farms") return "supplements";
  return category;
}

const THEODENT_KEEP = new Set(["classic", "kids", "300"]);

const VOLI_KEEP = new Set([
  "voli-hydration-hillside-honeydew",
  "voli-hydration-sunset-strawberry-lemonade",
  "voli-hydration-riptide-raspberry",
  "voli-hydration-pali-pina-colada",
  "voli-hydration-paradise-passionfruit",
]);

const HELA_KEEP = new Set(["organic-plant-protein", "unflavoured-pure-marine-collagen"]);

const INDIE_KEEP = new Set(["daily-face-tinted-sunscreen", "mineral-sunscreen"]);

const ANCESTRAL_KEEP = new Set([
  "tallow-deodorant-velvet-dust",
  "tallow-deodorant-unscented",
  "tallow-deodorant-sweet-meadow",
  "tallow-deodorant-pine-forest",
  "tallow-deodorant-mountain-spring",
  "tallow-body-cream-velvet-dust",
  "unscented-tallow-body-cream",
  "tallow-body-cream",
  "tallow-charcoal-cleansing-balm",
  "tallow-black-seed-oil-clarifying-cream",
  "unscented-tallow-honey-balm",
  "tallow-sun-balm",
  "tallow-honey-lipbalm-vanilla",
  "tallow-honey-lipbalm-orange",
  "tallow-honey-lipbalm",
  "original-tallow-honey-balm",
  "dry-body-oil",
  "beard-oil",
  "baby-protection-balm",
]);

const YAMABUSHI_KEEP = new Set([
  "lions-mane-tincture",
  "mamoru",
  "ibuki",
  "lion-s-mane-powder-ultra-fine-full-spectrum-cognitive-suppo",
  "sub-critical-fruiting-body-lions-mane-capsules",
]);

const KEEP_BY_SLUG = {
  theodent: THEODENT_KEEP,
  "voli-wellness": VOLI_KEEP,
  "hela-nutrition": HELA_KEEP,
  "indie-paradiso": INDIE_KEEP,
  "ancestral-cosmetics": ANCESTRAL_KEEP,
  "yamabushi-farms": YAMABUSHI_KEEP,
};
...
function flavorOptionIndex(raw, brand) {
  ...
  const handle = raw.handle || "";
  if (brand?.slug === "theodent") return -1;
  if (brand?.slug === "voli-wellness") return -1;
  if (brand?.slug === "indie-paradiso") return -1;
  if (brand?.slug === "yamabushi-farms") return -1;
  if (brand?.slug === "ancestral-cosmetics") return -1;
  if (brand?.slug === "hela-nutrition") {
    if (handle === "organic-plant-protein") {
...
function pickVariant(variants, brand, raw) {
  ...
  const scored = variants.map((v, i) => {
    ...
    if (/\b(two-pack|2-pack|three-pack|3-pack|four-pack|4-pack|2 boxes|4 boxes|6 boxes|0\.5oz|0\.5 oz|20 servings|3x90|250 g|250g|500 g|500g|1 kg|1kg|5g|sample size|refill only|2 jars|3 jars|5 jars|2 bars|3 bars|8 oz extra|16 oz|5lb|5 lb|family size|81 servings|15 servings|30 ct|3\.3 oz|refill cartridge|30g sample)\b/i.test(hay))
      score -= 8;
    if (brand?.slug === "hela-nutrition") {
      if (/\b(chocolate|vanilla|natural)\b/i.test(hay)) score += 4;
    }
    if (brand?.slug === "ancestral-cosmetics") {
      if (/\b1 pack\b/i.test(hay)) score += 10;
      if (/\b(2 pack|3 pack)\b/i.test(hay)) score -= 10;
    }
    if (brand?.slug === "yamabushi-farms") {
      if (/\b30 capsules\b/i.test(hay)) score += 10;
      if (/\b60 capsules\b/i.test(hay)) score -= 10;
    }
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel, currency) {
  let baseTitle = (raw.title || "")
    ...
    .replace(/\s+/g, " ")
    .trim();
  if (brand.slug === "theodent") {
    const h = raw.handle || "";
    if (h === "classic") baseTitle = "Theodent Classic Toothpaste";
    else if (h === "kids") baseTitle = "Theodent Kids Toothpaste";
    else if (h === "300") baseTitle = "Theodent 300 Clinical Strength Toothpaste";
    else if (!/^theodent\b/i.test(baseTitle)) baseTitle = `Theodent ${baseTitle}`;
  }
  if (brand.slug === "voli-wellness") {
    const h = raw.handle || "";
    if (h === "voli-hydration-riptide-raspberry") baseTitle = "Voli Hydration — Riptide Raspberry";
    else if (h === "voli-hydration-sunset-strawberry-lemonade")
      baseTitle = "Voli Hydration — Sunset Strawberry Lemonade";
    else if (h === "voli-hydration-paradise-passionfruit") baseTitle = "Voli Hydration — Paradise Passionfruit";
    else if (h === "voli-hydration-pali-pina-colada") baseTitle = "Voli Hydration — Pali Piña Colada";
    else if (h === "voli-hydration-hillside-honeydew") baseTitle = "Voli Hydration — Hillside Honeydew";
    else if (!/^voli\b/i.test(baseTitle)) baseTitle = `Voli ${baseTitle}`;
  }
  if (brand.slug === "hela-nutrition") {
    const h = raw.handle || "";
    if (h === "organic-plant-protein") baseTitle = "HeLa Organic Plant Protein";
    else if (h === "unflavoured-pure-marine-collagen") baseTitle = "HeLa Marine Collagen";
    else if (!/^hela\b/i.test(baseTitle)) baseTitle = `HeLa ${baseTitle}`;
  }
  if (brand.slug === "indie-paradiso") {
    const h = raw.handle || "";
    if (h === "mineral-sunscreen") baseTitle = "Indie Paradiso Mineral Sunscreen SPF 50+";
    else if (h === "daily-face-tinted-sunscreen") baseTitle = "Indie Paradiso Daily Face Tinted Sunscreen SPF 50+";
    else if (!/^indie paradiso\b/i.test(baseTitle)) baseTitle = `Indie Paradiso ${baseTitle}`;
  }
  if (brand.slug === "ancestral-cosmetics") {
    const h = raw.handle || "";
    if (h === "tallow-deodorant-mountain-spring") baseTitle = "Ancestral Tallow Deodorant — Mountain Spring";
    else if (h === "tallow-deodorant-pine-forest") baseTitle = "Ancestral Tallow Deodorant — Pine Forest";
    else if (h === "tallow-deodorant-sweet-meadow") baseTitle = "Ancestral Tallow Deodorant — Sweet Meadow";
    else if (h === "tallow-deodorant-unscented") baseTitle = "Ancestral Tallow Deodorant — Unscented";
    else if (h === "tallow-deodorant-velvet-dust") baseTitle = "Ancestral Tallow Deodorant — Velvet Dust";
    else if (h === "tallow-honey-lipbalm") baseTitle = "Ancestral Tallow & Honey Lip Balm — Peppermint";
    else if (h === "tallow-honey-lipbalm-orange") baseTitle = "Ancestral Tallow & Honey Lip Balm — Orange";
    else if (h === "tallow-honey-lipbalm-vanilla") baseTitle = "Ancestral Tallow & Honey Lip Balm — Vanilla";
    else if (h === "tallow-sun-balm") baseTitle = "Ancestral Tallow Sun Balm";
    else if (h === "tallow-body-cream") baseTitle = "Ancestral Tallow Body Cream";
    else if (h === "unscented-tallow-body-cream") baseTitle = "Ancestral Tallow Body Cream — Unscented";
    else if (h === "tallow-body-cream-velvet-dust") baseTitle = "Ancestral Tallow Body Cream — Velvet Dust";
    else if (h === "original-tallow-honey-balm") baseTitle = "Ancestral Tallow & Honey Balm";
    else if (h === "unscented-tallow-honey-balm") baseTitle = "Ancestral Tallow & Honey Balm — Unscented";
    else if (h === "tallow-charcoal-cleansing-balm") baseTitle = "Ancestral Tallow & Charcoal Cleansing Balm";
    else if (h === "tallow-black-seed-oil-clarifying-cream")
      baseTitle = "Ancestral Tallow & Black Seed Oil Clarifying Cream";
    else if (h === "dry-body-oil") baseTitle = "Ancestral Dry Body Oil";
    else if (h === "beard-oil") baseTitle = "Ancestral Beard Oil";
    else if (h === "baby-protection-balm") baseTitle = "Ancestral Baby Protection Balm";
    else if (!/^ancestral\b/i.test(baseTitle)) baseTitle = `Ancestral ${baseTitle}`;
  }
  if (brand.slug === "yamabushi-farms") {
    const h = raw.handle || "";
    if (h === "lions-mane-tincture") baseTitle = "Yamabushi MEZAME Lion's Mane Tincture";
    else if (h === "mamoru") baseTitle = "Yamabushi MAMORU Turkey Tail Tincture";
    else if (h === "ibuki") baseTitle = "Yamabushi IBUKI Cordyceps Lion's Mane Tincture";
    else if (h === "lion-s-mane-powder-ultra-fine-full-spectrum-cognitive-suppo")
      baseTitle = "Yamabushi Lion's Mane Powder";
    else if (h === "sub-critical-fruiting-body-lions-mane-capsules") baseTitle = "Yamabushi Lion's Mane Capsules";
    else if (!/^yamabushi\b/i.test(baseTitle)) baseTitle = `Yamabushi ${baseTitle}`;
  }
  ...
  if (flavorClean && brand.slug === "hela-nutrition") {
    if (!/^(chocolate|vanilla|natural)$/i.test(flavorClean)) flavorClean = null;
  }
  ...
  if (brand.slug === "theodent" || brand.slug === "voli-wellness" || brand.slug === "indie-paradiso") {
    ingredients = [];
  }
  if (brand.slug === "voli-wellness") {
    ingredients = [
  ...
  if (brand.slug === "indie-paradiso") {
    ingredients = ["Non-nano zinc oxide"];
  }
  if (brand.slug === "hela-nutrition") {
    const h = raw.handle || "";
    if (h === "organic-plant-protein") {
      const flavor = String(flavorClean || variant?.option1 || "").toLowerCase();
      const blend = [
        "Organic pea protein",
      ...
      if (flavor.includes("chocolate")) {
        ingredients = [...blend, "Organic cocoa powder", "Organic chocolate flavour", "Salt", "Monk fruit juice concentrate"];
      } else if (flavor.includes("vanilla")) {
        ingredients = [...blend, "Organic vanilla flavour", "Salt", "Monk fruit juice concentrate"];
      } else {
        ingredients = blend;
      }
    } else if (h === "unflavoured-pure-marine-collagen") {
  ...
  if (brand.slug === "ancestral-cosmetics") {
    const h = raw.handle || "";
    if (h.startsWith("tallow-deodorant")) {
      ingredients = [
    ...
    } else if (h.includes("lipbalm") || h.includes("honey-balm")) {
      ingredients = ["Grass-fed Irish beef tallow", "Honey", "Beeswax"];
    } else if (h === "tallow-sun-balm") {
      ingredients = ["Grass-fed Irish beef tallow", "Non-nano zinc oxide"];
    } else if (h.includes("body-cream")) {
      ingredients = ["Grass-fed Irish beef tallow"];
    } else {
      ingredients = [];
    }
  }
  if (brand.slug === "yamabushi-farms") {
    const h = raw.handle || "";
    if (h === "lions-mane-tincture") {
      ingredients = ["Lion's Mane fruiting body", "Food-grade ethanol", "Filtered water"];
    } else if (h === "mamoru") {
      ingredients = ["Turkey tail fruiting body", "Maitake fruiting body", "Shiitake fruiting body", "Food-grade ethanol", "Filtered water"];
    } else if (h === "ibuki") {
      ingredients = ["Cordyceps fruiting body", "Guayusa", "Lion's Mane fruiting body", "Food-grade ethanol", "Filtered water"];
    } else if (h === "lion-s-mane-powder-ultra-fine-full-spectrum-cognitive-suppo") {
      ingredients = ["Lion's Mane fruiting body"];
    } else if (h === "sub-critical-fruiting-body-lions-mane-capsules") {
      ingredients = ["Lion's Mane fruiting body"];
    }
  }
...
function expandProducts(raw, brand, brandId, index, currency) {
  ...
  if (flavorIdx >= 0 && variants.length > 1) {
    ...
    for (const v of variants) {
      ...
      if (brand?.slug === "hela-nutrition") {
        if (!/^(chocolate|vanilla|natural)$/i.test(label)) continue;
      }
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave109.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave109Products: waveCount,
        wave109BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave109-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave109=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
