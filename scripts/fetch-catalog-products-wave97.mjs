/**
 * Fetch Shopify catalogs for wave97 brands (c1445+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave97.mjs";
...
function refineCategory(category, title, brand) {
  ...
  if (brand?.slug === "clockwork") return "oral";
  if (brand?.slug === "elation-hydration") return "electrolytes";
  if (brand?.slug === "kono-nutrition") {
    if (/hydrat|electrolyte/i.test(t)) return "electrolytes";
    if (/pre-workout|magnesium|vitamin|astaxanthin/i.test(t)) return "supplements";
    return "protein";
  }
  if (brand?.slug === "mama-sol") return "sunscreen";
  if (brand?.slug === "rivas-ranch") {
    if (/sun slayer|sunscreen|spf|zinc/i.test(t) && /sun/.test(t)) return "sunscreen";
    if (/deodorant/i.test(t)) return "deodorant";
    return "skincare";
  }
  if (brand?.slug === "feral-fungi") return "supplements";
  return category;
}
...
function isOnNiche(raw, brand) {
  ...
  if (SKIP_TITLE.test(title) || WAVE97_EXTRA.test(title) || WAVE97_EXTRA.test(handle)) return false;
  if (Number((raw.variants || [])[0]?.price || 0) <= 0) return false;
  if (brand.slug === "clockwork") {
    if (/\b(ritual|combo|duo|kit|bundle|reserve|partner)\b/i.test(title)) return false;
    return /peppermint-toothpaste-with-nhap|natural-kids-toothpaste-orange-vanilla|natural-kids-toothpaste-strawberry-watermelon/.test(
      handle,
    );
  }
  if (brand.slug === "elation-hydration") {
    if (/\bbundle\b/i.test(title)) return false;
    return /adult-hydration|kids-hydration/.test(handle);
  }
  if (brand.slug === "kono-nutrition") {
    if (
      /\b(bundle|e-?book|tee\b|hat\b|frother|massage|gift|route|favorites|crew neck)\b/i.test(title) ||
      /gift|ebook|tee|hat|frother|massage|route|bundle|favorites/.test(handle)
    ) {
      return false;
    }
    return /\b(whey|protein|beef|hydrate|pre-workout|creatine)\b/i.test(hay);
  }
  if (brand.slug === "mama-sol") {
    if (/\b(duo|trio|system|wardrobe|brush|gift|donation)\b/i.test(title)) return false;
    return /100-mineral-moisturizing-sunscreen-spf-30|facebrella-tint|eyebrella-100-mineral|lipbrella-100-mineral/.test(
      handle,
    );
  }
  if (brand.slug === "rivas-ranch") {
    if (
      /^halal-/i.test(handle) ||
      /\b(6-pack|3-pack|gift)\b/i.test(title) ||
      /6-pack|3-pack|gift/.test(handle)
    ) {
      return false;
    }
    return /\b(deodorant|whipped|lip balm|sun slayer|bareface|calma|iron grip|breathe easy|tallow dots|deep relief)\b/i.test(
      hay,
    );
  }
  if (brand.slug === "feral-fungi") {
    if (
      /\b(bundle|set|coffee|gift|guide|reference)\b/i.test(title) ||
      /bundle|set$|coffee|gift|guide|reference/.test(handle)
    ) {
      return false;
    }
    return /\b(extract|tincture|essence|gummies|gummy)\b/i.test(hay);
  }
...
function flavorOptionIndex(raw, brand) {
  ...
  const handle = raw.handle || "";
  if (brand?.slug === "clockwork") return -1;
  if (brand?.slug === "elation-hydration") return -1;
  if (brand?.slug === "feral-fungi") return -1;
  if (brand?.slug === "kono-nutrition") {
    return options.findIndex((o) => /flavor/i.test(o.name || ""));
  }
  if (brand?.slug === "mama-sol") {
    return options.findIndex((o) => /color/i.test(o.name || ""));
  }
  if (brand?.slug === "rivas-ranch") {
    const idx = options.findIndex((o) => /flavor|scent/i.test(o.name || ""));
    return idx;
  }
...
function pickVariant(variants, brand, raw) {
  ...
  const scored = variants.map((v, i) => {
    ...
    if (/\b(two-pack|2-pack|three-pack|3-pack|four-pack|4-pack|2 boxes|4 boxes|6 boxes|0\.5oz|0\.5 oz|20 servings|3x90|250 g|250g|500 g|500g|1 kg|1kg|5g|sample size|refill only|2 jars|3 jars|5 jars|2 bars|3 bars|8 oz extra|16 oz|5lb|5 lb|family size|81 servings|15 servings|30 ct|3\.3 oz|refill cartridge)\b/i.test(hay))
      score -= 8;
    if (brand?.slug === "clockwork") {
      if (/100g|80g/i.test(hay)) score += 8;
    }
    if (brand?.slug === "kono-nutrition") {
      if (/30 serving bag|unflavored|madagascar vanilla/i.test(hay)) score += 8;
      if (/1 vanilla \+ 1 chocolate|stick packs/i.test(hay)) score -= 12;
    }
    if (brand?.slug === "rivas-ranch") {
      if (/\b4 oz\b/i.test(hay)) score += 10;
      if (/\b8 oz\b/i.test(hay)) score -= 6;
    }
    if (brand?.slug === "feral-fungi") {
      if (/\b1\s?oz\b|\b10ml\b/i.test(hay)) score += 10;
      if (/\b2\s?oz\b|\b4\s?oz\b/i.test(hay)) score -= 6;
    }
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel, currency) {
  let baseTitle = (raw.title || "")
    ...
    .trim();
  if (brand.slug === "clockwork") {
    if (/orange/i.test(baseTitle)) baseTitle = "Clockwork Kids nHAP Toothpaste — Orange Vanilla";
    else if (/strawberry/i.test(baseTitle)) baseTitle = "Clockwork Kids nHAP Toothpaste — Strawberry Watermelon";
    else baseTitle = "Clockwork Natural nHAP Toothpaste — Peppermint";
  }
  if (brand.slug === "elation-hydration") {
    if (/morning berry/i.test(baseTitle)) baseTitle = "Elation Hydration — Morning Berry";
    else if (/moon berry/i.test(baseTitle)) baseTitle = "Elation Hydration — Moon Berry";
    else if (/burst/i.test(baseTitle)) baseTitle = "Elation Hydration Kids — Berry Melon Burst";
    else if (/mellow/i.test(baseTitle)) baseTitle = "Elation Hydration Kids — Berry Melon Mellow";
    else if (!/^elation/i.test(baseTitle)) baseTitle = `Elation ${baseTitle}`;
  }
  if (brand.slug === "kono-nutrition") {
    if (/whey/i.test(baseTitle)) baseTitle = "KONO Regenerative Grass-Fed Whey Protein Isolate";
    else if (/beef/i.test(baseTitle)) baseTitle = "KONO Beef Protein Blend";
    else if (/hydrate/i.test(baseTitle)) baseTitle = "KONO Full Body Hydrate+";
    else if (/pre-workout/i.test(baseTitle)) baseTitle = "KONO Functional Pre-Workout";
    else if (!/^kono\b/i.test(baseTitle)) baseTitle = `KONO ${baseTitle}`;
  }
  if (brand.slug === "mama-sol") {
    if (/moisturizing sunscreen/i.test(baseTitle)) baseTitle = "Mama Sol FACEBRELLA Mineral Moisturizing Sunscreen SPF 30";
    else if (/tints/i.test(baseTitle)) baseTitle = "Mama Sol FACEBRELLA Tints Mineral Glow Serum SPF 45";
    else if (/eyebrella/i.test(baseTitle)) baseTitle = "Mama Sol EYEBRELLA Mineral Tinted Eye Brightener SPF 45";
    else if (/lipbrella/i.test(baseTitle)) baseTitle = "Mama Sol LIPBRELLA Mineral Tinted Lip Oil SPF 20";
    else if (!/^mama sol/i.test(baseTitle)) baseTitle = `Mama Sol ${baseTitle}`;
  }
  if (brand.slug === "rivas-ranch") {
    baseTitle = baseTitle.replace(/\s*☀️\s*/g, " ").replace(/\s*\|\s*.*$/, "").trim();
    if (!/^rivas/i.test(baseTitle)) baseTitle = `Rivas Ranch ${baseTitle}`;
  }
  if (brand.slug === "feral-fungi") {
    if (!/^feral/i.test(baseTitle)) baseTitle = `Feral Fungi ${baseTitle}`;
  }
  ...
  if (
    brand.slug === "clockwork" ||
    brand.slug === "elation-hydration" ||
    brand.slug === "feral-fungi"
  ) {
    ingredients = [];
  }
  if (brand.slug === "kono-nutrition") {
    if (/whey/i.test(title) && /chocolate|cacao/i.test(title)) {
      ingredients = [
    ...
    } else if (/whey/i.test(title)) {
      ingredients = ["Grass Fed Whey Protein Isolate", "Non-GMO Sunflower Lecithin"];
    } else if (/beef/i.test(title)) {
      ingredients = [
  ...
  if (brand.slug === "mama-sol") {
    if (/moisturizing sunscreen|spf 30/i.test(title)) {
      ingredients = [
        "Zinc Oxide",
  ...
  if (brand.slug === "rivas-ranch") {
    if (/deodorant/i.test(title)) {
      const scent = [];
      if (/lemon basil/i.test(title)) {
        scent.push("Lemon essential oil", "Basil essential oil");
      } else if (/lavender rose/i.test(title)) {
        scent.push("Lavender essential oil", "Rose essential oil");
      } else if (/cedar bergamot|bergamot/i.test(title)) {
        scent.push("Cedarwood essential oil", "Bergamot essential oil");
      } else if (/citrus/i.test(title)) {
        scent.push("Citrus essential oil");
      }
      ingredients = [
      ...
      ingredients = [
        ...
        "Vitamin E",
        "Menthol flakes",
        /naked/i.test(title) ? "Unflavored" : "Natural flavor",
      ].filter((x) => x !== "Unflavored");
    } else {
      ingredients = [];
    }
  }
...
function expandProducts(raw, brand, brandId, index, currency) {
  ...
  if (flavorIdx >= 0 && variants.length > 1) {
    ...
    for (const v of variants) {
      ...
      if (brand?.slug === "feral-fungi" && /^\s*\d+(\.\d+)?\s*(oz|ml)/i.test(label)) continue;
      if (brand?.slug === "kono-nutrition" && /\+|bundle|vanilla \+ 1 chocolate/i.test(label)) continue;
      if (brand?.slug === "rivas-ranch" && /flavor of the month|1 vanilla/i.test(label)) continue;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave97.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave97Products: waveCount,
        wave97BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave97-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave97=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
