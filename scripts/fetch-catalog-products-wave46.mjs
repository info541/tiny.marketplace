/**
 * Fetch Shopify catalogs for wave46 brands (c1136+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave46.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave46.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave46Products: waveCount,
        wave46BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave46-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave46=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
