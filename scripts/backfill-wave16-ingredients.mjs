/**
 * Drop leftover merch from wave16 and backfill missing ingredients from product pages.
 */
...
async function main() {
  ...
  const cleaned = all.filter((p) => !(isWave16(p) && MERCH.test(p.name)));
  const dropped = before - cleaned.length;

  const missing = cleaned.filter((p) => isWave16(p) && (!p.ingredients || p.ingredients.length === 0));
  let filled = 0;
  ...
  const wave = cleaned.filter(isWave16);
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave16Products: wave.length,
    wave16WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave16-ingredients-backfill.json"),
