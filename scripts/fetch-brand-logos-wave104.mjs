/**
 * Download wave104 brand logos into public/brands/{slug}.png (or .svg fallback).
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { brands } from "./catalog-brands-seed-wave104.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const outDir = path.join(__dirname, "../public/brands");
const catalogPath = path.join(__dirname, "../src/lib/catalog-brands-wave104.ts");
...
fs.writeFileSync(
  path.join(__dirname, "../data/brand-logo-manifest-wave104.json"),
