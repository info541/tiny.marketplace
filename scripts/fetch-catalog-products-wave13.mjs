/**
 * Fetch Shopify catalogs for wave13 brands (c641+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave13.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave13.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave13Products: waveCount,
        wave13BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave13-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave13=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
