/**
 * Fetch Shopify catalogs for wave10 brands (c581+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave10.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave10.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave10Products: waveCount,
        wave10BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave10-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave10=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
