/**
 * Write src/lib/catalog-brands-wave12.ts from the wave12 seed.
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { brands } from "./catalog-brands-seed-wave12.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const out = path.join(__dirname, "../src/lib/catalog-brands-wave12.ts");
...
const rows = brands.map((b, i) => {
  const id = `c${String(i + 621).padStart(3, "0")}`;
  return `  {
    id: ${tsString(id)},
