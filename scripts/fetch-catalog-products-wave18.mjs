/**
 * Fetch Shopify catalogs for wave18 brands (c738+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave18.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave18.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave18Products: waveCount,
        wave18BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave18-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave18=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
