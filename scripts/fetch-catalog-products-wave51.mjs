/**
 * Fetch Shopify catalogs for wave51 brands (c1164+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave51.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave51.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave51Products: waveCount,
        wave51BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave51-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave51=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
