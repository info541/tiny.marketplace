/**
 * Fetch Shopify catalogs for wave19 brands (c758+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave19.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave19.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave19Products: waveCount,
        wave19BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave19-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave19=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
