/**
 * Fetch Shopify catalogs for wave43 brands (c1108+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave43.mjs";
...
function expandProducts(raw, brand, brandId, index) {
  ...
  if (flavorIdx >= 0 && variants.length > 1) {
    ...
    for (const v of variants) {
      ...
      if (/\b(variety pack|staple flavors?|limited flavors?|mix of|mix vanilla|duo\b|toothbrush|brush eco|bass -|rows of bristles|donate it|3 pack|7 pack|single bottle|bulk bag|bundle of)\b/i.test(label)) continue;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave43.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave43Products: waveCount,
        wave43BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave43-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave43=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
