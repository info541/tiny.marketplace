/**
 * Fetch Shopify catalogs for wave36 brands (c1019+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave36.mjs";
...
const FLAVOR_OPTION = /flavor|flavour|scent|fragrance|smell|aroma|colour|color|shade|scented|finish|constitutive|herb|fluord|fluoride|the taste|dry shampoo colour|style/i;
...
function refineCategory(category, title, brand) {
  ...
  if (/\b(hydration drops?|electrolytes?|electrolyte powder|yuzu citrus|asian pear|white peach|lychee|watermelon electrolyte|jamaica electrolyte|acai electrolyte|pog electrolyte|electrolyte (tub|singles|caps|mix)|hydration & recovery|sleep & hydration|raspberry electrolyte|lemon and lime electrolyte|adapt variety|adapt sleep|adapt 30pk|adapt sample|lytes|lemonade|cota)\b/.test(t) && !/\bprotein\b/.test(t)) {
    return "electrolytes";
  }
  if (/\b(sun balm|mineral melt|mug guard|bod guard|sporto spray|melt stick|slip stick|golden guard|gold standard|the lifeguard|the gang|daily double|mineral spf|miracle jelly|miracle mask|sun cream|zinc lotion)\b/.test(t)) {
    return "sunscreen";
  }
...
function isOnNiche(raw, brand) {
  ...
  if (skipped) {
    ...
    if (!keepDespiteEssentialOil && !keepSample && !keepTallowAftershave && !keepProteinBag) return false;
  }
  if (NICHE.test(hay) || /\bscrub\b/i.test(hay)) return true;
  if (brand.slug === "cota-hydration" && /\b(electrolyte|lemonade|strawberry|peach)\b/i.test(title)) return true;
  if (brand.slug === "two-sisters-pure-goods" && /\b(sun cream|tallow|shampoo|hair (mist|serum|refresher)|lip nectar|lotion bar|face (cream|serum)|toner|soap|salt scrub|sleep spray)\b/i.test(title)) {
    return true;
  }
  if (brand.slug === "some-essentials" && /\b(zinc lotion|deodorant|face cream|eye cream|lotion bar|lip lotion|body oil|face serum|bar soap|muscle rub|mask)\b/i.test(title)) {
    return true;
  }
  if (brand.slug === "enml" && /\b(toothpaste|mouthwash|toothbrush|hydroxyapatite|enml)\b/i.test(title)) return true;
  if (brand.slug === "tallow-mama" && /\b(tallow|soap|butter|lip balm|cleansing oil)\b/i.test(title)) return true;
  if (brand.slug === "pure-tallow" && /\b(tallow|velvet hour|moon dew|rose noir|amber glow)\b/i.test(title)) return true;
  if (brand.slug === "austin-handcrafted" && /\b(shampoo|serum|beard oil)\b/i.test(title)) return true;
...
function expandProducts(raw, brand, brandId, index) {
  ...
  if (flavorIdx >= 0 && variants.length > 1) {
    ...
    for (const v of variants) {
      ...
      if (/\b(variety pack|staple flavors?|limited flavors?|mix of|mix vanilla)\b/i.test(label)) continue;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave36.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave36Products: waveCount,
        wave36BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave36-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave36=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
