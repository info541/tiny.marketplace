/**
 * Fetch Shopify catalogs for wave22 brands (c811+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave22.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave22.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave22Products: waveCount,
        wave22BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave22-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave22=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
