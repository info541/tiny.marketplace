/**
 * Fetch Shopify catalogs for wave107 brands (c1511+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave107.mjs";
...
function refineCategory(category, title, brand) {
  ...
  if (brand?.slug === "oris") return "oral";
  if (brand?.slug === "deadshot") return "electrolytes";
  if (brand?.slug === "alpenpower") return "protein";
  if (brand?.slug === "atom-fresh") {
    if (/deodorant|deo\b/i.test(t)) return "deodorant";
    return "skincare";
  }
  if (brand?.slug === "cowski") return "skincare";
  if (brand?.slug === "mushroom-revival") return "supplements";
  return category;
}

const ORIS_KEEP = new Set([
  "toothpaste",
  "nano-bamboo-toothbrush",
  "standard-bamboo-toothbrush",
  "toothbrushes-recharge",
]);

const DEADSHOT_KEEP = new Set([
  "hydro-paradise-punch-pog",
  "d-s-hydro",
  "ds-hydro-raspberry-lemonade",
  "ds-hydro-coloda-breeze",
  "ds-hydro-orchard-peach",
  "ds-hydro-alpine-berry",
]);

const ALPEN_KEEP = new Set([
  "bio-whey-protein-joghurt-wildberry-500-g-apbwp",
  "native-whey-isolat-vanille-500-g-apknwi",
  "bio-casein-protein-neutral-750-g-apcp",
  "bio-hanfprotein-600-g-b07bh35md7",
  "bio-erbsenprotein-600-g-b07bfgr6rv",
  "bio-k-rbiskernprotein-600-g-b07nd23ztt",
  "bio-sonnenblumenprotein-600-g-b07nd12z3t",
  "bio-leinsamenprotein-600-g-b07nd2hwll",
  "bio-reisprotein-600-g-aprp",
  "bio-mandelprotein-apmp500",
  "bio-vegan-protein-erbse-reis-natur-apern600",
  "bio-haferprotein-aphrp600",
  "collagen-protein-copr500",
  "organic-cream-protein",
  "bio-sahne-protein",
]);

const ATOM_KEEP = new Set([
  "natural-deodorant-balm",
  "cream-deodorant",
  "natural-body-wash",
  "body-wash",
  "natural-body-wash-refined-edition",
  "natural-body-lotion",
  "natural-hand-lotion",
]);

const COWSKI_KEEP = new Set([
  "tallow-soap-french-lavender-lemongrass-bergamot",
  "tallow-soap-sandalwood-vanilla-bean",
  "organic-grass-fed-tallow-foot-balm",
  "organic-tallow-baby-balm",
  "organic-grass-fed-tallow-lip-balm-vanilla",
  "organic-grass-fed-tallow-lip-balm-peppermint",
  "organic-grass-fed-tallow-balm",
  "organic-grass-fed-whipped-tallow-balm",
]);

const MUSHROOM_KEEP = new Set([
  "reishi-calm-gummies",
  "daily-10-mix-mushroom-gummies",
  "focus-lions-mane-gummies",
  "cordyceps-energy-gummies",
  "daily-10-mix-mushroom-capsules",
  "focus-lions-mane-capsules",
  "daily-10-mix-mushroom-tincture",
  "focus-lions-mane-tincture",
...
const KEEP_BY_SLUG = {
  oris: ORIS_KEEP,
  deadshot: DEADSHOT_KEEP,
  alpenpower: ALPEN_KEEP,
  "atom-fresh": ATOM_KEEP,
  cowski: COWSKI_KEEP,
  "mushroom-revival": MUSHROOM_KEEP,
};
...
function flavorOptionIndex(raw, brand) {
  ...
  const handle = raw.handle || "";
  if (brand?.slug === "deadshot") return -1;
  if (brand?.slug === "cowski") return -1;
  if (brand?.slug === "mushroom-revival") return -1;
  if (brand?.slug === "oris") {
    return options.findIndex((o) => /flavou?rs?|style/i.test(o.name || ""));
  }
  if (brand?.slug === "alpenpower") {
...
function pickVariant(variants, brand, raw) {
  ...
  const scored = variants.map((v, i) => {
    ...
    if (/\b(two-pack|2-pack|three-pack|3-pack|four-pack|4-pack|2 boxes|4 boxes|6 boxes|0\.5oz|0\.5 oz|20 servings|3x90|250 g|250g|500 g|500g|1 kg|1kg|5g|sample size|refill only|2 jars|3 jars|5 jars|2 bars|3 bars|8 oz extra|16 oz|5lb|5 lb|family size|81 servings|15 servings|30 ct|3\.3 oz|refill cartridge|30g sample)\b/i.test(hay))
      score -= 8;
    if (brand?.slug === "alpenpower") {
      if (/\b(500 g|500g|600 g|600g|750 g)\b/i.test(hay)) score += 8;
      if (/\b(1000 g|1000g|1 kg|1kg)\b/i.test(hay)) score -= 8;
    }
    if (brand?.slug === "atom-fresh") {
      if (/\b(default title|full size|standard)\b/i.test(hay)) score += 4;
      if (/\b(mini|travel|trio|duo|set)\b/i.test(hay)) score -= 6;
    }
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel, currency) {
  ...
  if (brand.slug === "oris") {
    const h = raw.handle || "";
    if (h === "toothpaste") baseTitle = "Oris Toothpaste Tablets";
    else if (h === "nano-bamboo-toothbrush") baseTitle = "Oris Nano Bamboo Toothbrush";
    else if (h === "standard-bamboo-toothbrush") baseTitle = "Oris Standard Bamboo Toothbrush";
    else if (h === "toothbrushes-recharge") baseTitle = "Oris Toothbrush ReCharge";
    else if (!/^oris\b/i.test(baseTitle)) baseTitle = `Oris ${baseTitle}`;
  }
  if (brand.slug === "deadshot") {
    const h = raw.handle || "";
    const flavorMap = {
      "hydro-paradise-punch-pog": "Paradise Punch POG",
      "d-s-hydro": "Lemon Ice",
      "ds-hydro-raspberry-lemonade": "Raspberry Lemonade",
      "ds-hydro-coloda-breeze": "Colada Breeze",
      "ds-hydro-orchard-peach": "Orchard Peach",
      "ds-hydro-alpine-berry": "Alpine Berry",
    };
    baseTitle = flavorMap[h] ? `Deadshot Hydro — ${flavorMap[h]}` : `Deadshot ${baseTitle}`;
  }
  if (brand.slug === "alpenpower") {
    const h = raw.handle || "";
    if (h === "bio-whey-protein-joghurt-wildberry-500-g-apbwp") baseTitle = "AlpenPower Organic Whey Protein";
    else if (h === "native-whey-isolat-vanille-500-g-apknwi") baseTitle = "AlpenPower Whey Protein Isolate";
    else if (h === "bio-casein-protein-neutral-750-g-apcp") baseTitle = "AlpenPower Organic Casein Protein";
    else if (h === "bio-hanfprotein-600-g-b07bh35md7") baseTitle = "AlpenPower Organic Hemp Protein";
    else if (h === "bio-erbsenprotein-600-g-b07bfgr6rv") baseTitle = "AlpenPower Organic Pea Protein";
    else if (h === "bio-k-rbiskernprotein-600-g-b07nd23ztt") baseTitle = "AlpenPower Organic Pumpkin Seed Protein";
    else if (h === "bio-sonnenblumenprotein-600-g-b07nd12z3t") baseTitle = "AlpenPower Organic Sunflower Protein";
    else if (h === "bio-leinsamenprotein-600-g-b07nd2hwll") baseTitle = "AlpenPower Organic Linseed Protein";
    else if (h === "bio-reisprotein-600-g-aprp") baseTitle = "AlpenPower Organic Rice Protein";
    else if (h === "bio-mandelprotein-apmp500") baseTitle = "AlpenPower Organic Almond Protein";
    else if (h === "bio-vegan-protein-erbse-reis-natur-apern600") baseTitle = "AlpenPower Organic Vegan Protein Pea Rice";
    else if (h === "bio-haferprotein-aphrp600") baseTitle = "AlpenPower Organic Oat Protein";
    else if (h === "collagen-protein-copr500") baseTitle = "AlpenPower Collagen Protein";
    else if (h === "organic-cream-protein" || h === "bio-sahne-protein") baseTitle = "AlpenPower Organic Cream Protein";
    else if (!/^alpenpower\b/i.test(baseTitle)) baseTitle = `AlpenPower ${baseTitle}`;
  }
  if (brand.slug === "atom-fresh") {
    const h = raw.handle || "";
    if (h === "natural-deodorant-balm") baseTitle = "Atom Fresh Natural Deodorant Balm";
    else if (h === "cream-deodorant") baseTitle = "Atom Fresh Natural Deodorant Cream";
    else if (h === "natural-body-wash" || h === "body-wash") baseTitle = "Atom Fresh Natural Body Wash";
    else if (h === "natural-body-wash-refined-edition") baseTitle = "Atom Fresh Natural Body Wash Refined";
    else if (h === "natural-body-lotion") baseTitle = "Atom Fresh Natural Body Lotion";
    else if (h === "natural-hand-lotion") baseTitle = "Atom Fresh Body & Hand Lotion";
    else if (!/^atom fresh\b/i.test(baseTitle)) baseTitle = `Atom Fresh ${baseTitle}`;
  }
  if (brand.slug === "cowski") {
    if (!/^cowski\b/i.test(baseTitle)) baseTitle = `CowSki ${baseTitle}`;
  }
  if (brand.slug === "mushroom-revival") {
    const h = raw.handle || "";
    if (h === "focus-lions-mane-tincture") baseTitle = "Mushroom Revival Lion's Mane Tincture";
    else if (h === "focus-lions-mane-capsules") baseTitle = "Mushroom Revival Lion's Mane Capsules";
    else if (h === "focus-lions-mane-gummies") baseTitle = "Mushroom Revival Lion's Mane Gummies";
    else if (h === "energy-cordyceps-tincture") baseTitle = "Mushroom Revival Cordyceps Tincture";
    else if (h === "cordyceps-energy-gummies") baseTitle = "Mushroom Revival Cordyceps Gummies";
    else if (h === "calm-reishi-tincture") baseTitle = "Mushroom Revival Reishi Tincture";
    else if (h === "reishi-calm-gummies") baseTitle = "Mushroom Revival Reishi Gummies";
    else if (h === "daily-10-mix-mushroom-tincture") baseTitle = "Mushroom Revival Daily 10 Tincture";
    else if (h === "daily-10-mix-mushroom-capsules") baseTitle = "Mushroom Revival Daily 10 Capsules";
    else if (h === "daily-10-mix-mushroom-gummies") baseTitle = "Mushroom Revival Daily 10 Gummies";
    else if (!/^mushroom revival\b/i.test(baseTitle)) baseTitle = `Mushroom Revival ${baseTitle}`;
  }
  ...
  if (flavorClean && brand.slug === "atom-fresh") {
    if (/rose \+ sweet/i.test(flavorClean)) flavorClean = null;
    else flavorClean = flavorClean.replace(/\s+/g, " ").trim();
  }
  if (flavorClean && brand.slug === "alpenpower") {
  ...
  if (brand.slug === "oris" || brand.slug === "deadshot" || brand.slug === "atom-fresh") {
    ingredients = [];
  }
  if (brand.slug === "alpenpower") {
    const h = raw.handle || "";
    if (h === "bio-whey-protein-joghurt-wildberry-500-g-apbwp") {
      ingredients = ["Organic whey protein concentrate"];
    } else if (h === "native-whey-isolat-vanille-500-g-apknwi") {
      ingredients = ["Whey protein isolate"];
    } else if (h === "bio-casein-protein-neutral-750-g-apcp") {
      ingredients = ["Organic casein protein"];
    } else if (h === "collagen-protein-copr500") {
      ingredients = ["Hydrolyzed collagen peptides"];
    } else if (h === "bio-erbsenprotein-600-g-b07bfgr6rv") {
      ingredients = ["Organic pea protein"];
    } else if (h === "bio-hanfprotein-600-g-b07bh35md7") {
      ingredients = ["Organic hemp protein"];
    } else if (h === "bio-reisprotein-600-g-aprp") {
      ingredients = ["Organic rice protein"];
    } else if (h === "bio-vegan-protein-erbse-reis-natur-apern600") {
      ingredients = ["Organic pea protein", "Organic rice protein"];
    }
  }
  if (brand.slug === "cowski") {
    const h = raw.handle || "";
    if (h === "organic-grass-fed-tallow-balm" || h === "organic-grass-fed-whipped-tallow-balm") {
      ingredients = [
    ...
    } else if (h === "organic-grass-fed-tallow-foot-balm") {
      ingredients = [
        "Organic grass-fed tallow",
        "Jojoba oil",
        "Coconut oil",
    ...
    } else if (h === "organic-grass-fed-tallow-lip-balm-peppermint") {
      ingredients = [
        "Organic grass-fed tallow",
        "Organic coconut oil",
        "Organic jojoba oil",
    ...
    } else if (h === "organic-grass-fed-tallow-lip-balm-vanilla") {
      ingredients = [
        "Organic grass-fed tallow",
        "Organic coconut oil",
        "Organic jojoba oil",
    ...
    } else if (h === "organic-tallow-baby-balm") {
  ...
  if (brand.slug === "mushroom-revival") {
    ...
    } else if (/cordyceps tincture/i.test(title)) {
      ingredients = ["Cordyceps fruiting body", "Organic alcohol", "Water"];
    } else if (/reishi tincture/i.test(title)) {
      ingredients = ["Reishi fruiting body", "Organic alcohol", "Water"];
    }
  }
...
function expandProducts(raw, brand, brandId, index, currency) {
  ...
  if (flavorIdx >= 0 && variants.length > 1) {
    ...
    for (const v of variants) {
      ...
      if (brand?.slug === "atom-fresh") {
        if (/rose \+ sweet/i.test(label)) continue;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave107.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave107Products: waveCount,
        wave107BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave107-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave107=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
