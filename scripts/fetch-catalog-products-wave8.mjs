/**
 * Fetch Shopify catalogs for wave8 brands (c541+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave8.mjs";
...
const NICHE =
  /\b(spf\s*\d+|sunscreen|sun\s?screen|sunblock|sun butter|sun protection|antiperspir|underarm|\bdeo\b|toothpaste|toothbrush|floss|mouthwash|oral|hydroxyapatite|theobromine|oil pull|tongue clean|shampoo|conditioner|hair (oil|mask|serum|care|cream|butter)|scalp|leave[- ]?in|protein|whey|casein|collagen|pea protein|chocho|electrolyte|hydration|daily (calm|energy|hydration)|vitamin|supplement|capsule|softgel|probiotic|adaptogen|mushroom|tonic|powder|serum|moisturizer|moisturiser|cleanser|face wash|facial|toner|retinol|niacinamide|cream|lotion|oil|mask|balm|mist|essence|exfoliant|lip|body (lotion|butter|wash|cream|oil|bar)|soap|tallow|organ|greens?|superfood)\b|deodor/i;
...
function inferCategory(brandCategories, title, productType, tags) {
  ...
  const rules = [
    ...
    ["electrolytes", /\b(electrolyte|hydration (powder|packet|mix|stick|multiplier|pak)|rapid hydration|hydro pak|power pak)\b/],
    ["supplements", /\b(vitamin|supplement|capsule|softgel|probiotic|adaptogen|mushroom|tonic herb|collagen|prenatal|greens?|organ|superfood)\b/],
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave8.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave8Products: waveCount,
        wave8BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave8-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave8=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
