/**
 * Fetch Shopify catalogs for wave24 brands (c848+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave24.mjs";
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave24.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave24Products: waveCount,
        wave24BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave24-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave24=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
