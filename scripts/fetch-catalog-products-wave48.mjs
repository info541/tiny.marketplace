/**
 * Fetch Shopify catalogs for wave48 brands (c1153+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave48.mjs";
...
const FLAVOR_OPTION = /flavor|flavour|scent|fragrance|smell|aroma|colour|color|shade|scented|finish|constitutive|herb|fluord|fluoride|the taste|dry shampoo colour|style|tint|formula|whip|blend|infusi[oó]n|\bmaku\b|botanical bars|choose your scent|^product$|^variants$/i;

/** Convert shop-native prices that would otherwise look like USD luxury SKUs. */
const FX_TO_USD = {
  "himalayan-hydration": 0.73,
};
...
function isOnNiche(raw, brand) {
  ...
  if (skipped) {
    const keepDespiteEssentialOil =
      /\b(tallow|deodorant|toothpaste|whey|protein|sun butter|zinc balm|body butter|hair oil)\b/i.test(title) &&
...
function expandProducts(raw, brand, brandId, index) {
  ...
  if (!skipFlavorExpand && flavorIdx >= 0 && variants.length > 1) {
    ...
    for (const v of variants) {
      ...
      if (!label || /^default title$/i.test(label)) continue;
      if (/\b(variety pack|staple flavors?|limited flavors?|mix of|mix vanilla|duo\b|toothbrush|brush eco|bass -|rows of bristles|donate it|3 pack|7 pack|4 lip balms|3 shampoo|3 soaps|3 cleanser|3 tins|bundle & save|single bottle|bulk bag|bundle of|twin pack|three pack|family pack|pump x2|x2 100g|i want all)\b/i.test(label)) continue;
      if (/^\s*\d+(\.\d+)?\s*(oz|ml|g|kg|lb|count|ct|pack)\s*$/i.test(label)) continue;
      const key = label.toLowerCase();
      if (seen.has(key)) continue;
      seen.add(key);
      const mapped = mapOne(raw, brand, brandId, index, v, label);
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave48.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave48Products: waveCount,
        wave48BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave48-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave48=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
