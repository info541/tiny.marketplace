/**
 * Fetch Shopify catalogs for wave33 brands (c999+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave33.mjs";
...
const FLAVOR_OPTION = /flavor|flavour|scent|fragrance|smell|aroma|colour|color|shade|scented|finish|constitutive/i;

/** Convert shop-native prices that would otherwise look like USD luxury SKUs. */
const FX_TO_USD = {
  "impact-everyday": 0.73, // CAD
};
...
function refineCategory(category, title, brand) {
  ...
  if (/\b(hydration drops?|electrolytes?|electrolyte powder)\b/.test(t)) return "electrolytes";
  if (/\b(nootropic energy|energy powder)\b/.test(t) && !/\belectrolyt\b/.test(t)) return "supplements";
  if (/\b(marine collagen|collagen peptides)\b/.test(t)) return "supplements";
  if (/\b(whey protein|plant protein|pea protein|whey isolate)\b/.test(t)) return "protein";
  if (/\bbeard oil\b/.test(t)) return "hair";
...
function isOnNiche(raw, brand) {
  ...
  if (skipped) {
    ...
    if (!keepDespiteEssentialOil) return false;
  }
  if (NICHE.test(hay) || /\bscrub\b/i.test(hay)) return true;
  if (brand.slug === "nurture-glow" && !/\b(tint|makeup|glow stick|highlighter|shipping|gift)\b/i.test(title)) {
    return true;
  }
  if (brand.slug === "thirsty-donkey" && /\b(set|remedy|cream|butter|oil|soap|balm|sun)\b/i.test(title)) {
    return true;
  }
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave33.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave33Products: waveCount,
        wave33BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave33-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave33=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
