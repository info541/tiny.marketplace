/**
 * Fetch Shopify catalogs for wave87 brands (c1391+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave87.mjs";
...
const WAVE87_EXTRA =
  /\b(gift card|e-gift|gift pack|gift set|gift basket|mystery (box|gift)|shipping protection|package protection|savedby|navidium|build your (own )?bundle|bundles?\b|duo\b|duet\b|trio\b|variety pack|holiday gift|shaker bottle|shakers?\b|whisk\b|sampler pack|cashback|enamel pin|empty bottle|frother|mixer|water bottle|sachet|sample kit|sweatshirt|hoodie|\bhat\b|scrunchie|pillowcase|keychain|tote|socks|tumbler|straws?|complimentary|e-?book|consultation|sponsorship|sisal|soap dish|stainless steel bottle|wholesale|subscription gift|joint support bundle|ritual|collection|discovery set|glove|eyelash|herbal drink|tick(?:ed)? off|konjac|gua sha|spawn|liquid culture|sterilized grain|jerky|chef.?s mix)\b/i;
...
function refineCategory(category, title, brand) {
  ...
  if (brand?.slug === "yafa-queen") {
    if (/toothpaste|toothbrush|oral/.test(t)) return "oral";
    if (/deodorants?/.test(t)) return "deodorant";
    if (/shampoo|conditioner|hair|detangler|shine spray|masque|batana/.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "sodii") return "electrolytes";
  if (brand?.slug === "madeof-life") return "protein";
  if (brand?.slug === "doos-naturals") {
    if (/sun balm|sunblock|sun lip|spf/.test(t)) return "sunscreen";
    if (/deodorants?/.test(t)) return "deodorant";
    if (/shampoo|hair clay/.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "tallow-twins") {
    if (/sun (proof|milk)|tallow tan|spf/.test(t)) return "sunscreen";
    if (/deodorants?/.test(t)) return "deodorant";
    if (/hair oil|scalp|cowlick|styling/.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "sunshine-fungi") return "supplements";
  return category;
}
...
function isOnNiche(raw, brand) {
  ...
  if (SKIP_TITLE.test(title) || WAVE87_EXTRA.test(title)) return false;
  if (Number((raw.variants || [])[0]?.price || 0) <= 0) return false;
  if (brand.slug === "yafa-queen") {
    if (
      /\b(duo|ritual|collection|discovery|gift card|bug me|eyelash|eyebrow|glove|herbal drink|hair reducing|palestine flag|oud|amber|musk cream|mukhallat|nighttime serum|massage oil|alum face)\b/i.test(
        title,
      ) ||
      /amber\s*&\s*rose/i.test(title)
    ) {
      return false;
    }
    return /\b(toothpaste|toothbrush|deodorants?|shampoo|conditioner|detangler|shine spray|masque|hair oil|batana|soap|balm|butter|cream|toner|serum|scrub|lip|oil|mask|cleanser|makeup remover)\b/i.test(
      hay,
    );
  }
  if (brand.slug === "sodii") {
    if (
      /\b(honey|sample|bundle|builder)\b/i.test(title) ||
      /^everyday hydration salts/i.test(title)
    ) {
      return false;
    }
    return /\b(hydration salts|electrolyte)\b/i.test(hay);
  }
  if (brand.slug === "madeof-life") {
    if (/\b(shipping protection|route|insurance)\b/i.test(title)) return false;
    return /\b(whey|protein)\b/i.test(hay);
  }
  if (brand.slug === "doos-naturals") {
    if (/\b(bundle|insect|repellent|gift)\b/i.test(title)) return false;
    return /\b(sun balm|sunblock|sun lip|deodorants?|shampoo|hair clay|tallow|soap|lip|salve|moisturizer|cream)\b/i.test(
      hay,
    );
  }
  if (brand.slug === "tallow-twins") {
    if (
      /\b(bundle|sample set|hat|gift card|gua sha|konjac|ticked off|tick|methylene blue)\b/i.test(
        title,
      )
    ) {
      return false;
    }
    return /\b(sun (proof|milk)|tallow tan|deodorants?|hair oil|scalp|cowlick|soap|lip|butter|cream|balm|mist|scrub|cleanser|baby|magnesium|face)\b/i.test(
      hay,
    );
  }
  if (brand.slug === "sunshine-fungi") {
    if (
      /\b(fresh|dried|jerky|chef|spawn|grain|culture|pioppino|chestnut|morel|lobster|oyster)\b/i.test(
        title,
      ) &&
      !/\b(dual extract|powder)\b/i.test(title)
    ) {
      return false;
    }
    return /\b(dual extract|reishi powder|lion.?s mane powder)\b/i.test(hay);
  }
...
function flavorOptionIndex(raw, brand) {
  ...
  const handle = raw.handle || "";
  if (brand?.slug === "sodii" || brand?.slug === "madeof-life" || brand?.slug === "sunshine-fungi")
    return -1;
  if (brand?.slug === "yafa-queen") return -1;
  if (brand?.slug === "doos-naturals") {
    return options.findIndex((o) => /^(scent|color)$/i.test(o.name || ""));
  }
  if (brand?.slug === "tallow-twins") {
    if (handle === "cowlick-hair-styling-balm") {
      return options.findIndex((o) => /^type$/i.test(o.name || ""));
    }
...
async function fetchCurrency(base, brand) {
  try {
    ...
    if (brand?.slug === "sodii") return "USD";
    if (brand?.slug === "tallow-twins" && currency === "USD") currency = "CAD";
    return currency;
  } catch {
    if (brand?.slug === "tallow-twins") return "CAD";
    return "USD";
  }
}
...
function pickVariant(variants, brand) {
  ...
  const scored = variants.map((v, i) => {
    ...
    if (/\b(two-pack|2-pack|three-pack|3-pack|four-pack|4-pack|2 boxes|4 boxes|6 boxes|0\.5oz|0\.5 oz|20 servings|3x90|250 g|250g|500 g|500g|1 kg|1kg|5g|sample size|refill only|2 jars|3 jars|5 jars|2 bars|3 bars|4 oz\. amber|8 oz|plus 4 oz|plus 8 oz|2\.5lb|5lb|small\b)\b/i.test(hay))
      score -= 8;
    if (brand?.slug === "yafa-queen") {
      if (/single tube|single pack|default title/i.test(hay)) score += 10;
      if (/6-pack|12-pack|3 pack/i.test(hay)) score -= 14;
    }
    if (brand?.slug === "doos-naturals") {
      if (/\b(single|2 fl\.?\s*oz|4 fl\.?\s*oz\.?|small)\b/i.test(hay)) score += 8;
      if (/3-pack|4 fl oz \(save|6 fl\.?\s*oz|large bundle/i.test(hay)) score -= 10;
    }
    if (brand?.slug === "tallow-twins") {
      if (/\b(100\s*ml|60\s*ml|unscented)\b/i.test(hay)) score += 6;
      if (/240\s*ml|bundle/i.test(hay)) score -= 10;
    }
    if (brand?.slug === "sodii") {
      if (/sachets?\s*\(30/i.test(hay) || /30 sachets/i.test(hay)) score += 8;
      if (/tubs?/i.test(hay)) score -= 4;
    }
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel, currency) {
  let baseTitle = (raw.title || "")
    ...
    .trim();
  if (brand.slug === "yafa-queen") {
    if (/fresh mint herbal toothpaste/i.test(baseTitle)) baseTitle = "Yafa Fresh Mint Herbal Toothpaste";
    else if (/anise.*toothpaste|toothpaste.*original/i.test(baseTitle))
      baseTitle = "Yafa Anise Cinnamon Clove Toothpaste";
    else if (/bamboo toothbrush/i.test(baseTitle)) baseTitle = "Yafa Bamboo Toothbrush";
    else if (!/^yafa\b/i.test(baseTitle)) baseTitle = `Yafa ${baseTitle}`;
  }
  if (brand.slug === "sodii") {
    if (!/^sodii\b/i.test(baseTitle)) baseTitle = `Sodii ${baseTitle}`;
  }
  if (brand.slug === "madeof-life") {
    if (/unflavored/i.test(baseTitle)) baseTitle = "Made Of Unflavored Whey Isolate";
    else if (/vanilla/i.test(baseTitle)) baseTitle = "Made Of Vanilla Whey Isolate";
    else if (/chocolate/i.test(baseTitle)) baseTitle = "Made Of Chocolate Whey Isolate";
    else if (!/^made of\b/i.test(baseTitle)) baseTitle = `Made Of ${baseTitle}`;
  }
  if (brand.slug === "doos-naturals") {
    if (/tallow & mineral deodorant/i.test(baseTitle)) baseTitle = "Doo's Tallow & Mineral Deodorant";
    else if (/tallow & zinc sun balm/i.test(baseTitle)) baseTitle = "Doo's Tallow & Zinc Sun Balm";
    else if (/tallow & honey moisturizer/i.test(baseTitle)) baseTitle = "Doo's Tallow & Honey Moisturizer";
    else if (/tallow & beeswax lip/i.test(baseTitle)) baseTitle = "Doo's Tallow & Beeswax Lip Balm";
    else if (/sun lip/i.test(baseTitle)) baseTitle = "Doo's Sun Lip Balm";
    else if (/dry shampoo/i.test(baseTitle)) baseTitle = "Doo's Rosemary Dry Shampoo";
    else if (/hair clay/i.test(baseTitle)) baseTitle = "Doo's Tallow Hair Clay";
    else if (!/^doo'?s\b/i.test(baseTitle)) baseTitle = `Doo's ${baseTitle}`;
  }
  if (brand.slug === "tallow-twins") {
    if (!/^tallow twins\b/i.test(baseTitle)) baseTitle = `Tallow Twins ${baseTitle}`;
  }
  if (brand.slug === "sunshine-fungi") {
    if (!/^sunshine fungi\b/i.test(baseTitle)) baseTitle = `Sunshine Fungi ${baseTitle}`;
  }
  ...
  if (
    brand.slug === "yafa-queen" ||
    brand.slug === "sodii" ||
    brand.slug === "tallow-twins"
  ) {
    ingredients = [];
  }
  if (brand.slug === "madeof-life") {
    ingredients = /unflavored/i.test(title)
      ? ["New Zealand Grass-Fed Whey Protein Isolate", "Sunflower Lecithin"]
      : [
  ...
  if (brand.slug === "doos-naturals") {
    ...
    if (/sun balm/i.test(title) && /zinc|tallow/i.test(text + title)) {
      ingredients = [
        "Grass-Fed Tallow",
        "Non-Nano Zinc Oxide",
        "Beeswax",
    ...
    } else if (/honey moisturizer/i.test(title) && /tallow|honey/i.test(text + title)) {
      ingredients = ["Grass-Fed Tallow", "Raw Honey", "Jojoba Oil", "Beeswax", "Vitamin E"];
    } else if (!looksLikeInci(ingredients, ingredients.join(", "))) {
      ingredients = [];
    }
  }
  if (brand.slug === "sunshine-fungi") {
    if (/dual extract/i.test(title)) {
      ...
      ingredients = /reishi/i.test(title)
        ? ["Dried Reishi Fruiting-Body Powder"]
        : ["Dried Lion's Mane Fruiting-Body Powder"];
    }
  }
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave87.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave87Products: waveCount,
        wave87BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave87-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave87=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
