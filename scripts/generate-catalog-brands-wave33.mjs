/**
 * Write src/lib/catalog-brands-wave33.ts from the wave33 seed.
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { brands } from "./catalog-brands-seed-wave33.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const out = path.join(__dirname, "../src/lib/catalog-brands-wave33.ts");
...
const rows = brands.map((b, i) => {
  const id = `c${String(i + 999).padStart(3, "0")}`;
  return `  {
    id: ${tsString(id)},
