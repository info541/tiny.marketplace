/**
 * Fetch Shopify catalogs for wave75 brands (c1319+).
 * - Keeps every on-niche product (no 25-item cap)
 * - Expands flavor / scent / sunscreen shade variants into their own listings
...
import { brands as seedBrands } from "./catalog-brands-seed-wave75.mjs";
...
const WAVE75_EXTRA =
  /\b(gift card|e-gift|gift pack|gift set|mystery box|shipping protection|package protection|build your (own )?bundle|bundle\b|duo\b|duet\b|trio\b|sets?\b|variety pack|holiday gift|shaker bottle|shakers?\b|sampler pack|ritual\b|makeover|bathroom cup|flavor pack|travel pack|2-pack|3-pack|4-pack|6-pack|tinted\s*lip|ebook|directory|room spray|sunglasses|branded hat|sample pack|best sellers|ultimate bundle|family pack|power duo|starter kit|subscription|laundry|dish wash|pet bar|perfume|irregular|underweight|welcome gift|soap rest|soap saver|empty foamer|wood scoop|banner|replacement shipping|scalp massager|lather bag|natural combs|hair brush|bundle builder|toiletry bag|printable|patient materials|board book|sticker sheet|whitening strips?|electric toothbrush|replacement heads?|charging cord|test product|gua sha|bug repellent|scrap bag|soap keeper|travel tin|travel set|dad cap|water bottle|buy 2|buy one|loose leaf tea|grow kit|fresh mushrooms?|courses?)\b/i;
...
function looksLikeInci(parts, raw) {
  ...
  if (
    parts.length >= 3 &&
    /water|aqua|glycerin|sodium|acid|extract|oil|oxide|protein|vitamin|tallow|wax|butter|zinc|salt|magnesium|potassium|gum|xylitol|hydroxyapatite|clay|shea|hydrobeef|cocoa|glycine|stevia|alcohol|honey|jojoba|goat milk|creatine|whey|theobromine|sorbitol|silica|dates|cacao|cinnamon/i.test(
      raw,
    )
  ) {
    return true;
  ...
  if (
    parts.length >= 2 &&
    /tallow|oil|wax|butter|zinc|beeswax|xylitol|hydroxyapatite|hydrobeef|alcohol|goat milk|whey|dates/i.test(raw) &&
    /,/.test(raw)
  ) {
    return true;
  }
...
function extractIngredients(html) {
  ...
  const vanilla = text.match(
    /Vanilla:\s*([^\n]{12,400})/i,
  );
  if (vanilla) {
    const parts = parseList(vanilla[1]);
    if (looksLikeInci(parts, vanilla[1])) return parts;
  }

  const inci = text.match(
    /\b(?:Aqua|Water|Eau|Glycerin|Sorbitol|Jojoba|Tallow|Organic Sunflower)\b(?:\s*\/\s*(?:Aqua|Water|Eau))?[^.]{0,40}(?:,\s*[A-Za-z0-9][^,]{1,100}){3,60}/i,
  );
  if (inci) {
...
function inferCategory(brandCategories, title, productType, tags) {
  ...
  const rules = [
    ...
    ["supplements", /\b(lion'?s mane|reishi|chaga|cordyceps|shiitake|maitake|oyster mushroom|mushroom (complex|tincture|extract)|adaptogen|creatine|vitamin d|magnesium|shilajit|collagen|maca|chlorella|d-?ribose|capsule|knotweed|dual extract)\b/],
...
function refineCategory(category, title, brand) {
  ...
  if (brand?.slug === "ollie") return "oral";
  if (brand?.slug === "purelytes") return "electrolytes";
  if (brand?.slug === "tidlos") return "protein";
  if (brand?.slug === "little-hands-hawaii") {
    if (/\b(sunscreen|spf|after sun|head to toe|face stick|sport stick)\b/.test(t)) return "sunscreen";
    return "skincare";
  }
  if (brand?.slug === "lavami") {
    if (/\b(shampoo|conditioner|hair|scalp)\b/.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "mushroom-microfarm") return "supplements";
  return category;
}
...
function isOnNiche(raw, brand) {
  ...
  if (SKIP_TITLE.test(title) || WAVE75_EXTRA.test(title)) return false;
  if (brand.slug === "ollie") {
    if (
      /\b(bundle|duo|pair|gift card|pack|kit|strips?|electric|sonic|replacement|charging|sticker|book|bag|subscription|sample|protection|donation|test product|trial|mix & match|enzy)\b/i.test(
        title,
      )
    ) {
      return false;
    }
    return /toothpaste|mouthwash|floss|n[- ]?hap|hydroxyapatite/i.test(hay);
  }
  if (brand.slug === "purelytes") {
    if (/\b(bundle|sample|hat|shaker)\b/i.test(title)) return false;
    return /electrolyt|unflavored|pineapple|lemon|strawberry|mango|purelytes/i.test(hay);
  }
  if (brand.slug === "tidlos") {
    if (/\b(gel|book|ebook|digital|sample|2-pack|2 pack)\b/i.test(title)) return false;
    return /protein|whey/i.test(hay);
  }
  if (brand.slug === "little-hands-hawaii") {
    if (/\b(combo|bundle|pack|kit|set)\b/i.test(title)) return false;
    return /\b(sunscreen|balm|butter|salve|head to toe)\b/i.test(hay);
  }
  if (brand.slug === "lavami") {
    if (/\b(gift card|trio|set|bundle|massager|custom)\b/i.test(title)) return false;
    return /\b(shampoo|conditioner|soap|lotion|lip|oil|facial)\b/i.test(hay);
  }
  if (brand.slug === "mushroom-microfarm") {
    if (/\b(tee|tote|bundle|fresh|grow|course|class)\b/i.test(title)) return false;
    return /dual extract|lion'?s mane|reishi|chaga|cordyceps|turkey tail|knotweed/i.test(hay);
  }
...
function flavorOptionIndex(raw, brand) {
  ...
  if (brand?.slug === "purelytes" || brand?.slug === "mushroom-microfarm") {
    return -1;
  }
  if (brand?.slug === "tidlos") {
    const flavour = options.findIndex((o) => /flavou?r/i.test(o.name || ""));
    return flavour;
  }
  if (brand?.slug === "ollie") {
    return -1;
  ...
  if (brand?.slug === "little-hands-hawaii") {
    ...
    const color = options.findIndex((o) => /colou?r|shade/i.test(o.name || ""));
    if (color >= 0 && /sunscreen|stick|head to toe/i.test(raw.title || "")) return color;
    return -1;
  }
  if (brand?.slug === "lavami") {
    const scent = options.findIndex((o) => /scent/i.test(o.name || ""));
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel, currency) {
  ...
  if (
    flavorClean &&
    /^(default title|single|1 pack|2 pack|3 pack|30 servings|60 servings|90 servings|2oz|4oz|3oz|1oz|6g|scented|buy one|buy two|buy three|1 jar|2 jars|1 tub|2 tubs|3 tubs|250g|500g|1 lb\.?|2 lbs\.?|3 lbs\.?|12 pack|24 pack|36 pack|spf 30|spf 50|30 spf|50 spf|1 lbs|3 lbs|box|no box|no packaging|boxes|no boxes)$/i.test(
      flavorClean,
    )
  ) {
    flavorClean = null;
  }
  if (flavorClean && /\b(months? supply|servings per|per tub|per pouch|lb\.?|lbs\.?|bundle pack)\b/i.test(flavorClean)) {
    flavorClean = null;
  }
  ...
  title = title
    ...
    .replace(/&#39;/g, "'")
    .replace(/&quot;/g, '"');
  if (brand.slug === "purelytes" && !/^purelytes\b/i.test(title)) {
    title = `Purelytes ${title}`;
  }
  ...
  if (brand.slug === "tidlos") {
    const tidlosFormulas = {
      vanilla: [
        "Grass-Fed Whey Protein Concentrate",
        "Organic Dates",
        "Organic Madagascar Vanilla Beans",
      ],
      chocolate: ["Grass-Fed Whey Protein Concentrate", "Organic Dates", "Organic Cacao"],
      snickerdoodle: [
        "Grass-Fed Whey Protein Concentrate",
        "Organic Dates",
    ...
    const key = String(flavorClean || "").toLowerCase();
    if (tidlosFormulas[key]) ingredients = tidlosFormulas[key];
  }
...
function expandProducts(raw, brand, brandId, index, currency) {
  ...
  if (flavorIdx >= 0 && variants.length > 1) {
    ...
    for (const v of variants) {
      ...
      if (!label) continue;
      if (/^\s*\d+(\.\d+)?\s*(oz|ml|g|kg|lb|count|ct|pack|fl\.?\s*oz|servings?|sticks?)/i.test(label)) continue;
      if (/\b(months? supply|servings per|per tub|per pouch|tubs?|lbs?\.?|bundle pack)\b/i.test(label)) continue;
      if (/^(scented|buy one|buy two|buy three|1 jar|2 jars|1 pack|2 pack|3 pack|box|no box|no packaging)$/i.test(label)) continue;
...
async function main() {
    ...
    try {
      ...
      rawProducts.forEach((raw, idx) => {
        for (const p of expandProducts(raw, brand, brandId, idx, currency)) {
          const nameKey = p.name.toLowerCase().replace(/\s+/g, " ").replace(/\.+$/, "").replace(/[—–-]/g, "-");
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave75.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave75Products: waveCount,
        wave75BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave75-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave75=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
