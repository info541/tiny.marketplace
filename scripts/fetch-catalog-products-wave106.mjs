/**
 * Fetch Shopify catalogs for wave106 brands (c1505+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave106.mjs";
...
function refineCategory(category, title, brand) {
  ...
  if (brand?.slug === "underbrush") return "oral";
  if (brand?.slug === "electrik-kids") return "electrolytes";
  if (brand?.slug === "noordcode") return "protein";
  if (brand?.slug === "lazuli-farms") {
    if (/sun balm|spf|after sun/i.test(t)) return "sunscreen";
    if (/deodorant/i.test(t)) return "deodorant";
    if (/shampoo|hair/i.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "cattle-creek") {
    if (/deodorant/i.test(t)) return "deodorant";
    if (/shampoo|pomade|beard|hair/i.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "roots-apothecary") {
    if (/deodorant/i.test(t)) return "deodorant";
    if (/serum|cleanse|salve|soap|tallow|magnesium spray|body/i.test(t) && !/tincture|extract|lion|reishi|cordyceps|turkey|rhodiola|ashwagandha|lemon balm|holy basil/i.test(t))
      return "skincare";
    return "supplements";
  }
  return category;
...
const UNDERBRUSH_KEEP = new Set([
  "remineralizing-gum",
  "pearl-powder-tooth-scrub",
  "pearl-tooth-serum",
  "boars-hair-toothbrush-heads",
  "aluminum-toothbrush-handle",
  "castor-oil-toothbrush-heads",
]);

const ELECTRIK_KEEP = new Set([
  "go-go-grape",
  "power-punch",
  "tropical-turbo",
  "razzy-rocket",
...
const NOORD_KEEP = new Set([
  "organic-pure-whey",
  "organic-pure-whey-cacao",
  "organic-pure-plant-protein",
  "pure-collagen",
  "pure-marine-collagen",
  "complete-collagen-unflavoured",
  "complete-collagen-orange",
]);

const LAZULI_KEEP = new Set([
  "natural-mineral-sunscreen-with-zinc-and-nourishing-tallow-broad-spectrum-uva-and-uvb-coverage-all-natural-ingredients",
  "zinc-lip-balm-with-spf-15-protection-shea-butter-tallow-beeswax-natural-lip-moisturizer",
  "after-sun-spray-cooling-aloe-mist",
  "tallow-deodorant-bergamot-grapefruit-with-arrowroot-zinc-essential-oils-beeswax-baking-soda-free",
  "tallow-deodorant-charcoal-and-tea-tree-with-arrowroot-zinc-essential-oils-beeswax-baking-soda-free",
  "tallow-deodorant-cedarwood-patchouli-lime-with-arrowroot-zinc-essential-oils-beeswax-baking-soda-free",
  "tallow-cleansing-balm-tallow-clay-essential-oils",
  "rose-facial-mist-toner-setting-spray",
  "botanical-face-oil-lightweight-anti-oxidant-rich-hydration",
  "unscented-tallow-manuka-honey-face-cream-for-sensitive-or-mature-skin-tallow-for-face",
  "daily-facial-lotion-gentle-calm-lotion-for-face-hands-and-body",
  "solid-lotion-bar-lemon-meringue-vanilla-lemon-cedarwood",
  "solid-lotion-bar-spa-blend-frankincense-geranium-bergamot-rose",
  "oat-shea-vanilla-lotion-hand-body-lotion",
  "solid-lotion-bar-lavender-sandalwood-sweet-orange",
  "solid-lotion-bar-unscented",
  "lone-wolf-men-s-soap-bar-pine-cypress-cedarwood-frankincense",
  "vanilla-mint-lip-balm-shea-butter-tallow-beeswax-natural-lip-moisturizer",
  "shampoo-bar-for-dry-or-curly-hair-rhassoul-clay-citrus-and-beeswax-for-healthy-shiny-hair-all-natural-shampoo-bar",
  "shampoo-bar-for-normal-hair-rosemary-nettle-and-mint-for-volume-oil-control-all-natural-shampoo-bar",
  "shampoo-bar-with-peppermint-patchouli-for-a-smoother-shave-all-natural-solid-shave-soap-bar",
  "vanilla-grapefruit-lip-balm-shea-butter-tallow-beeswax-natural-lip-moisturizer",
  "sweet-orange-lip-balm-shea-butter-tallow-beeswax-natural-lip-moisturizer",
  "vanilla-almond-lip-balm-tallow-beeswax-natural-lip-moisturizer",
  "cocoa-mint-lip-balm-cocos-butter-tallow-beeswax-peppemint-natural-lip-moisturizer",
  "lavender-she-lip-balm-shea-butter-tallow-beeswax-natural-lip-moisturizer",
  "tallow-lip-balm-tallow-honey-beeswax-lazuli-farms",
  "baby-bum-balm-tallow-beeswax-vanilla-lime-sweet-orange-rosemary-lavender-natural-healing",
  "blemish-stick-with-tea-tree-oul-tallow-beeswax-for-under-eye-circles-bags-and-eye-wrinkles",
  "eye-serum-tallow-beeswax-vanilla-lime-sweet-orange-rosemary-lavender-natural-healing",
  "clarifying-face-cream-for-oily-or-blemish-prone-skin-tallow-with-tea-tree-oil",
  "everything-cream-lazuli-farms",
  "hydrating-face-cream-for-dry-skin-lazuli-farms",
  "healing-salve-stick-tallow-calendula-lavender-lazuli-farms",
  "tallow-body-butter-unscented-lazuli-farms-1",
  "tallow-body-butter-lavender-lemongrass-lazuli-farms",
  "tallow-body-butter-vanilla-lemon-lazuli-farms",
  "lard-soap-with-lemongrass-clary-sage-and-lavender-lazuli-farms",
  "cleansing-charcoal-tea-tree-soap-lazuli-farms",
  "french-lavender-soap-gentle-unscented-lazuli-farms",
  "sweet-orange-soap-gentle-unscented-lazuli-farms",
  "oat-honey-gentle-unscented-lazuli-farms",
  "baby-lamb-soap-gentle-unscented-lazuli-farms",
  "gardener-or-farmer-scrubbing-soap-in-the-dirt-old-fashioned-lard-soap-lazuli-farms",
  "tallow-body-balm-lazuli-farms",
]);

const CATTLE_KEEP = new Set([
  "tallow-deodorant",
  "vanilla-butter",
  "unscented-butter",
  ...
  "face-cream",
  "face-cream-v1",
  "coffee-infused-under-eye-cream",
  "activated-charcoal-facial-cleanser-bar",
  "summer-cream",
  "touch-me-not-cream",
  "tallow-lip-balm",
  "tallow-lip-care",
  "tallow-charcoal-face-soap-bar",
  "soap-bar",
  "shampoo-bar",
  "mechanic-soap",
  "loofah-soap-bar",
...
const ROOTS_KEEP = new Set([
  "lions-mane-tincture-fruiting-body",
  "lions-mane-extract-organic",
  "turkey-tail-mushroom-extract-organic",
  "reishi-mushroom",
  "cordyceps",
  "elemental-mushroom-blend",
  "lemon-balm-tincture",
  "rhodiola-tincture",
  "holy-basil-tincture",
...
const KEEP_BY_SLUG = {
  underbrush: UNDERBRUSH_KEEP,
  "electrik-kids": ELECTRIK_KEEP,
  noordcode: NOORD_KEEP,
  "lazuli-farms": LAZULI_KEEP,
  "cattle-creek": CATTLE_KEEP,
  "roots-apothecary": ROOTS_KEEP,
};
...
function flavorOptionIndex(raw, brand) {
  ...
  const handle = raw.handle || "";
  if (brand?.slug === "electrik-kids") return -1;
  if (brand?.slug === "noordcode") return -1;
  if (brand?.slug === "lazuli-farms") return -1;
  if (brand?.slug === "underbrush") {
  ...
  if (brand?.slug === "roots-apothecary") {
    return options.findIndex((o) => /scent|essential oil|infusion|skin type/i.test(o.name || ""));
  }
...
function pickVariant(variants, brand, raw) {
  ...
  const scored = variants.map((v, i) => {
    ...
    if (/\b(two-pack|2-pack|three-pack|3-pack|four-pack|4-pack|2 boxes|4 boxes|6 boxes|0\.5oz|0\.5 oz|20 servings|3x90|250 g|250g|500 g|500g|1 kg|1kg|5g|sample size|refill only|2 jars|3 jars|5 jars|2 bars|3 bars|8 oz extra|16 oz|5lb|5 lb|family size|81 servings|15 servings|30 ct|3\.3 oz|refill cartridge|30g sample)\b/i.test(hay))
      score -= 8;
    if (brand?.slug === "underbrush") {
      if (/\b(1 pack|single|default title)\b/i.test(hay)) score += 8;
      if (/\b(3 pack|6 pack|12 pack|sizes?)\b/i.test(hay)) score -= 8;
    }
    if (brand?.slug === "cattle-creek") {
      if (/\b(4oz|4 oz|2oz|2 oz)\b/i.test(hay)) score += 6;
      if (/\b(8oz|8 oz|16oz|bundle)\b/i.test(hay)) score -= 6;
    }
    if (brand?.slug === "roots-apothecary") {
      if (/\b(60ml|60 ml|4 oz|single|default title)\b/i.test(hay)) score += 6;
      if (/\b(120ml|8 oz|2-pack|stack)\b/i.test(hay)) score -= 6;
    }
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel, currency) {
  ...
  if (brand.slug === "underbrush") {
    const h = raw.handle || "";
    if (h === "remineralizing-gum") baseTitle = "Underbrush Remineralizing Gum";
    else if (h === "pearl-powder-tooth-scrub") baseTitle = "Underbrush Pearl Powder Tooth Scrub";
    else if (h === "pearl-tooth-serum") baseTitle = "Underbrush Pearl Serum";
    else if (!/^underbrush\b/i.test(baseTitle)) baseTitle = `Underbrush ${baseTitle}`;
  }
  if (brand.slug === "electrik-kids") {
    const h = raw.handle || "";
    const flavorMap = {
      "go-go-grape": "Go Go Grape",
      "power-punch": "Power Punch",
      "tropical-turbo": "Tropical Turbo",
      "razzy-rocket": "Razzy Rocket",
    };
    baseTitle = flavorMap[h] ? `Elec'trik Kids — ${flavorMap[h]}` : `Elec'trik Kids ${baseTitle}`;
  }
  if (brand.slug === "noordcode") {
    const h = raw.handle || "";
    if (h === "organic-pure-whey") baseTitle = "Noordcode Organic Pure Whey";
    else if (h === "organic-pure-whey-cacao") baseTitle = "Noordcode Organic Pure Whey Cacao";
    else if (h === "organic-pure-plant-protein") baseTitle = "Noordcode Organic Pure Plant Protein";
    else if (h === "pure-collagen") baseTitle = "Noordcode Pure Collagen";
    else if (h === "pure-marine-collagen") baseTitle = "Noordcode Pure Marine Collagen";
    else if (h === "complete-collagen-unflavoured") baseTitle = "Noordcode Complete Collagen Unflavoured";
    else if (h === "complete-collagen-orange") baseTitle = "Noordcode Complete Collagen Orange";
    else if (!/^noordcode\b/i.test(baseTitle)) baseTitle = `Noordcode ${baseTitle}`;
  }
  if (brand.slug === "lazuli-farms") {
    const parts = baseTitle.split("|").map((s) => s.trim()).filter(Boolean);
    if (
      parts.length >= 2 &&
      /deodorant|body butter|lotion bar|lip balm|soap/i.test(parts[0]) &&
      !/with arrowroot|shea butter|tallow \+|natural lip|all-natural|for /i.test(parts[1])
    ) {
      baseTitle = `${parts[0]} — ${parts[1]}`;
    } else {
      baseTitle = parts[0];
    }
    if (!/^lazuli\b/i.test(baseTitle)) baseTitle = `Lazuli Farms ${baseTitle}`;
  }
  if (brand.slug === "cattle-creek") {
    baseTitle = baseTitle.replace(/\s+-\s*$/, "").replace(/\s+—\s+stack$/i, "").trim();
    if (!/^cattle creek\b/i.test(baseTitle)) baseTitle = `Cattle Creek ${baseTitle}`;
  }
  if (brand.slug === "roots-apothecary") {
    const h = raw.handle || "";
    if (h === "lions-mane-tincture-fruiting-body") baseTitle = "Roots Lion's Mane Tincture (Fruiting Body)";
    else if (h === "lions-mane-extract-organic") baseTitle = "Roots Lion's Mane Extract";
    else if (h === "turkey-tail-mushroom-extract-organic") baseTitle = "Roots Turkey Tail Mushroom Extract";
    else if (h === "reishi-mushroom") baseTitle = "Roots Reishi Mushroom Extract";
    else if (h === "cordyceps") baseTitle = "Roots Cordyceps Mushroom Extract";
    else {
      baseTitle = baseTitle
        .replace(/\.+$/, "")
        .replace(/\s*\((60|120)\s*ml\)\s*$/i, "")
        .replace(/\s+-\s+cognitive performance.*$/i, "")
        .replace(/\s+-\s+organic$/i, " — Organic")
        .replace(/\s+/g, " ")
        .trim();
      baseTitle = baseTitle
        .split(/(\s+|—)/)
        ...
        .join("");
      if (!/^roots\b/i.test(baseTitle)) baseTitle = `Roots ${baseTitle}`;
    }
  }
  ...
  if (flavorClean && /baker|collection|stack|variety/i.test(flavorClean)) flavorClean = null;
  if (flavorClean && /^sandalwood$/i.test(flavorClean)) flavorClean = "Sandalwood";
  if (flavorClean && brand.slug === "roots-apothecary") {
    flavorClean = flavorClean
      .replace(/\.+$/, "")
      .split(/(\s+|—|&)/)
      .map((part) => {
        if (!part || /^\s+$/.test(part) || part === "—" || part === "&") return part;
        if (/^(to|and|or|of)$/i.test(part)) return part.toLowerCase();
        return part.charAt(0).toUpperCase() + part.slice(1);
      })
      .join("");
  }
  ...
  if (brand.slug === "underbrush" || brand.slug === "electrik-kids") {
    ingredients = [];
  }
  if (brand.slug === "noordcode") {
    const h = raw.handle || "";
    if (h === "organic-pure-whey" || h === "organic-pure-whey-cacao") {
      ingredients = ["Organic grass-fed whey protein concentrate"];
    } else if (h === "organic-pure-plant-protein") {
      ingredients = ingredients.length ? ingredients : ["Organic plant protein"];
    }
  }
  if (brand.slug === "lazuli-farms") {
    if (/deodorant/i.test(title) && /bergamot/i.test(title)) {
      ingredients = [
        "Arrowroot powder",
        "Kaolin clay",
    ...
    } else if (/deodorant/i.test(title) && /charcoal/i.test(title)) {
      ingredients = [
        "Arrowroot powder",
        "Kaolin clay",
        "Grass-fed beef tallow",
    ...
    } else if (/deodorant/i.test(title) && /cedarwood/i.test(title)) {
      ingredients = [
        "Arrowroot powder",
        "Kaolin clay",
        "Grass-fed beef tallow",
  ...
  if (brand.slug === "roots-apothecary") {
    if (/lion.?s mane tincture/i.test(title)) {
      ingredients = ["Lion's Mane fruiting body", "Organic alcohol", "Water"];
    } else if (/lion.?s mane extract/i.test(title)) {
      ingredients = ["Organic Lion's Mane fruiting body"];
    } else if (/turkey tail/i.test(title) && /extract/i.test(title)) {
      ingredients = ["Organic turkey tail fruiting body"];
    } else if (/reishi/i.test(title) && /extract/i.test(title)) {
      ingredients = ["Organic reishi fruiting body"];
    } else if (/cordyceps/i.test(title) && /extract/i.test(title)) {
      ingredients = ["Organic cordyceps fruiting body"];
    }
  }
...
function expandProducts(raw, brand, brandId, index, currency) {
  ...
  if (flavorIdx >= 0 && variants.length > 1) {
    ...
    for (const v of variants) {
      ...
      if (brand?.slug === "cattle-creek" || brand?.slug === "roots-apothecary") {
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave106.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave106Products: waveCount,
        wave106BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave106-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave106=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
