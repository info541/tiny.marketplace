/**
 * Fetch Shopify catalogs for wave93 brands (c1427+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave93.mjs";
...
const FLAVOR_OPTION =
  /flavor|flavour|scent|fragrance|smell|aroma|colour|color|shade|scented|types?|blend|choose your scent|select one|inci|essential oil|choose product|single\/bundle|hair color|hue|formula|choose your flavou?r|approximate spf|variety|options|creamsicle|flavor\/size/i;
...
function refineCategory(category, title, brand) {
  ...
  if (brand?.slug === "zoobalab") return "oral";
  if (brand?.slug === "island-butter") return "sunscreen";
  if (brand?.slug === "blueshift") {
    if (/electrolytes?/.test(t)) return "electrolytes";
    return "supplements";
  }
  if (brand?.slug === "natural-force") {
    if (/whey|bone broth protein|collagen/.test(t)) return "protein";
    return "supplements";
  }
  if (brand?.slug === "tiffs-tallow") {
    if (/fun in the sun|mineral barrier|sun balm|\bspf\b/.test(t)) return "sunscreen";
    if (/deodorants?/.test(t)) return "deodorant";
    return "skincare";
  }
  if (brand?.slug === "josh-rosebrook") {
    if (/\bspf\b|day cream/.test(t)) return "sunscreen";
    if (/shampoo|conditioner|hair (cream|spray)|lift|serum spray/.test(t)) return "hair";
    return "skincare";
  }
  return category;
}
...
function isOnNiche(raw, brand) {
  ...
  if (SKIP_TITLE.test(title) || WAVE93_EXTRA.test(title)) return false;
  if (Number((raw.variants || [])[0]?.price || 0) <= 0) return false;
  if (brand.slug === "zoobalab") {
    return /toothpaste|mineralizing/i.test(hay);
  }
  if (brand.slug === "island-butter") {
    return /island butter/i.test(hay) && /5 oz|everyday/i.test(hay);
  }
  if (brand.slug === "blueshift") {
    if (
      /\b(bottle|blender|sidekick|fuelcell|storage cap|gift card)\b/i.test(title) ||
      /bottle|blender|sidekick|fuelcell|storage-cap|e-gift/.test(handle)
    ) {
      return false;
    }
    return /\b(electrolytes?|sleep|calm|turmeric|immune|joint|skin defense|energy|recover|pre-game|pre-workout)\b/i.test(
      hay,
    );
  }
  if (brand.slug === "natural-force") {
    if (
      /\b(mct|coffee|beans)\b/i.test(title) ||
      /mct|coffee|beans/.test(handle)
    ) {
      return false;
    }
    return /\b(whey|collagen|bone broth|vitamin d|omega)\b/i.test(hay);
  }
  if (brand.slug === "tiffs-tallow") {
    if (
      /\b(sun ritual|nurturing ritual|signature ritual|bundle|shipping|route|soap dish|alder)\b/i.test(title) ||
      /sun-ritual|nourishing-ritual|best-sellers-bundle|routeins|soap-dish|boardwalk/.test(handle)
    ) {
      return false;
    }
    return /\b(deodorant|fun in the sun|mineral barrier|whipped tallow|soap|scrub|polish|tallow balm)\b/i.test(
      hay,
    );
  }
  if (brand.slug === "josh-rosebrook") {
    if (
      /\b(sample|trio|threesome|kit|gift card|brush|cotton rounds|travellers|method|essentials|starter|\bset\b)\b/i.test(
        title,
      ) ||
      /sample|threesome|trio|kit|gift-cards|hair-brush|cotton-rounds|travellers|essential-method|discover-the-essentials|starter-kit|toner-set/.test(
        handle,
      )
    ) {
      return false;
    }
    return /\b(shampoo|conditioner|hair (cream|spray)|lift|serum spray|spf|day cream|cleanser|balm|serum|oil|toner|mask|exfoliator|ethereus|accelerator|concentrate|peptide|eye area|body acid)\b/i.test(
      hay,
    );
  }
...
function flavorOptionIndex(raw, brand) {
  ...
  const handle = raw.handle || "";
  if (brand?.slug === "zoobalab") return -1;
  if (brand?.slug === "island-butter") return -1;
  if (brand?.slug === "blueshift") return -1;
  if (brand?.slug === "josh-rosebrook") return -1;
  if (brand?.slug === "tiffs-tallow") return -1;
  if (brand?.slug === "natural-force") {
    if (handle === "organic-whey-protein-powder-grass-fed") {
      return options.findIndex((o) => /flavor/i.test(o.name || ""));
    }
    return -1;
  }
...
function pickVariant(variants, brand, raw) {
  ...
  const scored = variants.map((v, i) => {
    ...
    let score = 0;
    if (/\b(single|single pack|1 pack|1 box|default title|50 g|50g|30 ml|30g|30 g|100 g|100g|1\.5oz|1\.5 oz|90 servings|full size|unsweetened|1 jar|1 bar|2 oz|4 oz|8 oz|1 pound|1 lb|10 ct|30 servings|1\.7 oz)\b/i.test(hay))
      score += 8;
    if (/\b(two-pack|2-pack|three-pack|3-pack|four-pack|4-pack|2 boxes|4 boxes|6 boxes|0\.5oz|0\.5 oz|20 servings|3x90|250 g|250g|500 g|500g|1 kg|1kg|5g|sample size|refill only|2 jars|3 jars|5 jars|2 bars|3 bars|8 oz extra|16 oz|5lb|5 lb|family size|81 servings|15 servings|30 ct|3\.3 oz|refill cartridge)\b/i.test(hay))
      score -= 8;
    if (brand?.slug === "island-butter") {
      if (/5 oz/i.test(hay)) score += 14;
      if (/3\.4/i.test(hay)) score -= 12;
    }
    if (brand?.slug === "natural-force") {
      if (/\b1 pound\b/i.test(hay)) score += 14;
      if (/\b5lb|5 lb|family size\b/i.test(hay)) score -= 14;
      if (/\b30 servings\b/i.test(hay)) score += 10;
      if (/\b81 servings|15 servings\b/i.test(hay)) score -= 10;
    }
    if (brand?.slug === "blueshift") {
      if (/10 ct/i.test(hay)) score += 12;
      if (/30 ct/i.test(hay)) score -= 12;
    }
    if (brand?.slug === "tiffs-tallow") {
      if (/untitled-apr3|fun-in-the-sun|mineral-barrier/i.test(handle)) {
        if (/4 oz/i.test(hay)) score += 14;
        if (/2 oz/i.test(hay)) score += 4;
      } else {
        if (/2 oz/i.test(hay)) score += 14;
        if (/4 oz/i.test(hay)) score -= 4;
        if (/8 oz/i.test(hay)) score -= 14;
      }
    }
    if (brand?.slug === "josh-rosebrook") {
      if (/8 oz|1\.7 oz|full size/i.test(hay)) score += 12;
      if (/16 oz|extra size|refill cartridge|3\.3 oz/i.test(hay)) score -= 12;
    }
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel, currency) {
  let baseTitle = (raw.title || "")
    ...
    .trim();
  if (brand.slug === "zoobalab") {
    baseTitle = "ZOOBA Lab Fluoride-Free Mineralizing Toothpaste";
  }
  if (brand.slug === "island-butter") {
    baseTitle = "Island Butter Mineral Sun Butter";
  }
  if (brand.slug === "blueshift") {
    if (/electrolytes?/i.test(baseTitle)) baseTitle = "Blueshift Electrolytes Lemon Raspberry";
    else if (/skin defense/i.test(baseTitle)) baseTitle = "Blueshift Skin Defense Plus Hibiscus Orange";
    else if (/joint defense/i.test(baseTitle)) baseTitle = "Blueshift Joint Defense Blackberry Peach";
    else if (/immune defense/i.test(baseTitle)) baseTitle = "Blueshift Immune Defense Blueberry Acai";
    else if (/turmeric/i.test(baseTitle)) baseTitle = "Blueshift Turmeric Citrus Medley";
    else if (/pre-game/i.test(baseTitle)) baseTitle = "Blueshift Pre-Game Raspberry Lime";
    else if (/energy/i.test(baseTitle)) baseTitle = "Blueshift Energy + Focus Forest Berry";
    else if (/recover|post-workout/i.test(baseTitle)) baseTitle = "Blueshift Recover Post-Workout Strawberry Peach";
    else if (/sleep/i.test(baseTitle)) baseTitle = "Blueshift Sleep Lemon Lavender";
    else if (/pre-workout/i.test(baseTitle)) baseTitle = "Blueshift Pre-Workout Wild Grape";
    else if (/calm/i.test(baseTitle)) baseTitle = "Blueshift Calm Blackberry Sage";
    else if (!/^blueshift\b/i.test(baseTitle)) baseTitle = `Blueshift ${baseTitle}`;
  }
  if (brand.slug === "natural-force") {
    if (/whey/i.test(baseTitle)) baseTitle = "Natural Force Organic Grass-Fed Whey";
    else if (/marine collagen/i.test(baseTitle)) baseTitle = "Natural Force Marine Collagen";
    else if (/collagen peptides/i.test(baseTitle)) baseTitle = "Natural Force Organic Collagen Peptides";
    else if (/bone broth/i.test(baseTitle)) baseTitle = "Natural Force Organic Bone Broth Protein";
    else if (/vitamin d/i.test(baseTitle)) baseTitle = "Natural Force Pure Vitamin D";
    else if (/omega/i.test(baseTitle)) baseTitle = "Natural Force Pure Omega-3";
    else if (!/^natural force\b/i.test(baseTitle)) baseTitle = `Natural Force ${baseTitle}`;
  }
  if (brand.slug === "tiffs-tallow") {
    if (/fun in the sun|mineral barrier/i.test(baseTitle))
      baseTitle = "Tiff's Tallow Fun In The Sun Mineral Barrier Balm";
    else if (/deodorants?/i.test(baseTitle)) baseTitle = "Tiff's Tallow Baking-Soda-Free Deodorant";
    else if (/honey lemon/i.test(baseTitle)) baseTitle = "Tiff's Tallow Honey Lemon Goat Milk Soap";
    else if (/milk & oats|milk and oats/i.test(baseTitle))
      baseTitle = "Tiff's Tallow Milk & Oats Goat Milk Soap";
    else if (/garden go+ddess/i.test(baseTitle) && /scrub/i.test(baseTitle))
      baseTitle = "Tiff's Tallow Garden Goddess Ritual Scrub";
    else if (/flourish/i.test(baseTitle) && /scrub/i.test(baseTitle))
      baseTitle = "Tiff's Tallow Flourish Ritual Scrub";
    else if (/garden goddess/i.test(baseTitle))
      baseTitle = "Tiff's Tallow Garden Goddess Whipped Tallow";
    else if (/mountain reserve/i.test(baseTitle))
      baseTitle = "Tiff's Tallow Mountain Reserve Whipped Tallow";
    else if (/beloved beauty/i.test(baseTitle))
      baseTitle = "Tiff's Tallow Beloved Beauty Whipped Tallow";
    else if (/deep work/i.test(baseTitle)) baseTitle = "Tiff's Tallow Deep Work Balm";
    else if (/wise ones/i.test(baseTitle)) baseTitle = "Tiff's Tallow The Wise Ones Whipped Tallow";
    else if (/squeaky-clean|squeaky clean/i.test(baseTitle))
      baseTitle = "Tiff's Tallow Squeaky-Clean Whipped Tallow";
    else if (/lemon cupcake/i.test(baseTitle))
      baseTitle = "Tiff's Tallow Lemon Cupcake Whipped Tallow";
    else if (/lavender lullaby/i.test(baseTitle))
      baseTitle = "Tiff's Tallow Lavender Lullaby Whipped Tallow";
    else if (!/^tiff/i.test(baseTitle)) baseTitle = `Tiff's Tallow ${baseTitle}`;
  }
  if (brand.slug === "josh-rosebrook") {
    if (/nourish shampoo|deep nourishing shampoo/i.test(baseTitle))
      baseTitle = "Josh Rosebrook Nourish Shampoo";
    else if (/nourish conditioner|deep nourishing conditioner/i.test(baseTitle))
      baseTitle = "Josh Rosebrook Nourish Conditioner";
    else if (/balance shampoo/i.test(baseTitle)) baseTitle = "Josh Rosebrook Balance Shampoo";
    else if (/balance conditioner/i.test(baseTitle)) baseTitle = "Josh Rosebrook Balance Conditioner";
    else if (/hair cream|styling cream/i.test(baseTitle))
      baseTitle = "Josh Rosebrook Hair Cream";
    else if (/firm hold hair spray/i.test(baseTitle))
      baseTitle = "Josh Rosebrook Organic Firm Hold Hair Spray";
    else if (/^lift/i.test(baseTitle)) baseTitle = "Josh Rosebrook Lift Spray";
    else if (/serum spray/i.test(baseTitle)) baseTitle = "Josh Rosebrook Serum Spray";
    else if (/tinted.*spf/i.test(baseTitle))
      baseTitle = "Josh Rosebrook Nutrient Day Cream Tinted SPF 30";
    else if (/spf 30/i.test(baseTitle))
      baseTitle = "Josh Rosebrook Nutrient Day Cream SPF 30";
    else if (!/^josh rosebrook\b/i.test(baseTitle))
      baseTitle = `Josh Rosebrook ${baseTitle}`;
  }
  ...
  if (brand.slug === "zoobalab" || brand.slug === "island-butter" || brand.slug === "blueshift") {
    ingredients = [];
  }
  if (brand.slug === "natural-force") {
    if (/unflavored/i.test(title) && /whey/i.test(title)) {
      ingredients = ["Organic Grass-Fed Whey Protein Concentrate"];
    } else if (/marine collagen/i.test(title)) {
      ingredients = ["Marine Collagen Peptides"];
    } else if (/collagen peptides/i.test(title)) {
      ingredients = ["Organic Grass-Fed Collagen Peptides"];
    } else if (/bone broth/i.test(title)) {
      ingredients = ["Organic Grass-Fed Beef Bone Broth Protein"];
    } else {
      ingredients = [];
    }
  }
  if (brand.slug === "tiffs-tallow") {
    ...
    } else if (/honey lemon/i.test(title)) {
      ingredients = [
        "Beef tallow",
        "Olive oil",
        "Coconut oil",
    ...
    } else if (/milk & oats/i.test(title)) {
      ingredients = [
        "Beef tallow",
        "Olive oil",
        "Coconut oil",
    ...
    } else if (/garden goddess whipped/i.test(title)) {
      ingredients = [
        "Tallow",
        "Organic jojoba oil",
        "Sage",
...
function expandProducts(raw, brand, brandId, index, currency) {
  ...
  if (flavorIdx >= 0 && variants.length > 1) {
    ...
    for (const v of variants) {
      ...
      if (brand?.slug === "natural-force" && /\b5lb|5 lb|family size\b/i.test(label)) continue;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave93.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave93Products: waveCount,
        wave93BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave93-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave93=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
