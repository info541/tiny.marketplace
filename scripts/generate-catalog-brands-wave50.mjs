/**
 * Write src/lib/catalog-brands-wave50.ts from the wave50 seed.
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { brands } from "./catalog-brands-seed-wave50.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const out = path.join(__dirname, "../src/lib/catalog-brands-wave50.ts");
...
const rows = brands.map((b, i) => {
  const id = `c${String(i + 1164).padStart(4, "0")}`;
  return `  {
    id: ${tsString(id)},
...
/** Wave 50: small organic / toxic-free brands across marketplace categories (c1164+). */
export const catalogBrandsWave50: Brand[] = [
${rows.join(",\n")},
];
`;
