/**
 * Fetch Shopify catalogs for wave25 brands (c868+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave25.mjs";
...
function inferCategory(brandCategories, title, productType, tags) {
  ...
  const rules = [
    ...
    ["oral", /\b(toothpaste|toothbrush|floss|mouthwash|oral care|oral health|hydroxyapatite|n[- ]?ha|theobromine|oil pull|tongue clean|teeth)\b/],
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave25.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave25Products: waveCount,
        wave25BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave25-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave25=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
