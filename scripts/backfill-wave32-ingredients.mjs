/**
 * Drop leftover merch from wave32 and backfill missing ingredients from product pages.
 */
...
function reclassify(p) {
  ...
  if (/\b(mineral zinc sunscreen|mineral sunstick|sun balm|daydream mineral sunscreen|sunup|sun milk|tallow \+ non-nano zinc)\b/.test(t)) {
    p.category = "sunscreen";
  }
  if (/\b(tooth powder|volcanic tooth)\b/.test(t)) p.category = "oral";
  if (/\b(hydration drops?|electrolytes?)\b/.test(t)) p.category = "electrolytes";
  return p;
}
...
async function main() {
  ...
  const report = {
    ...
    newlyFilled: filled,
    wave32Products: wave.length,
    wave32WithIngredients: withIng,
  };
  fs.writeFileSync(
    path.join(root, "data/wave32-ingredients-backfill.json"),
