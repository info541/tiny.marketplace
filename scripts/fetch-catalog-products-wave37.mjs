/**
 * Fetch Shopify catalogs for wave37 brands (c1019+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave37.mjs";
...
function isOnNiche(raw, brand) {
  ...
  if (brand.slug === "haus-of-sage" && /\b(48 pack|white label|wholesale)\b/i.test(title)) return false;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave37.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave37Products: waveCount,
        wave37BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave37-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave37=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
