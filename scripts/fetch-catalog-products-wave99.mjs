/**
 * Fetch Shopify catalogs for wave99 brands (c1463+).
...
import { brands as seedBrands } from "./catalog-brands-seed-wave99.mjs";
...
const FLAVOR_OPTION =
  /flavor|flavour|scent|fragrance|smell|aroma|colour|color|shade|tint|scented|types?|blend|choose your scent|select one|inci|essential oil|choose product|single\/bundle|hair color|hue|formula|choose your flavou?r|approximate spf|variety|options|creamsicle|flavor\/size/i;
...
function refineCategory(category, title, brand) {
  ...
  if (brand?.slug === "fygg") return "oral";
  if (brand?.slug === "tears-hydration") return "electrolytes";
  if (brand?.slug === "purishh") {
    if (/electrolytes?|\bhydrat(?:e|ion)\b/i.test(t)) return "electrolytes";
    if (/protein|whey|collagen/i.test(t) && !/shampoo|conditioner/i.test(t)) return "protein";
    if (/shampoo|conditioner/i.test(t)) return "hair";
    if (/soap|tallow soap/i.test(t)) return "skincare";
    return "supplements";
  }
  if (brand?.slug === "lenus") {
    if (/sunscreen|sun screen/i.test(t) && !/moisturizer|day face/i.test(t)) return "sunscreen";
    if (/hair|scalp/i.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "verdant-wild") {
    if (/deodorant/i.test(t)) return "deodorant";
    if (/sunwise|zinc oxide cream|sunscreen|spf/i.test(t)) return "sunscreen";
    if (/shampoo|conditioner|hair|scalp|beard|dry shampoo/i.test(t)) return "hair";
    return "skincare";
  }
  if (brand?.slug === "long-beach-mushrooms") return "supplements";
  return category;
}
...
function isOnNiche(raw, brand) {
  ...
  if (SKIP_TITLE.test(title) || WAVE99_EXTRA.test(title) || WAVE99_EXTRA.test(handle)) return false;
  if (Number((raw.variants || [])[0]?.price || 0) <= 0) return false;
  if (brand.slug === "fygg") {
    if (/\b(bundle|routine|travel|stuffed|tusk|electric flosser|slate)\b/i.test(hay)) return false;
    return /^(hydroxyapatite-toothpaste|vanilla-vibes-n-hap-toothpaste|chocolate-swirl-n-hap-toothpaste|coconut-cream-n-hap-toothpaste|nox-toothpaste-4oz|propolis-silk-floss|all-ages-tongue-scraper|probiotic|nox-functional-mints-30-pack|hydroxyapatite-varnish)$/.test(handle);
  }
  if (brand.slug === "tears-hydration") {
    return /^(yuzu-matcha|passionfruit-orange-guava)$/.test(handle);
  }
  if (brand.slug === "purishh") {
    if (/\b(bundle|hat|duo|trio|honey|guide|protocol|gift)\b/i.test(hay)) return false;
    if (/stick-packs/.test(handle)) return false;
    return /^(protein-powder|purishh-electrolytes|micronized-creatine|source-magnesium|vitamin-d3-and-k2|purishh-raw-shilajit|raw-elements-shampoo|raw-elements-conditioner|raw-elements-tallow-soap-lemongrass)$/.test(handle);
  }
  if (brand.slug === "lenus") {
    if (/\b(sample|gift|sanitizer|respiratory|cold sore|set)\b/i.test(hay)) return false;
    if (/6-samples|gift-card|hand-air-sanitizer|lenus-no-39-respiratory|no-32-cold-sore|cleansing-earth-set/.test(handle))
      return false;
    return /(?:^|-)n(?:o)?-\d+/.test(handle) || NICHE.test(hay);
  }
  if (brand.slug === "verdant-wild") {
    if (/\b(candle|gift card|sample|perfume|subscription|club|mystery)\b/i.test(hay)) return false;
    if (/candle|gift-card|sample|perfume|subscription|club|room-body-mist|hekate|persephone|demeter/.test(handle)) return false;
    return NICHE.test(hay);
  }
  if (brand.slug === "long-beach-mushrooms") {
    if (/\b(grow ?kit|fresh|class|coffee|ritual)\b/i.test(hay)) return false;
    return /^(lions-mane-tincture|cordyceps-mushroom-tincture|turkey-tail-tincture|reishi-tincture|lions-mane-capsules|triple-tincture)$/.test(handle);
  }
  return NICHE.test(hay);
}
...
function flavorOptionIndex(raw, brand) {
  ...
  const handle = raw.handle || "";
  if (brand?.slug === "tears-hydration") return -1;
  if (brand?.slug === "long-beach-mushrooms") return -1;
  if (brand?.slug === "lenus") return -1;
  if (brand?.slug === "fygg") {
    if (/hydroxyapatite-varnish/.test(handle)) {
  ...
  if (brand?.slug === "verdant-wild") {
    if (/magnesium-deodorant|beeswax-lip-balm|hydrating-body-cream|shimmer-balm/.test(handle)) {
      return options.findIndex((o) => /scent|hue|formula/i.test(o.name || ""));
    }
    return -1;
  }
...
function pickVariant(variants, brand, raw) {
  ...
  const scored = variants.map((v, i) => {
    ...
    if (/\b(two-pack|2-pack|three-pack|3-pack|four-pack|4-pack|2 boxes|4 boxes|6 boxes|0\.5oz|0\.5 oz|20 servings|3x90|250 g|250g|500 g|500g|1 kg|1kg|5g|sample size|refill only|2 jars|3 jars|5 jars|2 bars|3 bars|8 oz extra|16 oz|5lb|5 lb|family size|81 servings|15 servings|30 ct|3\.3 oz|refill cartridge)\b/i.test(hay))
      score -= 8;
    if (brand?.slug === "fygg") {
      if (/\bsingle\b|1-pack|1 pack/i.test(hay)) score += 12;
      if (/3 pack|3-pack/i.test(hay)) score -= 12;
    }
    if (brand?.slug === "tears-hydration") {
      if (/5 sticks/i.test(hay)) score += 12;
      if (/10 sticks|30 sticks/i.test(hay)) score -= 12;
    }
    if (brand?.slug === "purishh") {
      if (/stick pack/i.test(hay)) score -= 12;
    }
    if (brand?.slug === "verdant-wild") {
      if (/\b4 oz\b|2\.5 oz|metal cap/i.test(hay)) score += 6;
      if (/2 oz|sample|pump top/i.test(hay)) score -= 4;
    }
...
function mapOne(raw, brand, brandId, index, variant, flavorLabel, currency) {
  ...
  if (brand.slug === "fygg") {
    const h = raw.handle || "";
    if (/nox-toothpaste/i.test(h) || /nitric oxide functional toothpaste/i.test(baseTitle))
      baseTitle = "Fygg NOX Nano-Hydroxyapatite Toothpaste — Wild Mint";
    else if (/vanilla/i.test(baseTitle)) baseTitle = "Fygg Nano-Hydroxyapatite Toothpaste — Vanilla Vibes";
    else if (/chocolate/i.test(baseTitle)) baseTitle = "Fygg Nano-Hydroxyapatite Toothpaste — Chocolate Swirl";
    else if (/coconut/i.test(baseTitle)) baseTitle = "Fygg Nano-Hydroxyapatite Toothpaste — Coconut Cream";
    else if (/varnish/i.test(baseTitle)) baseTitle = "Fygg Nano-Hydroxyapatite Remineralizing Varnish";
    else if (/propolis/i.test(baseTitle)) baseTitle = "Fygg Propolis Silk Floss";
    else if (/tongue/i.test(baseTitle)) baseTitle = "Fygg Stainless Steel Tongue Scraper";
    else if (/probiotic/i.test(baseTitle)) baseTitle = "Fygg Oral Probiotic Breath Mint";
    else if (/functional mints/i.test(baseTitle)) baseTitle = "Fygg Nitric Oxide Functional Mints";
    else baseTitle = "Fygg Nano-Hydroxyapatite Toothpaste — Mild Mint";
  }
  if (brand.slug === "tears-hydration") {
    if (/yuzu/i.test(baseTitle)) baseTitle = "Tears Hydration — Yuzu Matcha";
    else baseTitle = "Tears Hydration — Passionfruit Orange Guava";
  }
  if (brand.slug === "purishh") {
    if (/protein/i.test(baseTitle)) baseTitle = "PUR-ISHH Grass-Fed Whey + Collagen Protein";
    else if (/electrolyte/i.test(baseTitle)) baseTitle = "PUR-ISHH Electrolytes";
    else if (/creatine/i.test(baseTitle)) baseTitle = "PUR-ISHH Micronized Creatine";
    else if (/magnesium/i.test(baseTitle)) baseTitle = "PUR-ISHH Source Magnesium";
    else if (/d3|k2/i.test(baseTitle)) baseTitle = "PUR-ISHH Vitamin D3 & K2";
    else if (/shilajit/i.test(baseTitle)) baseTitle = "PUR-ISHH Raw Shilajit";
    else if (/shampoo/i.test(baseTitle)) baseTitle = "PUR-ISHH Raw Elements Shampoo";
    else if (/conditioner/i.test(baseTitle)) baseTitle = "PUR-ISHH Raw Elements Conditioner";
    else if (/soap/i.test(baseTitle)) baseTitle = "PUR-ISHH Raw Elements Tallow Soap — Lemongrass";
    else if (!/^pur/i.test(baseTitle)) baseTitle = `PUR-ISHH ${baseTitle}`;
  }
  if (brand.slug === "lenus") {
    baseTitle = baseTitle.replace(/^Nọ\s*/i, "LENUS No ").replace(/^No\s+/i, "LENUS No ");
    if (!/^lenus\b/i.test(baseTitle)) baseTitle = `LENUS ${baseTitle}`;
  }
  if (brand.slug === "verdant-wild") {
    if (!/^verdant\b/i.test(baseTitle)) baseTitle = `Verdant Wild ${baseTitle}`;
  }
  if (brand.slug === "long-beach-mushrooms") {
    if (/triple/i.test(baseTitle)) baseTitle = "Long Beach Mushrooms Triple Blend Tincture";
    else if (/capsule/i.test(baseTitle)) baseTitle = "Long Beach Mushrooms Lion's Mane Capsules";
    else if (/cordyceps/i.test(baseTitle)) baseTitle = "Long Beach Mushrooms Cordyceps Tincture";
    else if (/turkey/i.test(baseTitle)) baseTitle = "Long Beach Mushrooms Turkey Tail Tincture";
    else if (/reishi/i.test(baseTitle)) baseTitle = "Long Beach Mushrooms Reishi Tincture";
    else baseTitle = "Long Beach Mushrooms Lion's Mane Tincture";
  }
  ...
  if (
    brand.slug === "fygg" ||
    brand.slug === "tears-hydration" ||
    brand.slug === "purishh" ||
    brand.slug === "verdant-wild"
  ) {
    ingredients = [];
  }
  if (brand.slug === "lenus") {
    if (/no 18|hyaluronic acid/i.test(title) && /regenerating cream/i.test(title)) {
      ingredients = [];
    }
    if (ingredients[0] && /for normal to dry|try no\d+|contains nuts|fragrance free/i.test(ingredients[0])) {
      ingredients = ingredients.filter((s) =>
        /oil|butter|juice|wax|oxide|extract|distillate|glycerin|aloe|zinc|tocopherol|protein|acid|water|aqua|beeswax|clay|honey/i.test(
          s,
        ) && !/for normal|try no|contains nuts|fragrance free/i.test(s),
      );
    }
    if (/no 43|mineral sunscreen/i.test(title)) {
      ingredients = [
  ...
  if (brand.slug === "long-beach-mushrooms") {
    if (/capsule/i.test(title)) {
      ingredients = ["Organic Lion's Mane mushroom (fruiting body)"];
    } else if (/triple/i.test(title)) {
      ingredients = [
        "Organic Lion's Mane mushroom (fruiting body)",
        "Organic Reishi mushroom (fruiting body)",
        "Organic Turkey Tail mushroom (fruiting body)",
        "Organic cane alcohol",
        "Filtered water",
      ];
    } else if (/cordyceps/i.test(title)) {
      ingredients = [
        "Organic Cordyceps mushroom (fruiting body)",
        "Organic cane alcohol",
        "Filtered water",
      ];
    } else if (/turkey/i.test(title)) {
      ingredients = [
        "Organic Turkey Tail mushroom (fruiting body)",
        "Organic cane alcohol",
        "Filtered water",
      ];
    } else if (/reishi/i.test(title)) {
      ingredients = [
        "Organic Reishi mushroom (fruiting body)",
        "Organic cane alcohol",
        "Filtered water",
...
function expandProducts(raw, brand, brandId, index, currency) {
  ...
  if (flavorIdx >= 0 && variants.length > 1) {
    ...
    for (const v of variants) {
      ...
      if (brand?.slug === "purishh" && /\+|bundle|packets/i.test(label)) continue;
      if (brand?.slug === "fygg" && /^(single|3 pack|3-pack)$/i.test(label)) continue;
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(root, "data/catalog-products-manifest-wave99.json"),
    JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        wave99Products: waveCount,
        wave99BrandsWithProducts: perBrand.filter((b) => b.kept > 0).length,
  ...
  fs.writeFileSync(
    path.join(root, "data/wave99-selected.json"),
    JSON.stringify(
      seedBrands.map((b, i) => ({
        slug: b.slug,
        shopBase: b.shopBase,
  ...
  console.log(
    `\nDone total=${all.length} wave99=${waveCount} brandsWithProducts=${perBrand.filter((b) => b.kept > 0).length} failures=${failures.length}`,
  );
}
